...
// Write a JSON value that EXPIRES. For caches that are keyed by something open-ended — a ticker
// typed into a panel — so the store does not accumulate one entry per name ever looked at.
export async function kvSetJsonEx(key, value, seconds) {
  const res = await command(['SET', key, JSON.stringify(value), 'EX', String(Math.max(1, Math.round(seconds)))]);
  return res === 'OK' || res === true;
}
...
// The last thing the scheduled reconciliation did, for the console to show. Kept OUT of the console
// object on purpose: a browser save replaces that wholesale, so a note stored inside it would be
// wiped by the next save — silently, and only sometimes, which is the worst kind of wrong.
export const FLEX_NOTE_KEY = 'dvcap:flex:note:v1';
// The last wallet composition that was posted. Only what the diff needs — symbol, chain, quantity
// and the price it was valued at — never the derived total, which is not a thing to remember.
// v2: Hyperliquid spot joined the flattened row set. A v1 snapshot has no Hyperliquid rows, so
// diffing against it would report every one of them as newly bought. Bumping the key makes the next
// run a FIRST run, which seeds silently and posts nothing — the mechanism already built for exactly
// this, rather than a migration that has to be got right once and is then dead code forever.
export const WALLET_SNAPSHOT_KEY = 'dvcap:wallet:snapshot:v2';
// The day's events, accumulated by the half-hourly detection runs and drained by the one daily
// post. Separate from the snapshot on purpose: the snapshot is "what the wallet looks like now",
// the buffer is "what it did since the last card", and only the second is cleared by posting.
export const WALLET_PENDING_KEY = 'dvcap:wallet:pending:v2';
// Chain:contract → true, for every token the chain ever confirmed the wallet swapped for. Never
// cleared: an unknown today is answered by any confirmation on any earlier day (lib/walletcard.js).
export const WALLET_PROVENANCE_KEY = 'dvcap:wallet:provenance:v1';
