...
export function unrecordedTrades(rows = [], trades = [], { since = null, splitIntoTrades, derivePosition } = {}) {
  ...
  const byRoot = new Map();
  for (const t of mine) {
    const k = matchKey(t.root) || t.root || t.symbol;
    byRoot.set(k, [...(byRoot.get(k) || []), t]);
  }

  const trips = [], leftovers = [];
  for (const [key, ts] of byRoot) {
    ...
    for (const seg of splitIntoTrades(fills, { side })) {
      const d = derivePosition(seg, { side });
      const row = {
        root: ts[0].root, symbol: ts[0].symbol, key, side, kind: classOf(ts[0]),
