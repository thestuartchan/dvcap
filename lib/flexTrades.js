...
const normalise = (raw) => {
  ...
  return {
    // Whichever id this shape carries. It only has to be stable and unique — it is the thing that
    // stops a trade being recorded twice, and therefore the thing that makes a 30-day window safe.
    tradeId: String(raw.tradeID || raw.ibOrderID || raw.orderID || raw.transactionID || '').trim() || null,
