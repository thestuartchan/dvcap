...
import { rootOf, matchKey, classOf, isoDate, elements, rewindQty, COST_TOLERANCE_PCT } from './flex.js';
...
export function verify(after = [], positions = [], { derive, roots = null, asOf = null } = {}) {
  ...
  for (const key of new Set([...rowsByRoot.keys(), ...(touched || [])])) {
    ...
    const qty = open.reduce((a, x) => {
      const back = rewindQty(x.d, asOf);
      return a + (back == null ? (x.d.qty || 0) : back);
    }, 0);
