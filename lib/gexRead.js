...
export function gexRead({ row, byStrike = [], grid = null, now = new Date(), live = false } = {}) {
  ...
  if (grid?.dominated && grid.frontExpiry) {
    ...
    if (wa.majority) {
      ...
      const only = wa.agree === 1 ? wa.matched[0] : null;
      const expiringToday = only && String(only) === todayIn(now);
      lines.push(`⚠ Only ${wa.agree} of ${wa.total} expiries peaks at these strikes${only ? ` — ${only}${expiringToday ? ', which expires today' : ''}` : ''}. The rest peak wider, so these walls are that book rather than a multi-expiry level.`);
    }
  }
