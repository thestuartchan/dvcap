...
import { levelsOf, balanceOf, regimeDetail } from './gexLevels.js';
...
export function gexRead({ row, byStrike = [], grid = null, now = new Date(), live = false, kind = null, levels = null, atr = null } = {}) {
  ...
  if (skew?.heavier === 'below') {
    ...
    // ── "ROUGHLY BALANCED" WAS PRINTED AGAINST −$0.16B / +$6.97B ──
    // Four boards running. Balanced only when the smaller side is at least 40% of the larger;
    // otherwise the direction is named, because the direction is the trade-relevant sentence and
    // it was the one never printed.
    const bal = balanceOf({ above: skew.above, below: skew.below });
    if (bal.sentence) lines.push(bal.sentence);
  }

  // ── THE OBJECTS, NOT THE LABELS ──
  // "Walls at 700 and 750" named the strike with the most put-side gamma a wall whatever its
  // sign, its distance, or whether any expiry peaked there — a trapdoor on 17 and 18 Sep, a sum
  // on the 21st, the at-the-money pin on the 22nd. The read now says what each strike DOES:
  // cushion (put support), acceleration (trapdoor), pin. lib/gexLevels.js holds the tests.
  const cw = num(row.callWall), s = num(row.spot);
  const lv = levels || ((byStrike?.length && s != null)
    ? levelsOf({ byStrike, grid, spot: s, atr: atr ?? row.atr ?? null, callWall: cw })
    : null);
  if (lv) lines.push(regimeDetail(lv));
  else if (cw != null && s != null) {
    lines.push(`Call wall at ${f2(cw)} (${cw > s ? 'ceiling, above spot' : 'magnet, below spot'}). The put side — support, trapdoor and pin — is read from the per-strike rows, which this row does not carry.`);
  }
  ...
  // decision.
  // THE CALL WALL ONLY. The put side no longer has a headline strike to test: put support
  // qualifies by peaking in at least one expiry (its tile prints the count), the trapdoor is a
  // negative node and "does an expiry peak there" is its own tile's business, and a pin is by
  // definition the nearest expiry's book.
  const wa = wallAgreement(grid, row.callWall, null);
  if (grid?.dominated && grid.frontExpiry) {
    lines.push(`⚠ ${grid.frontExpiry} carries ${grid.frontShare}% of the gamma in the expiries read — these levels are largely one expiry's book, and it expires. Treat them as today's levels, not the week's.`);
  } else if (wa && wa.total > 1 && cw != null) {
    const named = (w) => w.agree > 0 && w.agree <= 2
      ? ` (${w.matched.map(m => String(m) === todayIn(now) ? `${m}, expiring today` : m).join(' / ')})` : '';
    if (wa.majority) {
      lines.push(`The call wall ${f2(cw)} holds across ${wa.agree} of ${wa.total} expiries${grid.frontShare != null ? `, the nearest carrying ${grid.frontShare}%` : ''} — a level rather than one day's positioning.`);
    } else if (wa.agree === 0) {
      // A call wall no expiry owns is a zone to fade into, not a line.
      lines.push(`⚠ No expiry peaks at the call wall ${f2(cw)} (0 of ${wa.total}) — it is a sum across expiries that peak elsewhere, so read it as a zone rather than a line.`);
    } else {
      lines.push(`The call wall ${f2(cw)} is ${wa.agree} of ${wa.total} expiries' own peak${named(wa)} — one book's level more than the board's.`);
    }
  }

  // ── A SINGLE NAME IS NOT AN INDEX ETF ──
  // Every threshold above — the flip margin, the cross-check tolerances, "walls hold across n
  // expiries" — was calibrated on QQQ and SPY: deep chains, dollar-spaced strikes, twenty expiries.
  // A single name's open interest clusters at round strikes and its chain is thinner, so the walls
  // are real (that is where the hedging is) while the flip zone is wider and less of a regime
  // signal. Said here, on the read, because the panel otherwise wears the same wording for both.
  if (kind === 'single-name' || kind === 'single-name-etf') {
    lines.push('Single-name chain: open interest clusters at round strikes and the book is thinner than an index ETF\'s, so the walls carry more than the flip — read the flip zone as wide, and the regime headline as a lean rather than a finding.');
  }
  ...
  return {
    ok: true, headline, state, stance, lines, age, regime: reg, skew, levels: lv,
    balance: skew ? balanceOf({ above: skew.above, below: skew.below }).state : null,
