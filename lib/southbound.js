...
// `diag` is the feed's own failure report ({ failed:[symbols], reason }) — when present the panel
// states what ACTUALLY failed instead of the old hardcoded guess that always blamed the A-share.
export function ahPremiumRead(level, d5, d20, diag = null) {
  if (level == null) {
    const why = diag?.reason ? ` — ${diag.reason}` : " — the price feed did not return the legs";
    return { text: `SMIC A/H premium unavailable${why}. Falling back to the manual Connect holding.`, tone: "muted" };
  }
