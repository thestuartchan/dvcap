...
// `filesFor` attaches pictures to ONE part: the first part whose text contains `marker`. That
// part is posted as a text card followed by one picture card per file; every other part is posted
// exactly as before. A part that carries pictures still goes out if the pictures fail to attach —
// it falls back to the plain post — so a rendering problem can never lose the brief.
export async function postLong(webhook, message, { limit = EMBED_LIMIT, rule = SECTION_RULE, label = null, breakAfter = [], filesFor = null } = {}) {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      ...
      const withFiles = filesFor?.files?.length && filesFor.marker && chunks[i].includes(filesFor.marker) && !filesFor.used;
      let id = null, pictures = null;
      if (withFiles) {
        filesFor.used = true;
        // CARDS, when the caller composed them: each instrument's picture under the text that
        // precedes its read. The part's own head line ("part 1 of 2") leads the first card. The
        // cards are used only when they carry the whole chunk — the marker in the first card is
        // the check — so a chunk that grew past the map can never lose text to them.
        // Whatever the chunk carries BEFORE the marker — the brief's title line and its rule — is
        // not in the cards, which were composed from the map alone, and it belongs at the top.
        const before = chunks[i].slice(0, chunks[i].indexOf(filesFor.marker));
        const cards = Array.isArray(filesFor.cards) && filesFor.cards.length && String(filesFor.cards[0]?.description || '').includes(filesFor.marker)
          ? filesFor.cards.map((c, k) => k === 0 ? { ...c, description: `${head}${before}${c.description}` } : c) : null;
        const cp = cards ? cardsPayload(cards, filesFor.files, { limit }) : null;
        if (cp && cp.files.length) {
          id = await postWithFiles(webhook, { embeds: cp.embeds, attachments: cp.attachments }, cp.files);
          pictures = id != null ? cp.files.length : 0;
        } else {
          id = await postWithFiles(webhook, imagePayload(filesFor.files, { description: body.slice(0, limit), limit }), filesFor.files);
          pictures = id != null ? filesFor.files.length : 0;
        }
        // The pictures could not be attached: the words still go, on their own.
        if (id == null) id = await post(webhook, { embeds: [{ description: body.slice(0, limit) }] });
      } else {
        id = await post(webhook, { embeds: [{ description: body.slice(0, limit) }] });
      }
      results.push({ part: i + 1, ok: id != null, id, ...(pictures != null ? { pictures } : {}) });
...
// ── PICTURES INSIDE THE CARD ─────────────────────────────────────────────────
// Discord takes attachments as multipart/form-data: `payload_json` carries the message and names
// each file by index, `files[n]` carries the bytes. An embed can show one attached picture by
// referring to it as attachment://<filename>, and a message can carry several embeds — so the map
// part of the brief goes out as one message whose first card is the text and whose next cards are
// the ladders, one per instrument, rather than as a separate gallery message ahead of it. The
// payload builder is pure so the shape is tested; the post uses the platform FormData and never
// touches `call`'s JSON path.
export function imagePayload(files = [], { content = '', description = null, limit = EMBED_LIMIT } = {}) {
  const embeds = [];
  if (description) embeds.push({ description: String(description).slice(0, limit) });
  for (const f of files) embeds.push({ image: { url: `attachment://${f.filename}` } });
  return {
...
// Cards in order, each with its text and, optionally, the picture that goes under it. The
// attachments are the files the cards name, in first-use order; Discord caps a message at ten
// embeds and 6,000 characters across them, and both are enforced here rather than discovered.
export const EMBEDS_MAX = 10;
export const EMBEDS_TOTAL = 6000;
export function cardsPayload(cards = [], files = [], { limit = EMBED_LIMIT } = {}) {
  const byName = new Map((files || []).map(f => [f.filename, f]));
  const used = [];
  const embeds = [];
  let total = 0;
  for (const c of (cards || []).slice(0, EMBEDS_MAX)) {
    const e = {};
    if (c.description) {
      const d = String(c.description).slice(0, Math.max(0, Math.min(limit, EMBEDS_TOTAL - total)));
      if (d) { e.description = d; total += d.length; }
    }
    if (c.filename && byName.has(c.filename)) {
      if (!used.includes(c.filename)) used.push(c.filename);
      e.image = { url: `attachment://${c.filename}` };
    }
    if (e.description || e.image) embeds.push(e);
  }
  return { embeds, attachments: used.map((name, i) => ({ id: i, filename: name })), files: used.map(n => byName.get(n)) };
}

export async function postWithFiles(webhook, payload, files = []) {
  if (!isWebhookUrl(webhook) || !files.length) return null;
  try {
    const form = new FormData();
    form.append('payload_json', JSON.stringify(payload));
    files.forEach((f, i) => form.append(`files[${i}]`, new Blob([f.bytes], { type: 'image/png' }), f.filename));
    const r = await fetch(`${webhook}?wait=true`, { method: 'POST', body: form, signal: AbortSignal.timeout(15000) });
    if (!r.ok) { console.error('discord files', r.status); return null; }
    const j = await r.json().catch(() => null);
    return j?.id ?? null;
  } catch (e) {
    console.error('discord files', e?.message || e);
    return null;
  }
}
