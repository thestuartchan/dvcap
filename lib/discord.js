...
// each file by index, `files[n]` carries the bytes. The payload builder is pure so the shape is
// tested; the post itself uses the platform FormData and never touches `call`'s JSON path.
export function imagePayload(files = [], { content = '' } = {}) {
  return {
...
export async function postImages(webhook, { content = '', files = [] } = {}) {
  if (!isWebhookUrl(webhook) || !files.length) return null;
  try {
    const form = new FormData();
    form.append('payload_json', JSON.stringify(imagePayload(files, { content })));
    ...
    if (!r.ok) { console.error('discord image', r.status); return null; }
    const j = await r.json().catch(() => null);
    return j?.id ?? null;
  } catch (e) {
    console.error('discord image', e?.message || e);
    return null;
  }
}
