...
export function splitForDiscord(message, { limit = EMBED_LIMIT, rule = SECTION_RULE } = {}) {
  ...
  for (const part of parts) {
    ...
    if (part.length <= limit) { cur = part; continue; }
    let buf = '';
...
export async function postLong(webhook, message, { limit = EMBED_LIMIT, rule = SECTION_RULE, label = null } = {}) {
  const chunks = splitForDiscord(message, { limit, rule });
