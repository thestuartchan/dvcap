...
// heads are. All three are dated the first of the month they describe and print three to five
// weeks later, so the tolerance is one publication cycle plus slack, in business days.
export const MONTHLY_SERIES = {
  cfnai:    { id: 'CFNAIMA3',  label: 'Chicago Fed activity (3m avg)', unit: 'idx', expectTitle: 'national activity index', lagBizDays: 50, scale: 1 },
  tempHelp: { id: 'TEMPHELPS', label: 'Temp-help employment',         unit: 'k',   expectTitle: 'temporary help',          lagBizDays: 45, scale: 1 },
  hours:    { id: 'AWHMAN',    label: 'Manufacturing hours',           unit: 'h',   expectTitle: 'average weekly hours',    lagBizDays: 45, scale: 1 },
};
...
function cfnaiLeg(raw, m, now) {
  ...
  return {
    ...
    read: `Chicago Fed index ${sgn(value)} (3-month average), ${word}${delta != null ? `, ${sgn(delta)} on the month` : ''}`,
...
export function monthlyPulse(raw = {}, { now = new Date() } = {}) {
  return compose([
...
export function growthAxis({ market, weekly, monthly } = {}) {
  const present = [['market', market], ['weekly', weekly], ['monthly', monthly]]
    .map(([k, p]) => ({ key: k, lean: lean(p?.verdict), verdict: p?.verdict }))
  ...
  if (present.length === 1) {
    ...
    return { state: only.key === 'market' ? 'market-only' : 'data-only',
             read: `Only ${LEG_NAME[only.key]} reads (${only.verdict.toLowerCase()}) — the other legs have too few usable inputs` };
  }
  ...
  // The faster legs share one lean and the slower legs another: a turn in progress.
  const i = present.findIndex(x => x.lean !== present[0].lean);
  const leading = present.slice(0, i), trailing = present.slice(i);
  if (trailing.every(x => x.lean === trailing[0].lean)) {
    const a = leading[0].lean, b = trailing[0].lean;
    const who = leading.length === 1 && leading[0].key === 'market' ? 'the market leads' : 'the faster series lead';
    return { state: a < b ? 'turning-down' : 'turning-up',
  ...
  return { state: 'split', read: present.map(x => `${LEG_NAME[x.key]} ${x.verdict.toLowerCase()}`).join(', ') };
}
const cap = str => str.charAt(0).toUpperCase() + str.slice(1);
