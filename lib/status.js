...
// hardcodes a hex — and since the theme tokens, neither does this file: each state names the
// semantic token (pos / warn / elevated / danger) and the palette on <html data-theme> says what
// colour that is today.
import { C } from './tokens.js';

export const STATUS = Object.freeze({
  BENIGN:   { token: 'BENIGN',   color: C.green,  bg: C.gBg, bdr: C.gBdr, rank: 0, meaning: 'No stress. Metric within normal range' },
  WATCH:    { token: 'WATCH',    color: C.amber,  bg: C.aBg, bdr: C.aBdr, rank: 1, meaning: 'Softening or approaching a threshold. Not actionable alone' },
  ELEVATED: { token: 'ELEVATED', color: C.orange, bg: C.oBg, bdr: C.oBdr, rank: 2, meaning: 'Meaningful deterioration. Actionable in combination' },
  DANGER:   { token: 'DANGER',   color: C.red,    bg: C.rBg, bdr: C.rBdr, rank: 3, meaning: 'Threshold breached' },
});
export const STATUS_TOKENS = Object.keys(STATUS);
// I.5 — the allow-list a lint check can assert against (token strings now, not hexes).
...
// `regimeConviction` is lib/regimeVintage.js's verdict on whether the consensus behind the regime
// is fresh enough to make a case. A decayed consensus still names the regime on the card; it does
// not get to argue for Stage 2 with it — the same rule that stops a contested regime doing so.
export function deriveAction({ oas, regimeLabel, regimePct, regimeContested, regimeConviction = true, labourVerdict, labourSevere, ratesNote, vintages = {} }) {
  ...
  if (!regimeContested && regimeConviction && regimePct != null && regimePct >= 50) rawStage = Math.max(rawStage, 2);
  ...
  if (regimeLabel) inputs.push({ name: 'regime', value: `${regimeLabel}${regimePct != null ? ` ${regimePct}%` : ''}${regimeContested ? ' (contested)' : ''}${!regimeConviction ? ' (consensus decayed — not arguing)' : ''}`, status: null, vintage: vintages.regime ?? null });
