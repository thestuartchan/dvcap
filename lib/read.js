...
function buildStructured({ credit, hyg, cross, korea, kofiaLatest, leaning, regimeSignal, conflicts, flip, caveats, confidence }) {
  ...
  if (credit?.level) {
    const state = `${credit.level}${credit.word ? ', ' + credit.word.toLowerCase() : ''}`;
    let detail = credit.d1?.to != null ? `OAS ${credit.d1.to}` : 'OAS —';
    if (credit.d1) detail += ` (${signed(credit.d1.delta)} ${credit.d1.basis})`;
    if (hyg?.available && hyg.changePct != null) detail += ` · HYG ${signed(hyg.changePct)}%`;
    rows.push({ label: 'CREDIT', state, detail, tone: /widen|stress|recession/i.test(state) ? 'amber' : 'muted' });
  }
