...
export function composeRead(state = {}) {
  const { credit, korea, cross, hyg, leaning, regimeSignal, kofiaLatest, staleNotes = [], usRthOpen = true } = state;
  const c = citer();
  const sentences = [];
  const conflicts = [];
  ...
  if (priorUsClose) caveats.push('US market shut — credit/VIX/NQ clauses are the prior US close, not a live read');
  ...
  return {
    sentences, conflicts, flip, confidence, caveats, structured,
    citations: c.cites,
    text: [
      ...sentences,
