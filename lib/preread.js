...
// and US hold both. A warning that is always on is not a warning; it teaches the reader to skip
// the panel, which is the one place a genuinely missed brief would show.
//
// Named here rather than read from vercel.json at runtime, because a serverless function should
// not depend on a config file being bundled beside it — and named in ONE place, with a test in
// test/preread.test.mjs asserting it matches vercel.json's actual cron paths. Add an EU cron and
// that test fails until this list is updated, which is the point.
export const SCHEDULED_REGIONS = Object.freeze(['asia', 'us']);
