...
// ─── Part B — bidirectional handoff CHAIN ─────────────────────────────────────
// The user trades the daily sequence (HKT): Asia 08:00 → Europe 15:00 → US 21:30 → (next) Asia.
// US → Asia was the only leg built; the structural edge is the OTHER direction — the user watches
// Asia (and Europe) trade live before anyone in New York has to decide. Each leg asks the same
// question of the "from" market's latest session, against the market that traded just upstream:
//   LED       — moved on LOCAL drivers the upstream did not (or amplified it). The downstream open
//               has NOT priced this. Tradeable.
//   FOLLOWED  — replayed the upstream move. No edge; already priced.
//   DIVERGED  — moved AGAINST the upstream. Rejected the read — the highest-information case.
export const CHAIN_CFG = Object.freeze({
  minPct:    0.3,  // |from move| below this → too quiet to read a leg
  quietRef:  0.3,  // |upstream move| below this → the upstream was quiet, so any real from-move LED
  leadRatio: 1.8,  // same-sign but |from| / |ref| beyond this → LED (amplified), else FOLLOWED
});

function legState(fromPct, refPct, cfg = CHAIN_CFG) {
  if (fromPct == null) return { state: 'NO DATA', tone: 'muted', note: 'no session data for this leg' };
  if (Math.abs(fromPct) < cfg.minPct) return { state: 'QUIET', tone: 'muted', note: `moved ${fmtPct(fromPct)} — too quiet to read a handoff` };
  const fs = sign(fromPct), rs = sign(refPct == null ? 0 : refPct);
  if (refPct == null || Math.abs(refPct) < cfg.quietRef)
    return { state: 'LED', tone: 'green', note: `moved ${fmtPct(fromPct)} on local drivers while the upstream was quiet (${refPct == null ? 'n/a' : fmtPct(refPct)}) — the downstream open has not priced this. Tradeable.` };
  if (fs !== rs)
    return { state: 'DIVERGED', tone: 'amber', note: `moved ${fmtPct(fromPct)} AGAINST the upstream ${fmtPct(refPct)} — rejected the read. Highest-information case.` };
  const ratio = Math.abs(fromPct) / Math.abs(refPct);
  if (ratio > cfg.leadRatio)
    return { state: 'LED', tone: 'green', note: `amplified the upstream ${fmtPct(refPct)} to ${fmtPct(fromPct)} — added local information beyond the replay.` };
  return { state: 'FOLLOWED', tone: 'muted', note: `replayed the upstream ${fmtPct(refPct)} (moved ${fmtPct(fromPct)}) — no edge, already priced.` };
}

// asiaPct / europePct / usPct: each region's aggregate equity session move (%). Returns the three
// legs in trading order, each labelled from→to with its LED/FOLLOWED/DIVERGED state and the upstream
// it was judged against.
export function handoffChain({ asiaPct = null, europePct = null, usPct = null } = {}, cfg = CHAIN_CFG) {
  const legs = [
    { from: 'ASIA',   to: 'EUROPE', fromPct: asiaPct,   refName: 'overnight US', refPct: usPct,     ...legState(asiaPct, usPct, cfg) },
    { from: 'EUROPE', to: 'US',     fromPct: europePct, refName: 'Asia',         refPct: asiaPct,   ...legState(europePct, asiaPct, cfg) },
    { from: 'US',     to: 'ASIA',   fromPct: usPct,     refName: 'Europe',       refPct: europePct, ...legState(usPct, europePct, cfg) },
  ];
  return { available: asiaPct != null || europePct != null || usPct != null, legs };
}
