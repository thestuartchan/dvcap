...
// ── THE SERVER'S OWN LOGOS ─────────────────────────────────────────────────────
// Custom emoji uploaded to the server the wallet card posts into, by id — Discord's API cannot
// resolve a name, only <:name:id>. Taken from the server on 2026-09-25, once the operator had the
// access the emoji settings need. They sit BETWEEN the operator's override and the Unicode marks:
// DISCORD_CHAIN_EMOJI still wins per chain (set a chain back to a Unicode mark there if an emoji is
// ever deleted, since a dangling id prints as :name: text), and every chain not named here keeps
// its Unicode stand-in. HyperEVM and Hyperliquid are two venues under one logo.
export const SERVER_EMOJI = Object.freeze({
  'Wallet': '<:Metamask:1547031346071736392>',
  'Ethereum': '<:Ethereum:1547031296696516728>',
  'Robinhood Chain': '<:Robinhood:1547031326727864352>',
  'Hyperliquid': '<:Hyperliquid:1547031379160858746>',
  'HyperEVM': '<:Hyperliquid:1547031379160858746>',
});
...
// The wallet's own mark, on the header line INSIDE the description.
//
// It went through the embed's author slot first, which takes an ordinary image URL and so rendered
// the real MetaMask logo with no setup. That slot is unusable here for a reason no amount of
// configuration fixes: Discord renders `author` ABOVE `title`, always, so the wallet line landed on
// top of "Daily Summary" and the card read back to front.
//
// Order beats fidelity. A unicode fox is not the MetaMask mark but it is recognisably the same
// animal, it renders in any slot, and it needs nothing. If an emoji id ever does turn up, naming
// "Wallet" in DISCORD_CHAIN_EMOJI swaps the real logo in here with no other change.
export const WALLET_MARK = '🦊';

export function headerMark(env = process.env) {
  return overrides(env)['Wallet'] || SERVER_EMOJI.Wallet || WALLET_MARK;
}

export function chainMark(label, env = process.env) {
  if (!label) return MARK_FALLBACK;
  const key = String(label);
  return overrides(env)[key] || SERVER_EMOJI[key] || CHAIN_MARK[key] || MARK_FALLBACK;
}
