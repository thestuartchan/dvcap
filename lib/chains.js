...
// The wallet's own mark, as an IMAGE rather than an emoji.
//
// This was a custom-emoji slot, which needed the operator to upload a logo and then find its
// snowflake id — and finding it turned out to be the hard part. It is unnecessary: the card is one
// embed, an embed has exactly one AUTHOR slot, and an author icon takes an ordinary image URL.
// The header is exactly one thing, so it fits, and it needs no setup at all.
//
// Custom emoji are still the only way to get a logo inline NEXT TO TEXT, which is what the chain
// headings inside the description need. Two different problems, two different mechanisms.
export const WALLET_ICON = 'https://icons.llamao.fi/icons/protocols/metamask?w=48&h=48';
