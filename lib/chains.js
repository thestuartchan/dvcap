...
// Real chain logos, fetched by Discord from the web when it renders the card — no upload, no
// custom emoji, nothing for the operator to set up. They are the author icon of a per-chain embed,
// which is the ONLY place Discord will draw an arbitrary image next to text: an embed description
// cannot carry inline images, and markdown image syntax does not render inside one. That constraint
// is why the card groups by chain rather than marking every line.
//
// One source on purpose, so the six read as a set rather than six different croppings. If a URL
// ever 404s Discord simply omits the icon and the chain name still shows, so a dead link degrades
// the card rather than breaking it.
const CHAIN_LOGO = Object.freeze({
  ...
  'Base':            'https://icons.llamao.fi/icons/chains/rsz_base?w=48&h=48',
  'Polygon':         'https://icons.llamao.fi/icons/chains/rsz_polygon?w=48&h=48',
  'HyperEVM':        'https://icons.llamao.fi/icons/chains/rsz_hyperliquid?w=48&h=48',
  'Hyperliquid':     'https://icons.llamao.fi/icons/chains/rsz_hyperliquid?w=48&h=48',
  'Robinhood Chain': 'https://icons.llamao.fi/icons/chains/rsz_robinhood?w=48&h=48',
});

// null rather than a placeholder URL — Discord omits the icon for a missing one, but renders a
// broken image for one that 404s, and a broken image is worse than none.
export function chainLogo(label) {
  return (label && CHAIN_LOGO[String(label)]) || null;
}

// The card's own header mark, read from the same map under the reserved key "Wallet". It is not a
// chain, so it does not belong in CHAIN_MARK; it shares the parser because it shares the failure
// modes, and one malformed env var should degrade one way rather than two.
//
// Empty string when unset, not a placeholder: the header reads "Wallet · today" perfectly well
// without one, and a fallback glyph in front of it would be decoration nobody asked for.
export function headerMark(env = process.env) {
  return overrides(env)['Wallet'] || '';
}
