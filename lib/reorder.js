...
// Open positions on one underlying (SOFI shares and the SOFI vertical) are drawn together, at the
// place of the first of them in `rows`. Moving ONE of them with moveOnto split the pair in the
// stored order: the screen still drew them together, wherever the first happened to be, so a drag
// or a ▼ could do nothing visible, or move the pair somewhere the reader did not drop it.
//
// Here the dragged row's whole group moves. Onto a row of the SAME group it is an ordinary move
// within the group. Onto another group it lands past that group: after its last member when moving
// down, before its first when moving up — so the drawn order (group at its first member) is the
// order the reader asked for. `groupOf(row)` returns the key; rows outside any group give their own.
export function moveGroupOnto(list = [], dragId, overId, groupOf = (r) => r?.id) {
  const drag = list.find(r => r && r.id === dragId), over = list.find(r => r && r.id === overId);
  if (!drag || !over || dragId === overId) return list;
  const gd = groupOf(drag), go = groupOf(over);
  if (gd === go) return moveOnto(list, dragId, overId);
  const firstOf = (g, l) => l.findIndex(r => r && groupOf(r) === g);
  const down = firstOf(gd, list) < firstOf(go, list);
  const members = list.filter(r => r && groupOf(r) === gd);
  const rest = list.filter(r => !(r && groupOf(r) === gd));
  let at;
  if (down) { at = -1; rest.forEach((r, i) => { if (r && groupOf(r) === go) at = i; }); at += 1; }
  else at = firstOf(go, rest);
  const next = [...rest.slice(0, at), ...members, ...rest.slice(at)];
  return next.every((r, i) => r === list[i]) ? list : next;
}
