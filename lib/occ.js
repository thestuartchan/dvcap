...
export function mergeOccIv(occ, cboeContracts = []) {
  ...
  // BY SIDE, ON THE MATCHED CONTRACTS. The cross-check compares our call and put open interest
  // with CBOE's, and ours is OCC's on the contracts that joined while theirs was CBOE's across
  // everything they list in the band — two universes, so a 17.9% put-OI gap on 2026-09-23 was
  // mostly contracts one side did not have. Like for like is CBOE's OI on the same contracts.
  const side = { call: { occ: 0, cboe: 0 }, put: { occ: 0, cboe: 0 } };
  ...
  for (const c of cboeContracts) {
    ...
    const sd = side[c.type === 'call' ? 'call' : 'put'];
    sd.occ += settled; sd.cboe += c.oi ?? 0;
  ...
  return {
    ...
    matchedBySide: { callOcc: side.call.occ, callCboe: side.call.cboe, putOcc: side.put.occ, putCboe: side.put.cboe },
  };
}
...
// Measured 2026-09-14 against an outside read of the same chain: the three nearest dailies were
// the Monday, Tuesday and Wednesday books, and the Wednesday one carried −$165M per 1%. The
// Friday-week-after book (25 September) carried about −$830M and was not read at all — five times
// the Wednesday book and larger than the October monthly — because it was neither one of the three
// nearest nor the nearest past any horizon. The 18th was only in the sample because it happened to
// be the nearest past seven days. Weeklies expire on Fridays and that is where the open interest
// sits, so the shape rule now takes the next two Fridays explicitly, before the horizons.
export const DEFAULT_FRIDAY_COUNT = 2;
...
export function refreshExpiries(stored = [], available = [], now = new Date()) {
  ...
  const want = defaultExpiriesOrdered(available, now);
  // Topped up in the RULE's order — nearest first, then the Fridays, then each horizon — so what
  // comes back is the set that rule would have chosen, not whatever happens to sort first. A stored
  // six-expiry list captured under the old shape grows to the new one here, on its next refresh.
...
export function defaultExpiries(available = [], now = new Date()) {
  return [...defaultExpiriesOrdered(available, now)].sort();
}
// The same set, in the order the rule picked it — for topping up a stored list.
export function defaultExpiriesOrdered(available = [], now = new Date()) {
  const today = now.toISOString().slice(0, 10);
  ...
  // The next Fridays not already taken as near dailies — the weekly books, where the gamma is.
  const fridays = live.filter(d => new Date(d + 'T00:00:00Z').getUTCDay() === 5 && !picked.has(d));
  for (const d of fridays.slice(0, DEFAULT_FRIDAY_COUNT)) picked.add(d);
...
// 01:10Z one night, still unrolled at 01:46Z another — and cost a session wake per sample. The
// measurement was thrown away each time: the vintage store keeps only the CURRENT fingerprint, so
// every transition it observed was overwritten by the next one.
//
// It should never have been an agent's job. Every settled recompute already fetches the file and
// already fingerprints it; what was missing was two fields and somewhere to put them. A bracket is
// the LAST time the old fingerprint was seen and the FIRST time the new one was — neither is a
// measurement alone, and `lastSeenAt` was the half nobody was keeping.
//
// So every call contributes an observation, the log survives this container and this conversation,
// and the resolution improves on its own every time the panel is opened.
export const ROLL_LOG_MAX = 80;        // ~three months of weeknight rolls

// Refreshed on EVERY observation, not only on a change. This is the half that makes a bracket
// possible: without it the best lower bound is when the old fingerprint was first seen, which on a
// file that holds still for twenty hours is useless.
export function seenRecord(vintage, parsed, now = new Date()) {
  if (!vintage?.fingerprint) return null;
  return {
    fingerprint: vintage.fingerprint,
    firstSeenAt: vintage.firstSeenAt,
    lastSeenAt: now.toISOString(),
    rows: parsed?.rows ?? null,
    totalOi: parsed?.totalOi ?? null,
  };
}

// One entry per TRANSITION. Returns null when nothing rolled, and null on a first-ever sighting —
// which is not a roll observation but the absence of one, and recording it as a roll would put a
// bracket of unknown width into a series whose whole value is the width.
export function rollEntry(prevSeen, vintage, parsed, { symbol = null } = {}) {
  if (!vintage?.changed || !prevSeen?.fingerprint) return null;
  const from = prevSeen.lastSeenAt || null;
  const to = vintage.firstSeenAt || null;
  // A bracket needs both ends. An old record written before `lastSeenAt` existed has only one, and
  // says so rather than reporting the width to the fingerprint's first sighting — which would be
  // the age of the whole previous settlement, not the width of the roll.
  const bracketMin = (from && to) ? Math.round((Date.parse(to) - Date.parse(from)) / 60000) : null;
  return {
    symbol, at: to, from, to, bracketMin,
    fp: vintage.fingerprint, rows: parsed?.rows ?? null, totalOi: parsed?.totalOi ?? null,
    prevFp: prevSeen.fingerprint, prevRows: prevSeen.rows ?? null, prevTotalOi: prevSeen.totalOi ?? null,
    // Signed, because the direction is the check: a settlement DROPS the expired series and ADDS
    // the session's new open interest, so rows falling while OI rises is the shape of a real roll.
    rowsDelta: (parsed?.rows != null && prevSeen.rows != null) ? parsed.rows - prevSeen.rows : null,
    oiDelta: (parsed?.totalOi != null && prevSeen.totalOi != null) ? parsed.totalOi - prevSeen.totalOi : null,
  };
}

export function appendRoll(log = [], entry, max = ROLL_LOG_MAX) {
  if (!entry) return Array.isArray(log) ? log : [];
  return [...(Array.isArray(log) ? log : []), entry].slice(-max);
}

// ── READING A WEEK OF IT ─────────────────────────────────────────────────────
// The operational question is not "what minute does OCC publish" — it is "is the book rolled by the
// time the US pre-read fires". Those need different evidence and only one of them needs a minute.
//
// `againstUtc` is the hour:minute the answer is wanted for (the pre-read cron is 12:42Z). The
// margin reported is to the LATEST bracket end observed, because the worst night is the one that
// governs, and the count of nights is carried beside it so a two-night answer cannot read as
// settled.
export function rollSummary(log = [], { symbol = null, againstUtc = '12:42' } = {}) {
  const rows = (Array.isArray(log) ? log : [])
    .filter(e => e && (!symbol || e.symbol === symbol) && e.to);
  if (!rows.length) return { n: 0, note: 'no roll observed yet — the log starts at the first transition seen' };
  const minOfDay = (iso) => { const d = new Date(iso); return d.getUTCHours() * 60 + d.getUTCMinutes(); };
  const DAY = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
  const entries = rows.map(e => ({
    ...e,
    // The SESSION the settlement belongs to, not the UTC day it landed on — a roll at 02:30Z on a
    // Saturday is Friday's settlement, and bucketing it as Saturday would invent a weekend session.
    session: DAY[new Date(Date.parse(e.to) - 6 * 3600000).getUTCDay()],
    endMin: minOfDay(e.to), startMin: e.from ? minOfDay(e.from) : null,
  }));
  const [wh, wm] = String(againstUtc).split(':').map(Number);
  const wantMin = wh * 60 + wm;
  // ── ONLY MEASUREMENTS ARE TIMED ───────────────────────────────────────────
  // An entry with no bracket is a transition we noticed, not a roll we watched: the first
  // observation after a deploy catches up with a settlement that happened hours earlier, and its
  // `to` is when WE looked rather than when OCC wrote. Timing the summary off those put a 16:32Z
  // catch-up in as the latest roll end and reported "-3.8h clear of 12:42Z" — a margin that reads
  // as a catastrophic failure and describes nothing. Found on the first real data this saw.
  const timed = entries.filter(e => e.bracketMin != null && e.endMin < 12 * 60);
  const tightest = [...timed].sort((a, b) => a.bracketMin - b.bracketMin)[0] ?? null;
  const latest = timed.reduce((a, e) => (e.endMin > (a?.endMin ?? -1) ? e : a), null);
  const hhmm = (m) => `${String(Math.floor(m / 60)).padStart(2, '0')}:${String(m % 60).padStart(2, '0')}`;
  // A CATCH-UP IS STILL WORTH SEEING, so it stays in `entries` and is counted separately rather
  // than dropped — the log should show that a transition was observed even when it cannot be timed.
  const untimed = entries.length - timed.length;
  return {
    n: entries.length, timed: timed.length, untimed, entries,
    tightestBracketMin: tightest?.bracketMin ?? null,
    // Null rather than a number derived from an untimed entry. There is no honest fallback here:
    // reporting one anyway is what produced the negative margin.
    latestEndUtc: latest ? hhmm(latest.endMin) : null,
    marginMin: latest ? wantMin - latest.endMin : null,
    // A COUNT OF NIGHTS, ALWAYS. Brackets that disagree are not a schedule, and a recommendation
    // that does not carry how thin it is will be read as one.
    note: !timed.length
      ? `${entries.length} transition${entries.length === 1 ? '' : 's'} observed, none of them bracketed — `
        + `each was noticed after the fact, so there is no roll time to report yet`
      : `${timed.length} roll${timed.length === 1 ? '' : 's'} bracketed · latest ended ${hhmm(latest.endMin)}Z`
        + ` · ${((wantMin - latest.endMin) / 60).toFixed(1)}h clear of ${againstUtc}Z`
        + (tightest?.bracketMin != null ? ` · tightest bracket ${tightest.bracketMin}min` : '')
