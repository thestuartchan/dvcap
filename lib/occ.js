...
export function rollSummary(log = [], { symbol = null, againstUtc = '12:42' } = {}) {
  ...
  // Latest END of any bracket, in minutes past 00:00Z, with anything before 12:00Z treated as the
  // small hours of the following day — which is where every roll observed so far has landed.
  const latest = entries.reduce((a, e) => e.endMin > (a?.endMin ?? -1) && e.endMin < 12 * 60 ? e : a, null) ?? entries[0];
  const tightest = entries.filter(e => e.bracketMin != null).sort((a, b) => a.bracketMin - b.bracketMin)[0] ?? null;
  const hhmm = (m) => `${String(Math.floor(m / 60)).padStart(2, '0')}:${String(m % 60).padStart(2, '0')}`;
  return {
    n: entries.length, entries,
    tightestBracketMin: tightest?.bracketMin ?? null,
    latestEndUtc: hhmm(latest.endMin),
    marginMin: wantMin - latest.endMin,
    // A COUNT OF NIGHTS, ALWAYS. Three brackets that disagree are not a schedule, and a
    // recommendation that does not carry how thin it is will be read as one.
    note: `${entries.length} roll${entries.length === 1 ? '' : 's'} observed · latest ended ${hhmm(latest.endMin)}Z`
