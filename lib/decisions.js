...
// 'reclassified' — a swing trade still open on its fourth session, moved into the position book
// without asking (the standing preference: apply first, tell after). Not a fill, so it carries no
// quantity and enters no override statistic; it is the note the log keeps of the move.
export const ACTIONS = Object.freeze(['opened', 'added', 'closed', 'trimmed', 'declined', 'reclassified']);
...
export function decisionEntry({
  ...
  return {
    ...
    // How much of the consensus behind that regime was alive when the size was suggested. Without
    // it a ×1.0 from a four-month-old consensus is indistinguishable from one computed that morning,
    // and the guard retrospective cannot separate a stale regime from a wrong one.
    regimeVintage: str(regime?.vintage?.grade, 12),
    consensusAlive: num(regime?.vintage?.alive),
