...
export function decisionEntry({
  ...
  return {
    ...
    addNumber: fill?.side === 'sell' ? null : buys + 1,
    fillPrice: num(fill?.price),
    priorBuys: buys,
    unrealisedPctBefore: num(pnl?.unrealizedPct),
    addToLoser: fill?.side !== 'sell' && (derived?.qty > 0) && num(pnl?.unrealizedPct) != null && pnl.unrealizedPct < 0,
...
export function overrideStats(log = []) {
  const rows = (Array.isArray(log) ? log : [])
    .filter(d => d?.overrideRatio != null && d.side === 'buy' && d.action !== 'declined');
...
export function overrideTrend(log = [], { bucket = 'month' } = {}) {
  const rows = (Array.isArray(log) ? log : [])
    .filter(d => d?.overrideRatio != null && d.side === 'buy' && d.action !== 'declined' && d.at);
...
export function guardOutcomes(log = [], rows = []) {
  ...
  const entries = (Array.isArray(log) ? log : []).filter(
    d => d?.guards && d.action !== 'declined' && d.side === 'buy' && closed.has(d.id) && num(d.takenQty) != null);
