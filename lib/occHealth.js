...
import { levelsLog } from './gexLevels.js';
...
export function healthSample(res, { symbol, at = new Date().toISOString(), published = false, rung = null } = {}) {
  ...
  const s = {
    ...
    // ── THE LEVELS, PER BOARD ──
    // put_support_strike, trapdoor_near/deep, pin_lo/hi, call_wall_strike/kind, balance_state,
    // rf_rate/rf_status. The monthly review reads these back to answer "how often did price stop
    // at put support versus fall through the trapdoor" — the question the labels exist for.
    ...(res.levels ? levelsLog(res.levels, { rate: res.row?.rate ?? null, rateStatus: res.row?.rateStatus ?? null }) : {}),
  };
...
export function transitionRuns(rollLog = [], { symbol = null, gapMin = SAME_WRITE_MAX_MIN } = {}) {
  const all = (Array.isArray(rollLog) ? rollLog : []).filter(e => e?.to && (!symbol || e.symbol === symbol));
  // ── PER SYMBOL, ALWAYS ────────────────────────────────────────────────────
  // Grouping the whole log at once reported QQQ at 16:32:17 and SPY at 16:32:18 as ONE settlement
  // that "wrote in more than one step", spanning zero minutes — found on the first real data this
  // function ever saw. Two roots publishing a second apart is two roots, not a progressive write,
  // and the two can never be interleaved into one run.
  const bySym = new Map();
  for (const e of all) bySym.set(e.symbol ?? '?', [...(bySym.get(e.symbol ?? '?') || []), e]);
  const runs = [];
  for (const rows of bySym.values()) {
    let last = null;
    for (const e of [...rows].sort((a, b) => String(a.to).localeCompare(String(b.to)))) {
      ...
      else { last = { symbol: e.symbol ?? null, startAt: e.to, endAt: e.to, n: 1, spanMin: 0 }; runs.push(last); }
    }
  }
  runs.sort((a, b) => String(a.startAt).localeCompare(String(b.startAt)));
