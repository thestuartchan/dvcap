...
export function atrSummary(bars = [], period = ATR_PERIOD) {
  ...
  return {
    atr: +atr.toFixed(6),
    // The last close the ATR was taken against. The stock sizer had no price at all — its
    // concentration cap printed "no price" — because only an option's spot arrived, via the
    // greeks. The close that already sits in this series is the price the ATR is a fraction of.
