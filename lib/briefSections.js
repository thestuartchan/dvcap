...
// Two yields and the gap between them. The LEVEL of either is a slow-moving fact; the shape is
// what changes, and the shape is the part with a plain-English reading.
export function ratesLine({ us2y, us10y } = {}) {
  if (us2y == null && us10y == null) return null;
  const parts = [];
  if (us2y != null) parts.push(`**2yr ${us2y}%**`);
  if (us10y != null) parts.push(`**10yr ${us10y}%**`);
  let tail = '';
  if (us2y != null && us10y != null) {
    const gap = +us10y - +us2y;
    tail = gap < -0.05
      ? ' — short money costs more than long, which is the unusual way round'
      : gap < 0.25 ? ' — the two are nearly level' : ' — the normal way round';
  }
