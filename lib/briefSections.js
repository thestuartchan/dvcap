...
// ── 🏛 SUPPLY ────────────────────────────────────────────────────────────────
// The rates lines say where yields ARE. On an auction day the question a duration book has is
// where the demand was, and on 2026-09-10 the brief carried neither the auction nor its result on
// the morning the long end was the whole story.
//
// The tail is not here on purpose — it needs the when-issued yield at the bid deadline, which no
// free feed publishes. Bid-to-cover against the trailing run of the SAME tenor answers the same
// question from numbers Treasury actually prints. See lib/auctions.js.
export function supplyLine(auctions) {
  if (!auctions) return null;
  const done = (auctions.today || []).filter(a => a.bidToCover != null && a.read?.note);
  if (done.length) {
    const a = done[0];
    const bits = [`🏛 **US ${a.label} auction** ${a.highYield != null ? `${a.highYield.toFixed(3)}%` : ''}`.trim(), a.read.note];
    if (a.read.dealerNote) bits.push(a.read.dealerNote);
    return bits.join(' — ');
  }
  // Bids not yet in. The countdown is the useful part before the result exists.
  const pending = (auctions.pendingToday || [])[0];
  if (pending) return `🏛 **US ${pending.label} auction today** — $${(pending.offering / 1e9).toFixed(0)}bn, bids close ${pending.closesAt} ET`;
  const next = auctions.nextLongEnd || auctions.next;
  if (next) return `🏛 **Next US supply** — ${next.label} $${(next.offering / 1e9).toFixed(0)}bn on ${next.date || next.auctionDate}`;
  return null;
}

// ── 🧩 SOFTWARE vs HARDWARE ──────────────────────────────────────────────────
// The brief's breadth line reports who is leading. It does not report whether the AI capex the
// leadership is built on is being MONETIZED, and the two diverge at cycle turns. One line, and
// only when the pair has actually said something: below half its own normal day this pair moves
// together, and a line that fires every morning is a line a reader stops seeing.
//
// NEVER LABELLED OFF ONE SESSION. The pending flag is carried into the brief exactly as it is on
// the board — a brief that announces a regime turn on one volatile day is worse than no line.
export function monetizationLine(m) {
  if (!m?.available || m.candidate === 'MOVING TOGETHER') return null;
  const pp = (v) => v == null ? '—' : `${v >= 0 ? '+' : ''}${v}pp`;
  const bits = [`🧩 **${m.softSym}** ${m.soft >= 0 ? '+' : ''}${m.soft}% · **${m.hardSym}** ${m.hard >= 0 ? '+' : ''}${m.hard}%`
    + ` — ${pp(m.spread)}${m.atrMult != null ? ` (${m.atrMult}×ATR)` : ''}`];
  bits.push(m.confirmed ? `**${m.state}**` : `**${m.candidate}** — ${m.pending}`);
  if (m.purity?.state === 'CONTAMINATED') bits.push(m.purity.note);
  if (m.cum20 != null) bits.push(`20d ${pp(m.cum20)}${m.cum60 != null ? ` · 60d ${pp(m.cum60)}` : ''}`);
  if (m.ratesRuledOut === true) bits.push('rising 30-year does not explain it — software is the longer-duration leg');
  return bits.join(' · ');
}
...
export function creditLine({ oas, date, state, stale = false, hygPct = null } = {}) {
  ...
  // The ETF's price change, beside the spread's level. Named, because they are different
  // instruments measuring the same worry at different speeds — the spread is where lenders are
  // marked at the close, the ETF is where the tape is right now.
  const live = hygPct != null ? ` · the junk-bond ETF (HYG) is **${pctWord(hygPct)}** today` : '';
  // THE GAUGE IS NAMED. It is the ICE BofA high-yield option-adjusted spread, and a reader who
  // wants to look it up, argue with it, or find it on another screen cannot do any of that from a
  // sentence that only describes it.
  const when = stale
    ? ` — **no new print** since ${date || 'the last one'}`
    : date ? ` _(as of ${date})_` : '';
  return `🩹 **Credit (HY OAS):** risky companies pay **${oas}pp** over government debt${when} — ${words}${live}`;
}
...
// A MISSING LEG IS A FACT, NOT A GAP. Observed live on 2026-09-10: a transient FRED miss dropped
// the 2yr and the line rendered "Money: 10yr 4.8% · 30yr 5.25%" with no shape comment and nothing
// to say a leg was absent. A reader cannot tell that from a line that simply never mentions the
// 2yr, which is the same ambiguity as a flat print and a missing print rendering identically —
// and the shape claim silently disappears with it, so the line quietly says less without saying
// that it is saying less.
export function ratesLine({ us2y, us10y, us30y, jgb = null } = {}) {
  if (us2y == null && us10y == null && us30y == null) return null;
  const parts = [], absent = [];
  for (const [label, v] of [['2yr', us2y], ['10yr', us10y], ['30yr', us30y]]) {
    if (v != null) parts.push(`**${label} ${v}%**`); else absent.push(label);
  }
  ...
  // Named, and only when the shape had to be withheld because of it — a leg missing from a line
  // that was making no claim anyway is bookkeeping, and this file does not narrate its own gaps.
  const gap = (!tail && absent.length) ? ` — no print for the ${listWords(absent)}, so the shape is not stated` : '';
  // ── JAPAN, ON THE SAME LINE ────────────────────────────────────────────────
  // Merged rather than given a line of its own. The Asia brief already runs to a dozen backdrop
  // rows and this is the same fact about a different issuer — and reading the two curves against
  // each other is the point, which two separate lines make harder rather than easier.
  //
  // It carries its OWN date. The MoF curve publishes a business day behind, so on a Monday it is
  // Friday's and saying so is the difference between a lag and an error.
  const jp = jgbTail(jgb);
  return `💵 **Money:** ${parts.join(' · ')}${tail}${gap}${jp}`;
}

// The Japan leg, or nothing. Never a partial curve dressed as a full one.
export function jgbTail(jgb) {
  if (!jgb) return '';
  const bits = [];
  for (const [label, key] of [['10yr', '10yr'], ['30yr', '30yr']]) {
    const f = jgb[key];
    if (f?.value != null) bits.push(`**${label} ${f.value}%**${f.deltaBps ? ` (${f.deltaBps >= 0 ? '+' : ''}${f.deltaBps}bp)` : ''}`);
  }
  if (!bits.length) return '';
  const d = jgb['10yr']?.date || jgb.date || null;
  return ` · 🇯🇵 **JGB** ${bits.join(' · ')}${d ? ` _(as of ${d})_` : ''}`;
}

// ── 🔀 THE HANDOFF ───────────────────────────────────────────────────────────
// The one line in the Asia brief that is about the session ABOUT TO OPEN rather than the one that
// just closed. lib/handoff.js has computed it on every Asia assemble since it was written and it
// has never reached a brief: at 22:48Z on 2026-09-10 it read GAP-DOWN RISK on a 1.82pp gap with
// the open three hours away, and the message said nothing about it.
//
// FORWARD ONLY. The backward half ("Asia moved with 2 of 3 overnight drivers") is a validation of
// a session that is over; the reader at 06:48 local is deciding about the one that has not
// started. The backward read stays on the dashboard, where a reader can go looking for it.
//
// OBSERVATIONAL. It says where convergence points, never what to do about it.
export function handoffLine(handoff) {
  const f = handoff?.forward;
  if (!f?.available || f.gap == null) return null;
  // Below a threshold there is no gap worth naming, and a line that fires every morning is one a
  // reader stops seeing. Half a per cent is roughly a quiet overnight for this pair of tapes.
  if (Math.abs(f.gap) < HANDOFF_MIN_PP) return null;
  const drivers = (f.drivers || []).filter(d => d?.name && d.detail)
    .map(d => `${d.name} ${d.detail}`).join(' · ');
  const when = f.nextOpenH != null ? `, ${f.nextOpenH}h to the open` : '';
  return `🔀 **Handoff:** **${f.verdict}** — Asia last closed ${pctWord(f.asiaRef) ?? '—'} against a US finish of ${pctWord(f.usRef) ?? '—'},`
    + ` a **${Math.abs(f.gap).toFixed(2)}pp** gap${when}${drivers ? ` · ${drivers}` : ''}`;
}
export const HANDOFF_MIN_PP = 0.5;

// ── 🇨🇳 CHINA ────────────────────────────────────────────────────────────────
// Korea gets one line carrying its currency, its fear gauge and its state. China — which is HSTECH,
// HSI, SMIC, BYD, Alibaba, Tencent and Biren, about half the names in this brief — had no line at
// all: no currency gauge, and an A/H premium that is a whole scenario leg and was never printed.
//
// THE PREMIUM IS THE MAINLAND'S CONVICTION, priced. A-shares and H-shares are claims on the same
// company; the gap between them is what onshore money will pay that offshore money will not. It
// moves independently of the macro tape, which is exactly why it is worth a line of its own rather
// than being read off HSTECH.
export function chinaLine({ cnh = null, cnhPct = null, ah = null } = {}) {
  const bits = [];
  if (cnh != null) {
    // A RISING USD/CNH IS A WEAKER YUAN. Quoted dollars-per-yuan the sign would invert, and this
    // pair is quoted the other way up — the same trap lib/fx.js documents for the won.
    const dir = cnhPct == null ? '' : Math.abs(cnhPct) < 0.05 ? ' — flat'
      : cnhPct > 0 ? ' — yuan weaker' : ' — yuan firmer';
    bits.push(`USD/CNH **${cnh}**${dir}`);
  }
  if (ah?.premium != null) {
    const d5 = ah.d5 == null ? '' : ` (${ah.d5 >= 0 ? '+' : ''}${ah.d5}pp 5d)`;
    const read = ah.d5 == null ? ''
      : ah.d5 > 0 ? ' — widening, so the H-share is the funding leg rather than the beneficiary'
      : ' — compressing, which is mainland conviction draining out of the name';
    bits.push(`SMIC A/H premium **${ah.premium}pp**${d5}${read}`);
  }
  if (!bits.length) return null;
  return `🇨🇳 **China:** ${bits.join(' · ')}`;
}
...
export function changeSection({ items = [], flipsIf = null, lead = null } = {}) {
  ...
  // ── THE THIRD STATE: NAMED, NOT SCORED ─────────────────────────────────────
  // A gauge the board withheld used to drop out of this list entirely, because the filter above
  // only knows fired from pending. On 2026-09-10 that would have taken "The 30-year yield clears
  // 5.35%" out of the brief on the morning the live yield was six tenths of a basis point under
  // it — trading a wrong mark for no mention at all, which is not an improvement. The level is
  // the useful part and it stays; what goes is the verdict, and the line says so.
  const withheld = items.filter(i => i && i.name && i.unscored);
  for (const i of pending) out.push(`👁 **${i.name}**${i.watch ? ` → ${i.watch}` : ''}`);
  for (const i of withheld) out.push(`⌀ **${i.name}** — not scored${i.detail ? `: ${i.detail}` : ''}`);
...
export function plainTripwire(item) {
  ...
  // `unscored` and its reason ride through the gloss. Without them the withheld state cannot be
  // told from an ordinary missing input by the time it reaches changeSection.
  const carry = { tripped: item.tripped, unscored: item.unscored === true, detail: item.unscored ? item.detail : null };
  const hit = TRIPWIRE_PLAIN.find(t => t.match.test(item.name));
  if (!hit) return { name: item.name, ...carry, watch: null, means: null };
  return {
...
export function sinceSection(now = {}, prev = null) {
  ...
  for (const [name, walls] of Object.entries(now.walls || {})) {
    ...
    // Same-named keys only. A snapshot from before the put side was split carries `putWall`;
    // one from after carries `support` and `trapdoorNear`. Comparing yesterday's put wall (700,
    // the trapdoor) to today's support would print a move that never happened.
    const SIDES = [['support', 'put support'], ['trapdoorNear', 'trapdoor'], ['callWall', 'call wall'], ['putWall', 'put wall']];
...
// ── 📐 TREND FUNDS (CTA MODEL) ───────────────────────────────────────────────
// WHAT IT ANSWERS: where mechanical trend funds probably sit, and the nearest price that starts
// them moving. From the replica in lib/cta.js — a model, not their orders — through
// lib/ctaBook.js, the same cached read the Daily tab shows.
//
// Short by design: where they sit, which of those are crowded (the most fuel if a trend turns),
// and the two nearest triggers stated as what they do. Levels apply to the close, and the section
// says so, because a trend fund acts on closes and an intraday poke through a level is not one.
export const CTA_NAMES = Object.freeze({ ES: 'S&P', NQ: 'Nasdaq', ZN: '10Y note', DX: 'dollar', CL: 'crude', GC: 'gold' });
export const CTA_TRIGGER_SIGMAS = 3;       // a trigger further than three normal days away is not "near"
export const CTA_MOVE_ARROW = 0.2;         // a 20-point swing in a week is worth an arrow

const ctaPx = (p) => (p == null ? '—' : Number(p).toLocaleString('en-US', { maximumFractionDigits: p >= 100 ? 2 : 3 }));
const ctaPct = (x) => `${x > 0 ? '+' : x < 0 ? '−' : ''}${Math.abs(x).toFixed(1)}%`;

export function ctaSection(book, { now = new Date() } = {}) {
  const ms = (book?.markets || []).filter(m => m?.ok && m.position != null);
  if (!ms.length) return null;
  const name = (m) => CTA_NAMES[m.key] || m.key;
  const where = ms.map(m => {
    const size = Math.round(Math.abs(m.position) * 100);
    const arrow = m.change != null && Math.abs(m.change) >= CTA_MOVE_ARROW ? (m.change > 0 ? ' ▲' : ' ▼') : '';
    return `${name(m)} **${m.stance}**${m.stance === 'neutral' ? '' : ` ${size}%`}${arrow}`;
  }).join(' · ');
  const out = [`**Where they sit:** ${where}`];

  const crowded = ms.filter(m => Math.abs(m.position) >= 0.8).map(name);
  if (crowded.length) out.push(`**Crowded:** ${crowded.join(', ')} — the most fuel if the trend turns`);

  const what = (m) => {
    const c = m.cut;
    if (c.net) return `tips net ${c.tips} ${c.side} it`;
    return m.position > 0 ? 'model selling starts below it' : 'short covering starts above it';
  };
  const near = ms.filter(m => m.cut && m.cut.flipSigmas != null && Math.abs(m.cut.flipSigmas) <= CTA_TRIGGER_SIGMAS)
    .sort((a, b) => Math.abs(a.cut.flipSigmas) - Math.abs(b.cut.flipSigmas)).slice(0, 2);
  out.push(near.length
    ? `**Nearest triggers:** ${near.map(m => `${name(m)} **${ctaPx(m.cut.flip)}** (${ctaPct(m.cut.flipPct)}) — ${what(m)}`).join(' · ')}`
    : `**Nearest triggers:** none within ${CTA_TRIGGER_SIGMAS} normal days of price`);

  const ageMin = book?.at ? (now.getTime() - Date.parse(book.at)) / 60000 : null;
  const stale = ageMin != null && ageMin > 120 ? ` ⚠️ model from ${String(book.at).slice(11, 16)}Z` : '';
  out.push(`_A model of trend funds, not their orders; levels apply to the close.${stale}_`);
  return out.map(l => `• ${l}`).join('\n');
}
