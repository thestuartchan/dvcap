...
// CRYPTO ADDS A SECOND KIND OF COLLISION. BTC and ETH are not equity tickers, so they passed as
// "unambiguous" — but they are the ordinary names for the SPOT assets, and someone typing BTC into
// a console almost never means the 5-bitcoin CME contract. The symbol does not decide, which is
// the only thing this set has ever been about, so the name now says that rather than "equity".
export const ROOT_AMBIGUOUS = Object.freeze(new Set([
  // also real equity tickers
  'MET', 'PL', 'CL', 'ES', 'NG', 'SI', 'HG', 'GC', 'RB', 'HO', 'SIL', 'MBT',
  // also the names of the spot assets the contract is written on
  'BTC', 'ETH',
]));
// Kept as the previous name — same set, and callers/tests still read it.
export const EQUITY_AMBIGUOUS = ROOT_AMBIGUOUS;

export const isUnambiguousFuture = (symbol) => {
  const root = futuresRoot(symbol);
  return Object.prototype.hasOwnProperty.call(FUTURES_MULTIPLIER, root) && !ROOT_AMBIGUOUS.has(root);
};
