...
export function backfillMultipliers(rows = []) {
  ...
  for (const r of (Array.isArray(rows) ? rows : [])) {
    ...
    const known = FUTURES_MULTIPLIER[root];
    if (r?.margined && !has && known != null) {
      fixed.push({ id: r.id, symbol: r.symbol, root, from: Number.isFinite(m) ? m : null, to: known });
      out.push({ ...r, multiplier: known });
      continue;
    }
    // Not margined, but named like a contract and priced like a share. Could be either.
    if (!r?.margined && !has && known != null) review.push({ id: r.id, symbol: r.symbol, root, wouldBe: known });
    out.push(r);
  }
