...
export function gaugesLeaning({ credit, korea, vix, nq, kofiaLatest, units7709Series } = {}) {
  const items = [];
  ...
  if (credit?.dir == null) na('OAS widening', 'no prior print');
    ...
    add('OAS widening', rising5d && aboveFloor, `${credit.level}${credit.word ? ' · ' + credit.word : ''}${d5note}`);
  }
  ...
  if (vix?.dir == null) na('VIX rising', 'no prior close');
           ...
           `${vix.price}${vix.changePct != null ? ` (${vix.changePct >= 0 ? '+' : ''}${vix.changePct}% 1D)` : ''}`);
  ...
  else if (kofiaStale(f.asOf) || kofiaStale(r.asOf)) na('Retail not absorbing', `stale flows (foreign ${f.asOf}, retail ${r.asOf})`);   // P4 — stale manual print can't fire a live tripwire
  ...
  // 6) CSOP 7709 units outstanding falling — the deleveraging tell (P2). Units, NOT price or AUM:
  //    a leveraged China-tech ETF SHEDDING shares means creations are reversing and holders are
  //    exiting the trade, which leads the name. Fires on a >3% one-day unit drop OR two consecutive
  //    down days. Gated on freshness — a stale unit count is a stale signal, so a stale print goes
  //    UNAVAILABLE rather than firing yesterday's move as if it were today's.
  const u = kofiaLatest?.units7709;
  if (u?.value == null || u.delta == null) na('7709 units falling', 'no unit print stored');
  else if (u.asOf && kofiaStale(u.asOf)) na('7709 units falling', `stale print (${u.asOf})`);
  else {
    const prev = u.value - u.delta;                                   // reconstruct the prior level
    const dayPct = prev > 0 ? (u.delta / prev) * 100 : null;          // 1d % change in units
    const bigDrop = dayPct != null && dayPct <= -3;                   // >3% shed in a day
    const twoDown = consecutiveRuns(units7709Series, 'falling') >= 2; // or two consecutive down days
    add('7709 units falling', bigDrop || twoDown,
        `${(u.value / 1e6).toFixed(1)}M${dayPct != null ? ` (${dayPct >= 0 ? '+' : ''}${dayPct.toFixed(1)}% 1d)` : ''}${twoDown && !bigDrop ? ' · 2d down' : ''}`);
  }
