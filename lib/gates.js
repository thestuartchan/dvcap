...
import { SCENARIO_CFG } from './scenarios.js';
...
export function gaugesLeaning({ credit, korea, vix, nq, kofiaLatest, us30y } = {}) {
  const items = [];
  ...
  if (us30y?.value == null) na('30Y > ' + SCENARIO_CFG.hawkish30y + '%', 'no print', 'Hawkish / Disorderly');
  else add('30Y > ' + SCENARIO_CFG.hawkish30y + '%', us30y.value > SCENARIO_CFG.hawkish30y,
           `${us30y.value}%${us30y.deltaBps != null ? ` (${us30y.deltaBps >= 0 ? '+' : ''}${us30y.deltaBps}bp 1d)` : ''}`,
           'Hawkish / Disorderly');
