...
import { SCENARIO_CFG, staleReach } from './scenarios.js';
...
export function gaugesLeaning({ credit, korea, vix, nq, kofiaLatest, us30y, now = null } = {}) {
  const nowISO = String(now || new Date().toISOString()).slice(0, 10);
  // The KOFIA staleness rule counts business days to a DATE, so it takes the same pinned instant
  // rather than the wall clock — see the golden-brief note in test/prereadRender.test.mjs.
  ...
  // `unscored` separates "the input never arrived" from "the input arrived and the board declined
  // to mark it". Both are excluded from numerator and denominator; only the second still has a
  // number worth naming, and the brief renders it rather than dropping the line.
  const na  = (name, why, scenario, extra = null) => items.push({ name, tripped: null, detail: why, scenario, ...(extra || {}) });
  ...
  // 3b) THE LONG END REPRICING. The 30Y was fetched, carried a 10s30s delta, and was already a
  //     condition in the scenario engine — where SCENARIO_CFG.hawkish30y has sat at 5.35 all along
  //     — and none of that reached the brief. On 2026-09-10 the yield was 5.25: ten basis points
  //     from a threshold this codebase already calls "hawkish repricing underway", with nothing
  //     anywhere telling a reader so.
  //
  //     The threshold is IMPORTED, not restated. Two copies of 5.35 is two numbers that can drift,
  //     and the failure mode is the gauge and the scenario disagreeing about the same level.
  //
  //     AND IT TAKES THE SAME GUARD THE SCENARIO BOARD TAKES. On 2026-09-10 this chip evaluated
  //     `5.25% > 5.35%` → false and rendered as a quiet, untripped gauge while the live 30Y was
  //     5.344%. A tripwire scored on a print old enough to have crossed its own line is not a
  //     tripwire; it is a gauge that has been unplugged without saying so. Withheld means
  //     UNAVAILABLE — out of the numerator AND out of the denominator, never counted as calm.
  const y30Label = '30Y > ' + SCENARIO_CFG.hawkish30y + '%';
  const y30Stale = us30y?.value == null ? null : staleReach(us30y, SCENARIO_CFG.hawkish30y, nowISO);
  const y30Vintage = us30y?.live ? ` · live ${us30y.liveAsOf || ''}`.trimEnd() : us30y?.date ? ` · obs ${us30y.date}` : '';
  if (us30y?.value == null) na(y30Label, 'no print', 'Hawkish / Disorderly');
  else if (y30Stale) na(y30Label,
    `${us30y.value}%${y30Vintage}, ${y30Stale.days} business day${y30Stale.days === 1 ? '' : 's'} old and inside the range it could have moved since`,
    'Hawkish / Disorderly', { unscored: true, staleDays: y30Stale.days, value: us30y.value });
  else add(y30Label, us30y.value > SCENARIO_CFG.hawkish30y,
           `${us30y.value}%${us30y.deltaBps != null ? ` (${us30y.deltaBps >= 0 ? '+' : ''}${us30y.deltaBps}bp 1d)` : ''}${y30Vintage}`,
           'Hawkish / Disorderly');
  ...
  else if (kofiaStale(f.asOf, nowD) || kofiaStale(r.asOf, nowD)) na('Retail not absorbing', `stale flows (foreign ${f.asOf}, retail ${r.asOf})`, 'Korea mechanical');   // P4 — stale manual print can't fire a live tripwire
...
export function csop7709Tripwire({ units7709, series, now = new Date() } = {}) {
  const u = units7709;
  if (!u || u.value == null || u.delta == null) {
    return { available: false, tripped: null, note: 'no unit print stored' };
  }
  if (u.asOf && kofiaStale(u.asOf, now)) {
