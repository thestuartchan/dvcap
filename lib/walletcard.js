...
import { chainMark, WALLET_ICON } from './chains.js';
...
export function buildWalletCard(events = [], holdings = [], {
  ...
  return {
    embeds: [{
      // WHOSE WALLET, as a real image. An author icon takes an ordinary URL, so this needs nothing
      // uploaded and no emoji id hunted down — which is what the custom-emoji header cost and never
      // delivered. Sits above the title, which is the standard embed hierarchy: identity small,
      // what-this-is large.
      author: { name: header, icon_url: WALLET_ICON },
