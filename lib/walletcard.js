...
// ── A CHAIN THAT DID NOT ANSWER WAS NOT EMPTIED ──────────────────────────────
// 2026-09-25, 20:22Z: the Robinhood Chain read came back with nothing and the run listed 6 holdings
// instead of 28. Only chains that answered were flattened into today's rows, so the diff saw every
// Robinhood Chain token gone — seven "Sold" lines — and at 23:27Z, when the chain answered again,
// seven "Bought" lines. The card posted all fourteen for a night with no trades in it.
//
// A chain that did not answer says nothing about what it holds, so its rows are yesterday's,
// carried forward unchanged: no sale, and nothing to buy back when it returns. A chain that DID
// answer but came back empty where it held something before is more likely a bad read than an
// exit, so it is carried once and believed on the second empty read in a row — a real exit is
// reported one run late rather than a glitch being reported as an exit. `chains` is
// [{ chain, ok, rows }] (rows = how many it returned); `suspect` is the previous run's record of
// chains carried for reading empty. Returns the rows to diff and store, and the new record.
export function carryUnanswered(now = [], prevRows = [], chains = [], { suspect = {} } = {}) {
  const rows = now.slice();
  const carried = [], nextSuspect = {};
  const prevOn = (label) => (Array.isArray(prevRows) ? prevRows : []).filter(r => r && (r.chain || '') === label);
  for (const c of chains) {
    const label = c?.chain || '';
    const before = prevOn(label);
    if (!before.length) continue;                       // nothing held there to lose
    const answered = !!c.ok, empty = answered && !(c.rows > 0);
    if (answered && !empty) continue;                   // a real read: trust it
    if (empty && suspect?.[label]) continue;            // empty twice running: believe it
    if (empty) nextSuspect[label] = true;
    rows.push(...before.filter(r => !rows.some(x => (x.chain || '') === label && x.coin === r.coin && (x.address || null) === (r.address || null))));
    carried.push(label);
  }
  return { rows, carried, suspect: nextSuspect };
}
