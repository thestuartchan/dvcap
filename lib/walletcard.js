...
export function buildWalletCard(events = [], holdings = [], { title = 'Wallet · today', perps = [] } = {}) {
  ...
  // THE HEADER LIVES IN THE DESCRIPTION, NOT THE TITLE. Discord renders a custom emoji inside an
  // embed description and NOT inside its title — a title carrying <:name:id> prints that text
  // literally. Since the header is the one place a wallet mark belongs, the header moves.
  //
  // Bold first line reads as a title anyway, and this way the whole card is one rendering context:
  // if an emoji works anywhere on it, it works here.
  const hm = headerMark();
  const parts = [
    `${hm ? hm + ' ' : ''}**${title}**`,
  ...
  return {
    embeds: [{
      // No `title` field: it cannot render the emoji, so the header is the description's first line.
      description: desc,
      color: accent,
