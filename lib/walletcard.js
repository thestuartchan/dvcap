...
//
// Folded chains are NAMED rather than dropped, in one muted line. A holdings overview that quietly
// omits a holding is worse than a slightly longer one — the whole point is that the reader can
// trust the list is the list. The line says which chains, never what or how much, so it discloses
// strictly less than the section it replaces.
export const MIN_CHAIN_HOLDINGS = 2;

export function buildWalletCard(events = [], holdings = [], { title = 'Wallet · today' } = {}) {
  ...
  // Folded BEFORE the embed cap is measured, so thin chains cannot push a real one out.
  const shown = new Map();
  const folded = [];
  for (const [chain, rows] of groups) {
    if (rows.length >= MIN_CHAIN_HOLDINGS) shown.set(chain, rows);
    else if (chain) folded.push(chain);
  }

  const parts = [evs.length ? evs.map(eventLine).join('\n') : '_No changes today._'];
  if (folded.length) {
    const one = folded.length === 1;
    parts.push(`_${folded.join(', ')} — ${one ? 'a single holding' : 'a single holding each'}, not shown._`);
  }
  const lead = { title, description: parts.join('\n\n'), color: accent };
  const embeds = [lead];

  if (shown.size && shown.size <= MAX_EMBEDS - 1) {
