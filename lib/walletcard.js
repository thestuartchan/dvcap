...
import { chainMark } from './chains.js';
...
// Led by the chain's mark, so the eye can group by chain without reading to the end of the line.
// The chain is still named there — the mark is a cue, not a key the reader has to have learnt.
...
// The payload. Built from the allow-listed views only, so nothing rides along from upstream.
export function buildWalletCard(events = [], holdings = [], { title = 'Wallet · today' } = {}) {
  ...
  return {
    embeds: [{
      ...
      // share-of-book arithmetic waiting to happen once somebody knows one of them. The footer no
      // longer recites what is withheld: the rules are enforced by the allow-lists and their tests,
      // not by a disclaimer, and the card reads better without one.
