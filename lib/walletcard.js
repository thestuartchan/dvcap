...
export function buildWalletCard(events = [], holdings = [], { title = 'Daily Summary', header = 'Wallet', perps = [] } = {}) {
  ...
  // TWO HEADERS, DOING DIFFERENT JOBS. The embed TITLE says what the message is — "Daily Summary" —
  // and is plain text, which is all a title can be: Discord renders a custom emoji inside an embed
  // description and NOT inside its title, where <:name:id> prints literally.
  //
  // The first description line says whose wallet, and that is where the mark goes, because that is
  // where Discord will actually draw it.
  const hm = headerMark();
  const parts = [
    `${hm ? hm + ' ' : ''}**${header}**`,
  ...
  return {
    embeds: [{
      // Plain text only — a title cannot render a custom emoji, so the mark lives on the first
      // description line instead and this says what the message IS.
      title,
      description: desc,
      color: accent,
