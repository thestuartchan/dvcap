...
export function chainPrices(chain, venue = new Map()) {
  const out = new Map();
  const add = (sym, hl) => { const q = hl && venue.get(hl); if (q) out.set(sym, q); };
  if (chain?.native) add(chain.native.symbol, chain.native.hl);
  for (const tk of chain?.tokens || []) add(tk.symbol, tk.hl);
  return out;
}
...
export async function fetchWallet({ chain = CHAINS.hyperevm, address = process.env[HL_ADDRESS_ENV],
                                   spotMeta, prices, timeoutMs = 10000 } = {}) {
  ...
  const targets = chain.tokensFrom === 'hyperliquid'
  ...
  try {
    ...
    return { ok: true, error: null, ...shell, ...spotHoldings(balances, prices || new Map()) };
  } catch (e) {
...
export async function fetchWallets({ address = process.env[HL_ADDRESS_ENV], spotMeta, spotPrices,
  ...
  const results = await Promise.all(keys.map(k => {
    ...
    return fetchWallet({ chain, address, spotMeta, prices, timeoutMs }).then(r => ({ key: k, ...r }));
  }));
