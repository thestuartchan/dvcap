  ...
  try {
    ...
    if (network && useDiscovery && found.ok) {
      ...
      if (how.ok) {
        ...
        // had. A row is only ever `acquired: false` when the chain was asked and said so.
        for (const b of balances) {
          const a = byAddr.get(b.coin);
          if (!a) continue;
          if (how.acquisition.get(a) === ACQUIRED) b.acquired = true;
          else b.acquired = false;
        }
      }
    }
    ...
    const out = { ok: true, error: null, ...shell, discovery, ...spotHoldings(balances, merged) };
