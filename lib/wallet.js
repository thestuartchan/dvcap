...
export async function fetchWallet({ chain = CHAINS.hyperevm, chainKey = 'hyperevm', address = process.env[HL_ADDRESS_ENV],
  ...
  try {
    ...
    if (network && useDiscovery && found.ok) {
      const how = await fetchAcquisition({ network, address, timeoutMs });
      acquisition = { asked: true, ok: !!how.ok, error: how.ok ? null : (how.error || 'unreadable'), truncated: !!how.truncated };
      if (how.ok) {
        const byAddr = new Map(targets.map(t => [t.name, String(t.address).toLowerCase()]));
    ...
    const held = new Set(balances.map(b => b.coin));
    const unpriced = targets.filter(t => held.has(t.name) && !merged.has(t.name));
    if (slug && unpriced.length) {
      // A SYMBOL THAT IS NOT UNIQUE CANNOT BE PRICED BY SYMBOL. Discovery turns up tokens whose
      // ticker is chosen to collide — one on this very chain crams several hundred tickers into
      // its symbol — and the holdings map is keyed by symbol, so a duplicate would price one token
      // at another's mark. Ambiguous names are left unvalued rather than guessed between.
      const seen = new Map();
      for (const t of unpriced) seen.set(t.name, (seen.get(t.name) || 0) + 1);
      const uniq = unpriced.filter(t => seen.get(t.name) === 1);
      ...
      if (dex.ok && dex.prices.size) {
        ...
        for (const t of uniq) {
          ...
          merged.set(t.name, { price: q.price, volume: q.volume, changePercent: null,
    ...
    if (SAFETY_CHAIN_IDS[chainKey] && suspect.length) {
      const byName = new Map(targets.map(t => [t.name, t.address]));
      const safety = await fetchTokenSafety({
        chainKey, timeoutMs, addresses: suspect.map(r => byName.get(r.coin)).filter(Boolean),
      });
      if (safety.ok) {
        for (const r of out.rows) {
          const f = safety.flags.get(String(byName.get(r.coin) || '').toLowerCase());
