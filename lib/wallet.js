  ...
  try {
    ...
    if (slug && unpriced.length) {
      ...
      if (dex.ok && dex.prices.size) {
        ...
        for (const t of uniq) {
          const q = dex.prices.get(String(t.address).toLowerCase());
          if (q?.price != null) merged.set(t.name, { price: q.price, volume: q.volume, changePercent: null, viaPool: q.quote || true });
        }
      }
    }
