...
import { discoverTokens, alchemyConfigured, NETWORKS, fetchAcquisition, ACQUIRED } from './alchemy.js';
import { fetchDexPrices, allowQuotes, DEX_CHAIN } from './dexscreener.js';
import { fetchTokenSafety, SAFETY_CHAIN_IDS } from './tokensafety.js';
...
export function walletBalances(nativeWei, results = [], targets = [], nativeToken = CHAINS.hyperevm.native) {
  ...
  results.forEach((r, i) => {
    ...
    // The contract address rides along: it is the one thing that tells two tokens of one name
    // apart, and the card's provenance memory (lib/walletcard.js inheritProvenance) keys on it.
    out.push({ coin: t.name, total: amt, hold: 0, free: amt, entryNtl: 0, native: false, address: t.address ? String(t.address).toLowerCase() : null });
  });
  return out;
}

// A wallet's swap history can name a great many contracts; one multicall still reads them, but
// not without bound.
export const MAX_EXTRAS = 300;
...
export async function fetchWallet({ chain = CHAINS.hyperevm, chainKey = 'hyperevm', address = process.env[HL_ADDRESS_ENV],
                                   spotMeta, prices, timeoutMs = 10000, useDiscovery = true, pinned = [] } = {}) {
  ...
  try {
    ...
    // The transfer history is read ALONGSIDE discovery rather than after it, because it now does
    // two jobs: it says which holdings were chosen (below), and it names contracts discovery may
    // not have reached.
    const [found, how] = await Promise.all([
      (useDiscovery && network)
        ? discoverTokens({ network, address, timeoutMs })
        : Promise.resolve({ ok: false, configured: alchemyConfigured(), error: null, tokens: [], truncated: false }),
      (useDiscovery && network) ? fetchAcquisition({ network, address, timeoutMs }) : Promise.resolve(null),
    ]);

    // ── WHAT WAS CHOSEN IS ALWAYS READ ───────────────────────────────────────────────────────
    // 2026-09-25: the real PONS was missing from the card for a third day. Discovery stops at
    // MAX_PAGES of the indexer's list, and a wallet on a chain that airdrops spam by the hundred
    // can hold more contracts than that — so which tokens make the cut is the indexer's ordering,
    // not the wallet's. A holding swapped for (the transfer history says so) or ever confirmed as
    // swapped for (the caller's provenance memory, `pinned`) is read by address on top of
    // whatever discovery returned — by its own balanceOf, which is the chain's answer rather than
    // an indexer's. Always, not only past a truncated list: an indexer can also simply miss a
    // contract, and a chosen contract that reads zero is one since sold, which costs nothing to learn.
    const lc = (a) => String(a || '').toLowerCase();
    let extras = [];
    if (found.ok) {
      const have = new Set(found.tokens.map(t => lc(t.address)));
      const chosen = how?.ok ? [...how.acquisition].filter(([, v]) => v === ACQUIRED).map(([a]) => lc(a)) : [];
      extras = [...new Set([...chosen, ...(pinned || []).map(lc)])]
        .filter(a => /^0x[0-9a-f]{40}$/.test(a) && !have.has(a)).slice(0, MAX_EXTRAS);
    }

    let targets, raw = null;
    if (found.ok && (found.tokens.length || extras.length)) {
      const addrs = [...found.tokens.map(t => t.address), ...extras];
      ...
      targets = named;
      // An extra has no raw balance from the indexer; null asks the balanceOf pass below for it.
      raw = named.map(t => rawBy.get(t.address.toLowerCase()) ?? null);
    } else {
    ...
    const needIdx = targets.map((t, i) => (raw == null || raw[i] == null) ? i : -1).filter(i => i >= 0);
    const [nat, mc] = await Promise.all([
      rpc({ jsonrpc: '2.0', id: 1, method: 'eth_getBalance', params: [address, 'latest'] }),
      needIdx.length
        ? call(encodeAggregate3(needIdx.map(i => ({ target: targets[i].address, allowFailure: true, callData: balanceOfCall(address) }))))
        : null,
    ]);
    ...
    const polled = mj?.result ? decodeAggregate3(mj.result) : [];
    const results = targets.map(() => ({ success: false, data: '0x' }));
    if (raw != null) raw.forEach((v, i) => { if (v != null) results[i] = { success: true, data: v }; });
    needIdx.forEach((i, j) => { if (polled[j]) results[i] = polled[j]; });
    const balances = walletBalances(nj?.result, results, targets, chain.native);

    // ── WHETHER IT WAS CHOSEN ────────────────────────────────────────────────────────────────
    // Holding a token out of the total because nothing vouches for the CONTRACT punished tokens
    // that were deliberately swapped for. The question that actually separates "I chose this" from
    // "this was dropped on me" is whether the wallet gave anything up to get it — which the chain
    // answers, and which does not care whether the token is any good. A scam someone bought is
    // still a position they took; a fine token dropped unbidden is still not a decision.
    let acquisition = { asked: false, ok: false, error: null, truncated: false };
    if (network && useDiscovery && found.ok && how) {
      acquisition = { asked: true, ok: !!how.ok, error: how.ok ? null : (how.error || 'unreadable'), truncated: !!how.truncated };
      if (how.ok) {
        // THREE ANSWERS, NOT TWO. "Checked and swapped for", "checked and not", and "could not
        // check" are different facts: the public card hides what arrived unbidden, and on a day
        // the transfer history cannot be read it must not hide every unverified holding as if it
        // had. A row is only ever `acquired: false` when the chain was asked and said so — and
        // "said so" means the contract's arrival was actually in the history that was read. Past
        // a truncated read, a token the history never mentioned is unchecked, not unsolicited.
        //
        // BY THE BALANCE'S OWN CONTRACT. This was a symbol → address map, so two tokens of one name
        // shared whichever address was listed last, and the holding took its lookalike's verdict.
        for (const b of balances) {
          const a = b.address;
          if (!a) continue;
          const verdict = how.acquisition.get(a);
          if (verdict === ACQUIRED) b.acquired = true;
          else if (verdict != null || !how.truncated) b.acquired = false;
          else b.acquired = null;
        }
      }
    }
    ...
    const discovery = { used: !!found.ok, configured: found.configured, truncated: !!found.truncated,
                        error: found.ok ? null : (found.error || null), seen: targets.length, pinned: extras.length,
                        // How many of those the chain says are still held — the ones discovery missed.
                        recovered: balances.filter(b => b.address && extras.includes(b.address)).length };

    // ── A PRICE FOR WHAT THE VENUE DOES NOT LIST ─────────────────────────────────────────────
    // Hyperliquid is right for what Hyperliquid trades and silent about everything else, which
    // left a real holding in a $1.6m pool sitting unvalued beside genuine dust. Dexscreener is
    // asked only about the leftovers, once per chain.
    //
    // The quote leg is pinned BY ADDRESS to this chain's verified tokens plus the native asset —
    // on Robinhood Chain deliberate fake USDG and WETH clones sit in pools looking tradeable, and
    // a pair quoted against one returns a confident wrong number. Depth and volume floors do the
    // rest; lib/dexscreener.js keeps the reason a pair was refused.
    let merged = prices || new Map();
    const slug = DEX_CHAIN[chainKey];
    const heldAddr = new Set(balances.map(b => b.address).filter(Boolean));
    const unpriced = targets.filter(t => heldAddr.has(lc(t.address)) && !merged.has(t.name));
    if (slug && unpriced.length) {
      // PRICED BY CONTRACT, NOT BY SYMBOL. Discovery turns up tokens whose ticker is chosen to
      // collide — one on this very chain crams several hundred tickers into its symbol — and a
      // price keyed by symbol would hand one token another's mark. That used to be avoided by
      // leaving every shared name unpriced, which left the REAL token unpriced too the day a
      // lookalike arrived, and an unpriced row never reaches the card. Pool prices are keyed by
      // address instead (lib/hyperliquid.js spotHoldings reads the address first), so each token
      // gets its own pool's price and a lookalike's cannot land on the holding.
      const uniq = [...new Map(unpriced.map(t => [lc(t.address), t])).values()];
      const dex = await fetchDexPrices({
        chainSlug: slug, addresses: uniq.map(t => t.address), timeoutMs,
        quoteAllow: allowQuotes((chain.tokens || []).map(tk => tk.address)),
      });
      if (dex.ok && dex.prices.size) {
        // WHETHER THE TOKEN IS KNOWN, not merely where its price came from. A registry token is
        // one whose address was verified into lib/chains.js; if the venue happens to be silent and
        // its price arrives from a pool instead, it is still a known token and still counts. What
        // must not count is a token nobody vouched for — the airdrop that arrived unbidden.
        const known = new Set((chain.tokens || []).map(tk => String(tk.address).toLowerCase()));
        merged = new Map(merged);
        for (const t of uniq) {
          const key = String(t.address).toLowerCase();
          const q = dex.prices.get(key);
          if (q?.price == null) continue;
          merged.set(key, { price: q.price, volume: q.volume, changePercent: null,
                            viaPool: q.quote || true, verified: known.has(key) });
        }
      }
    }

    const out = { ok: true, error: null, ...shell, discovery, acquisition, ...spotHoldings(balances, merged) };

    // ── AND WHY, FOR THE ONES NOBODY VOUCHED FOR ─────────────────────────────────────────────
    // Asked only about tokens that are HELD and UNVOUCHED — the set someone means by "I don't
    // remember buying this". A verified token needs no verdict and a zero balance needs no
    // explanation, so neither costs a request.
    const suspect = out.rows.filter(r => !r.native && r.viaPool && !r.verified);
    if (SAFETY_CHAIN_IDS[chainKey] && suspect.length) {
      const safety = await fetchTokenSafety({
        chainKey, timeoutMs, addresses: suspect.map(r => r.address).filter(Boolean),
      });
      if (safety.ok) {
        for (const r of out.rows) {
          const f = safety.flags.get(lc(r.address));
          // NOTHING FLAGGED IS NOT A CLEAN BILL. The verdict travels verbatim; there is no
          // "safe" badge, because this checks known shapes of bad and cannot certify good.
          if (f) r.safety = { verdict: f.verdict, reasons: f.reasons };
        }
      }
    }
    return out;
...
// `pinned` maps a chain's label to contract addresses that must be read even past a truncated
// discovery — the card's provenance memory of what was ever swapped for.
export async function fetchWallets({ address = process.env[HL_ADDRESS_ENV], spotMeta, spotPrices,
                                     markets = {}, timeoutMs = 10000, pinned = {} } = {}) {
  ...
  const results = await Promise.all(keys.map(k => {
    ...
    return fetchWallet({ chain, chainKey: k, address, spotMeta, prices, timeoutMs, pinned: pinned?.[chain.label] || [] }).then(r => ({ key: k, ...r }));
  }));
