...
export async function fetchWallet({ chain = CHAINS.hyperevm, chainKey = 'hyperevm', address = process.env[HL_ADDRESS_ENV],
  ...
  try {
    ...
    // whatever discovery returned. Only past a TRUNCATED discovery: an untruncated one has already
    // seen every non-zero balance, and a chosen contract it did not list is one since sold.
    const lc = (a) => String(a || '').toLowerCase();
    let extras = [];
    if (found.ok && found.truncated) {
      const have = new Set(found.tokens.map(t => lc(t.address)));
      const chosen = how?.ok ? [...how.acquisition].filter(([, v]) => v === ACQUIRED).map(([a]) => lc(a)) : [];
      extras = [...new Set([...chosen, ...(pinned || []).map(lc)])].filter(a => /^0x[0-9a-f]{40}$/.test(a) && !have.has(a));
    }
