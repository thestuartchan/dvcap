...
export function eventPositioning({ events = EVENTS, trends = {}, nowMs } = {}, cfg = EVENT_CFG) {
  ...
  const reason = out.length ? null
    ...
    // SCOPE, SAID OUT LOUD. On 2026-09-09 this read "nothing inside 14 days — next is Micron (MU)
    // in 21" while the calendar on the same render showed US CPI in 2 days and FOMC + dot plot in
    // 7. The list is earnings only and always has been; unlabelled, "nothing inside 14 days" reads
    // as a general all-clear over a fortnight containing two tier-1 macro events.
    : next ? `no EARNINGS catalyst inside ${cfg.horizonDays} days — next is ${next.name} (${next.sym}) in ${next.daysTo}. Macro events (CPI, FOMC) are not in this list — see the calendar`
    : `no earnings catalyst inside the horizon — macro events are not in this list`;
  return {
    // Named on the object too, so a caller cannot render this panel without the qualifier.
    scope: 'earnings',
