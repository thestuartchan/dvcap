...
export function decayLines({ front, frontShare, expiringToday, support, moves, rest } = {}) {
  ...
  if (frontShare != null) {
    // ── "OF THE GROSS GAMMA" WAS A CLAIM ABOUT THE WHOLE BOOK ────────────────
    // It is a share of the expiries this read covers — six of roughly twenty, chosen in
    // lib/occ.js for a function budget and for a universe the rung below can be compared against.
    // A smaller denominator makes the share larger, so the old wording rounded UP a number whose
    // whole job is to say how much of what you are looking at is about to stop existing.
    out.push(`**${frontShare}% of the gamma in the expiries read expires at ${when}**${expiringToday ? '' : ` (${front})`}.`);
  }
  const wall = (label, s, m) => {
    if (!m) return;
    const heavy = s?.frontShare != null && s.frontShare >= SUPPORT_HEAVY;
    const side = label.startsWith('call') ? 'call' : 'put';
    if (m.material) {
      out.push(`The **${label} at ${m.from}** ${heavy ? `is ${s.frontShare}% that expiry, and ` : ''}`
        + `does not survive it — the heaviest ${side} positioning behind it sits at **${m.to}** `
  ...
  // "PUT-HEAVY STRIKE", NOT "PUT WALL". The put side's wall is now put SUPPORT (lib/gexLevels.js),
  // a positive node that qualifies by distance and by peaking; the heaviest put-side strike is a
  // different object — it can be the trapdoor — and it is what walls() hands this function.
  wall('put-heavy strike', support?.put, moves?.putWall);
  ...
  if (out.length === 1 && frontShare != null) {
    out.push('The call wall, the put-heavy strike and the pivot survive it — the levels on this board are not one expiry’s book.');
  }
  return out;
}
