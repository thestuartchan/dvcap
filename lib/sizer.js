...
export function sizeTrade({
  ...
  // What the book already carries in THIS underlying, in delta-notional (shares plus option
  // delta, summed per root — lib/bookExposure.js byUnderlying). Null means unknown, which is
  // reported as unknown, never as zero. `underlyingUnpriced` is how many of that root's lines the
  // book could not price, so a clean-looking total that is missing its largest leg says so.
  underlyingExposure = null, underlyingUnpriced = 0,
  ...
  const existing = num(underlyingExposure);
  const addedSingle = addedForBook;
  const combined = addedSingle == null ? null : (existing ?? 0) + addedSingle;
  const singleName = {
    cap: +capUsd.toFixed(2), capPct: L.singleNamePct,
    existing: existing == null ? null : +existing.toFixed(2), known: existing != null,
    unpriced: num(underlyingUnpriced) ?? 0,
    added: addedSingle == null ? null : +addedSingle.toFixed(2),
    combined: combined == null ? null : +combined.toFixed(2),
    pct: combined == null ? null : +((combined / eq) * 100).toFixed(1),
    past: combined != null && Math.abs(combined) > capUsd,
    // In shares of the underlying, at its price: 5 × 0.90 delta ≈ 450 shares.
    shareEquivalent: (combined != null && p > 0) ? Math.round(combined / p) : null,
    addedShareEquivalent: (addedSingle != null && p > 0) ? Math.round(addedSingle / p) : null,
    follows: ent != null ? 'entered' : 'suggested',
    deltaSource: isOption ? (deltaSource || (d != null ? 'exchange' : null)) : 'shares',
  };
  ...
  if (singleName.past) {
    warnings.push(`${symbol || 'this name'} would carry $${Math.round(Math.abs(singleName.combined)).toLocaleString('en-US')} of delta-notional (${singleName.pct}% of NLV), above the ${L.singleNamePct}% single-name cap`
      + (singleName.existing ? ` — $${Math.round(Math.abs(singleName.existing)).toLocaleString('en-US')} of it already held` : ''));
  }
...
export function sizerRun(result, { at = new Date().toISOString(), window = null } = {}) {
  ...
  return {
    ...
    singleName: result.singleName ? {
      underlying_exposure: result.singleName.combined ?? null,
...
export function capReview(runs = [], { now = new Date(), days = 30 } = {}) {
  ...
  const past = rows.filter(r => r.singleName.past_cap);
  const modelled = rows.filter(r => r.singleName.delta_source === 'modelled');
  return {
    n: rows.length, past: past.length, modelled: modelled.length, days,
    note: rows.length
      ? `${past.length} of ${rows.length} option entr${rows.length === 1 ? 'y' : 'ies'} in ${days}d exceeded the single-name cap on delta-notional${modelled.length ? ` · ${modelled.length} on a modelled delta` : ''}`
      : null,
  };
}
