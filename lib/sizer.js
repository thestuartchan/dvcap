...
export function sizeTrade({
  ...
  // The per-unit price the single-name arithmetic divides by: a share, or one contract.
  const perUnitPx = isFuture ? (p != null ? p * mult : null) : p;
  ...
  const unit = isOption ? (m != null ? m * L.contractMultiplier : null) : perUnitPx;
  const premium = (size != null && unit != null) ? +(size * unit).toFixed(2) : null;
  const perUnitDelta = isOption ? ((d != null && p != null) ? d * L.contractMultiplier * p : null) : perUnitPx;
