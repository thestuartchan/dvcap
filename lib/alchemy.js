...
export async function fetchAcquisition({ network, address, key = alchemyKey(), timeoutMs = 10000,
  ...
  // A capped read is REPORTED. A wallet with more transfers than the cap allows has a history this
  // cannot see the end of, and a token whose arrival sits past the cap is not "never paid for" —
  // it is unchecked. The caller keeps those as unknown rather than as airdrops.
  let truncated = false;
  ...
  try {
    ...
    return { ok: true, error: null, acquisition: out, truncated };
  } catch (e) {
    return { ok: false, error: redact(e?.message || e, key), acquisition: out, truncated };
  }
}
