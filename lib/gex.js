...
// walls() sums every expiry into one number per strike, which is the right aggregate and the wrong
// diagnostic. On 2026-09-01 this book's call wall and put wall both landed within a point of spot
// (707 and 708 against 707.14) while a commercial provider had them spread at 713 and 710 — the
// shape you get when a single 0DTE expiry swamps five others rather than when several expiries
// agree on a level.
//
// The contracts already carry their expiry and the raw chains are stored with it. The dimension is
// discarded at AGGREGATION, not at capture, so recovering it costs nothing but this function.
//
// A wall that six expiries agree on is a level. A wall that exists because today's expiry has
// enormous gamma at one strike is gone by tomorrow, and sizing around it is sizing around a
// number that will not survive the session.
export function gammaGrid(chain = [], { S, r = 0, q = 0, now } = {}) {
  const byExpiry = new Map();
  const cells = new Map();
  for (const c of (Array.isArray(chain) ? chain : [])) {
    const k = num(c?.strike), oi = num(c?.oi), e = c?.expiry || null;
    const g = contractGamma(c, { S, r, q, now });
    if (k == null || oi == null || g == null || oi <= 0 || !e) continue;
    const signed = (c.type === 'call' ? 1 : -1) * g * oi;

    if (!byExpiry.has(e)) byExpiry.set(e, { expiry: e, callGamma: 0, putGamma: 0, net: 0, callOi: 0, putOi: 0, strikes: new Map() });
    const ex = byExpiry.get(e);
    if (c.type === 'call') { ex.callGamma += g * oi; ex.callOi += oi; } else { ex.putGamma += g * oi; ex.putOi += oi; }
    ex.net += signed;
    ex.strikes.set(k, (ex.strikes.get(k) || 0) + signed);

    const ck = `${e}|${k}`;
    cells.set(ck, (cells.get(ck) || 0) + signed);
  }

  const expiries = [...byExpiry.values()].sort((a, b) => String(a.expiry).localeCompare(String(b.expiry)));
  const totalAbs = expiries.reduce((a, e) => a + Math.abs(e.net), 0);
  const rows = expiries.map(e => {
    // Each expiry's own heaviest strike either side — this is what says whether the aggregate wall
    // is one expiry's opinion or a consensus.
    let topCall = null, topPut = null;
    for (const [k, v] of e.strikes) {
      if (v > 0 && (topCall == null || v > topCall.v)) topCall = { k, v };
      if (v < 0 && (topPut == null || v < topPut.v)) topPut = { k, v };
    }
    return {
      expiry: e.expiry,
      netGexUsd: toDollarGex(e.net, S),
      callGexUsd: toDollarGex(e.callGamma, S),
      putGexUsd: toDollarGex(e.putGamma, S),
      callOi: e.callOi, putOi: e.putOi,
      shareOfAbs: totalAbs > 0 ? +((Math.abs(e.net) / totalAbs) * 100).toFixed(1) : null,
      peakCallStrike: topCall?.k ?? null,
      peakPutStrike: topPut?.k ?? null,
    };
  });

  const front = rows[0] || null;
  const frontShare = front?.shareOfAbs ?? null;
  // Above half, the aggregate is substantially one expiry's book. That is not wrong — it is what
  // the market is holding — but it is a different object from a multi-expiry level, and a reader
  // who is not told cannot know which one they are looking at.
  const dominated = frontShare != null && frontShare >= 50;

  return {
    expiries: rows,
    cells: [...cells.entries()].map(([ck, v]) => {
      const [expiry, k] = ck.split('|');
      return { expiry, strike: +k, netGexUsd: toDollarGex(v, S) };
    }),
    frontExpiry: front?.expiry ?? null,
    frontShare, dominated,
    // SAYS WHAT IT MEASURES, which is gamma CONCENTRATION. It used to end "— the walls are a
    // multi-expiry level rather than one day's positioning", which is a claim about the WALLS and
    // does not follow: on QQQ 2026-09-08 the front carried only 24.9%, so this passed, while one
    // of the six expiries actually peaked at the headline strikes. Whether the walls hold is
    // counted in lib/gexRead.js, where the walls are known; this does not guess at it.
    note: frontShare == null ? 'no expiry breakdown available'
      : dominated
        ? `${front.expiry} carries ${frontShare}% of the gamma in the expiries read — the headline walls are largely that one expiry's book, and it expires`
        : `gamma is spread across ${rows.length} expiries, the nearest carrying ${frontShare}% — no single expiry dominates the book`,
  };
}

// ── THE HEATMAP, SHAPED ──────────────────────────────────────────────────────
// gammaGrid returns every strike-by-expiry cell — on a real QQQ chain that is upwards of five
// hundred, most of them noise. This decides which get pixels and how hard each is shaded. It lives
// here rather than in the panel because both decisions are judgement calls with a wrong answer,
// and a wrong answer here is a chart that says the opposite of the truth.
export const HEAT_ROWS = 26;
export const HEAT_PCTL = 0.90;

// ── WHICH STRIKES GET A ROW ──
// Top-N ACROSS THE GRID hid the flip zone twice: on 21 Sep the table omitted 715–719 while the
// day's negatives sat there, and on 22 Sep it omitted 720–725 with the flip zone at 711–736. A
// grid-wide ranking is owned by whichever expiry is heaviest, and the strikes that matter today
// are usually light in the back months. So the ranking is PER COLUMN — each expiry's own heaviest
// strikes make the cut — and a short list of strikes can never be dropped: the flip-zone edges
// (and one strike either side), the trapdoor, the support, the call wall and the pin box. N grows
// to fit; the table is the evidence for the tiles and cannot be allowed to hide what they are about.
export const HEAT_MIN_PER_COLUMN = 3;

export function heatCells(grid, { rows = HEAT_ROWS, must = [], flipZone = null } = {}) {
  const cells = grid?.cells;
  const expiries = (grid?.expiries || []).map(e => e.expiry);
  if (!Array.isArray(cells) || !cells.length || !expiries.length) return null;

  const gross = new Map();
  const at = new Map();
  const perExpiry = new Map(expiries.map(e => [e, []]));
  for (const c of cells) {
    const v = num(c?.netGexUsd);
    if (c?.strike == null || v == null) continue;
    gross.set(c.strike, (gross.get(c.strike) || 0) + Math.abs(v));
    at.set(`${c.expiry}|${c.strike}`, v);
    if (perExpiry.has(c.expiry) && v !== 0) perExpiry.get(c.expiry).push({ k: c.strike, a: Math.abs(v) });
  }
  const universe = [...gross.keys()].sort((a, b) => a - b);
  const keep = new Set();
  // 1. Each column's own heaviest strikes.
  const perCol = Math.max(HEAT_MIN_PER_COLUMN, Math.ceil(rows / expiries.length));
  for (const list of perExpiry.values()) {
    list.sort((x, y) => y.a - x.a).slice(0, perCol).forEach(x => keep.add(x.k));
  }
  // 2. Fill to N with the grid-wide ranking, which is still the right tiebreak for the remainder.
  for (const [k] of [...gross.entries()].sort((a, b) => b[1] - a[1])) {
    if (keep.size >= rows) break;
    keep.add(k);
  }
  // 3. The strikes that may never be dropped. A strike not listed at all is mapped to its nearest
  // listed neighbour, so a flip at 720.5 forces 720 (and, as an edge, 715 and 725 beside it).
  const nearest = (v) => universe.length ? universe.reduce((a, b) => Math.abs(b - v) < Math.abs(a - v) ? b : a) : null;
  const forced = new Set();
  for (const m of must) { const k = nearest(num(m)); if (k != null) forced.add(k); }
  if (flipZone && num(flipZone.lo) != null && num(flipZone.hi) != null) {
    for (const edge of [flipZone.lo, flipZone.hi]) {
      const k = nearest(edge);
      if (k == null) continue;
      const i = universe.indexOf(k);
      for (const j of [i - 1, i, i + 1]) if (j >= 0 && j < universe.length) forced.add(universe[j]);
    }
  }
  const added = [...forced].filter(k => !keep.has(k)).sort((a, b) => b - a);
  for (const k of forced) keep.add(k);
  const strikes = [...keep].sort((a, b) => b - a);

  // ── WHY NOT SCALE TO THE LARGEST CELL ──
  // That was the first version and it defeated the chart. On a book with 80% of its gamma in one
  // expiry — the exact case this view exists to diagnose — the single heaviest cell sets the scale
  // and every other column washes out to blank. A reader concludes the other expiries hold nothing,
  // when what happened is that one cell ate the range.
  //
  // So the scale is capped at the 90th percentile of the cells shown. Combined with the square root
  // the panel applies, mid-range cells get visible separation instead of being crushed against zero.
  // It costs the ability to read magnitude off colour — which the tooltip and the by-strike chart
  // both give exactly — and buys comparison ACROSS a row, which is the only thing this view is for.
  const mags = [];
  for (const k of strikes) for (const e of expiries) {
    const v = at.get(`${e}|${k}`);
    if (v != null && v !== 0) mags.push(Math.abs(v));
  }
  mags.sort((a, b) => a - b);
  const cap = mags.length ? mags[Math.min(mags.length - 1, Math.floor(mags.length * HEAT_PCTL))] : 0;

  // ── THE ONE LEVEL EACH EXPIRY IS ABOUT ──
  // The heaviest cell in each column, by size, whatever its sign. The panel draws exactly one
  // cell per column with a ring in the sign's own colour, so the eye finds the day's level first
  // and reads its sign from the ring — without the shading, which is capped and compressed on
  // purpose, having to carry that job.
  ...
  for (const e of expiries) {
    ...
    if (best) colMax.set(e, best.k);
  }
  return { expiries, strikes, at, cap, shown: strikes.length, total: gross.size, perColumn: perCol,
           forced: [...forced].sort((a, b) => b - a), added, colMax };
}

// How hard to shade one cell, 0..1. Square-rooted on top of the percentile cap: the cap stops an
// outlier owning the range, the root spreads the mid-range out instead of crushing it against zero.
// The floor keeps a real-but-small cell visible — a blank cell has to mean "no gamma here", because
// that is how it will be read.
export const HEAT_ALPHA_FLOOR = 0.14;
export function heatAlpha(v, cap) {
  const x = num(v);
  const c = num(cap);
  if (x == null || x === 0 || c == null || c <= 0) return 0;
  return +(HEAT_ALPHA_FLOOR + (1 - HEAT_ALPHA_FLOOR) * Math.sqrt(Math.min(1, Math.abs(x) / c))).toFixed(3);
}
