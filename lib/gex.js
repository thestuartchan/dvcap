...
export function heatCells(grid, { rows = HEAT_ROWS } = {}) {
  ...
  // Strikes ranked by the gross gamma they carry ACROSS ALL EXPIRIES, then the heaviest kept —
  // not a fixed band around spot. The point of the view is to find where the book is concentrated,
  // and a band centred on spot assumes the answer before drawing it.
  const strikes = [...gross.entries()].sort((a, b) => b[1] - a[1]).slice(0, rows)
    .map(([k]) => k).sort((a, b) => b - a);
  ...
  return { expiries, strikes, at, cap, shown: strikes.length, total: gross.size };
}
