...
export function renderWatchlist(rows = []) {
  ...
  return rows.map(r => {
    ...
    // NO LEVERAGED PRODUCT BESIDE THE NAME. The row used to end with the 2x or inverse instrument
    // for the direction of the move — "[7709.HK up to 2x · HKEX]" after a rising SK Hynix. That
    // reads as "and here is how to go long it", which is a trade suggestion on a public channel
    // from a list whose only claim is that a name moved and whether its peers agreed. Removed
    // 2026-09-21; the watchlist observes, it does not point at an instrument.
    return line;
  }).join('\n');
}
