...
// Below this many INDEPENDENT views, agreement is not evidence of anything. Three is the smallest
// panel where one source can be the odd one out and still leave two to form a view.
export const THIN_PANEL = 3;

// DISPERSION MUST STATE THE COUNT, because the same number means opposite things at different panel
// sizes. Two sources both saying 35% produced "sources cluster within 0pp", which reads as a firm
// consensus and is the exact opposite of the truth: two views cannot disagree by much, and 0pp of
// spread across two of them is an absence of evidence rather than agreement. A reader cannot tell
// those apart from the spread alone, so the count goes in the sentence rather than beside it.
export function dispersionRead(c) {
  if (!c || c.value == null) return null;
  const n = c.nEffective ?? c.nSources ?? 0;
  if (c.spread == null) return { wide: false, thin: true, n, text: 'single source — no dispersion to read' };
  const panel = `${n} independent ${n === 1 ? 'view' : 'views'}`;
  // Wide is wide at any size: sources that far apart are not describing the same world, and that
  // is worth saying even when there are only two of them.
  if (c.spread >= WIDE_SPREAD_PP) {
    return { wide: true, thin: n < THIN_PANEL, n,
             text: `${panel} disagree by ${c.spread}pp (${c.lo}–${c.hi}%) — treat the level as a range, not a point` };
  }
  if (n < THIN_PANEL) {
    return { wide: false, thin: true, n,
             text: `only ${panel} — too few to disagree, so ${c.spread}pp of spread is not consensus` };
  }
  return { wide: false, thin: false, n, text: `${panel} cluster within ${c.spread}pp (${c.lo}–${c.hi}%)` };
}
