...
export const SEC_YIELDS = {
  ...
  // Both refreshed 2026-09-10 from the issuers' own pages. USFR read 3.71% as of 2026-08-06 and
  // SGOV 3.57% as of 2026-07-30 — 35 and 42 days old, and both wrong by more than the gap between
  // the two funds. SGOV is fetched live now (lib/fundYield.js); these remain the floor beneath both.
  // dtb3AtAsOf is FRED's DTB3 on the figure's OWN date, so the reconciliation measures the model
  // rather than the bill having moved: DTB3 was 3.80 on 2026-09-08.
  USFR: { value: 3.68, asOf: '2026-09-08', expense: 0.15, dtb3AtAsOf: 3.80, src: 'WisdomTree product page · 30-day SEC yield' },
  SGOV: { value: 3.62, asOf: '2026-09-08', expense: 0.09, dtb3AtAsOf: 3.80, src: 'iShares product page · 30 Day SEC Yield' },
};
...
//   SGOV +0.18pp — it holds 0–3 month bills averaging ~1 month, and the front curve is upward
//                  sloping here, so shorter paper earns less than DTB3.
//   USFR +0.06pp — the FRN discount margin net of the fee. The old model put this at +0.20pp,
//                  which was mostly the missing discount→BEY conversion.
//
// REFITTED 2026-09-10, when both funds' figures were refreshed from their issuers (SGOV 3.62,
// USFR 3.68, both as of 2026-09-08, DTB3 3.80 that day). They were 0.12 and −0.03, fitted to
// figures from 2026-07-30 and 2026-08-06.
//
// A NOTE ON WHAT A RE-FIT IS HERE, because I got this wrong on the way in and the suite caught it.
// The residual is NOT a statistical estimate over many days that would be overfitted by moving it
// on one observation. It is the DEFINITIONAL offset that makes the proxy reproduce the published
// figure at the bill rate of that figure's own date — test/cashyield.test.mjs asserts exactly that
// and calls it the calibration contract: "if it ever fails, the residual is stale and the card's ✓
// is lying". Refreshing an anchor without refitting its offset does not preserve caution, it
// breaks the identity the whole reconciliation rests on. The DRIFT from this anchor is what
// proxyDivergence measures, and that is the thing a 10bp threshold is watching.
//
// ── WHAT THE OLD OFFSETS WERE MISSING, MEASURED ──────────────────────────────
// Against the refreshed anchors at DTB3 3.80 (BEY 3.89), the PREVIOUS residuals read 6bp high on
// SGOV and 9bp high on USFR — same direction, similar size. Both were inside the 10bp threshold,
// which is why nothing had flagged them, and both were simply the anchors having aged: five and
// six weeks of front-curve drift the old offsets were still absorbing.
export const PROXY = {
  SGOV: { expense: 0.09, residual: 0.18, formula: 'BEY(DTB3) − 0.09% fee − 0.18% maturity' },
  USFR: { expense: 0.15, residual: 0.06, formula: 'BEY(DTB3) − 0.15% fee − 0.06% margin' },
};
export const PROXY_DIVERGENCE_BP = 10;

// The funds this card tracks, and where a person can read each one's figure. SGOV's is fetched
// (lib/fundYield.js); USFR's cannot be — WisdomTree sits behind a Cloudflare bot challenge that
// answers 403 to every automated route while loading fine in a browser — so it is hand-entered
// against this link.
export const SEC_YIELD_TICKERS = ['USFR', 'SGOV'];
export const ISSUER_PAGE = Object.freeze({
  USFR: { url: 'https://www.wisdomtree.com/investments/etfs/fixed-income/usfr',
          label: 'WisdomTree · USFR', field: 'SEC 30-Day Yield', fetchable: false,
          why: 'Cloudflare bot challenge — 403 to every automated route, loads in a browser' },
  SGOV: { url: 'https://www.ishares.com/us/products/314116/ishares-0-3-month-treasury-bond-etf',
          label: 'iShares · SGOV', field: '30 Day SEC Yield', fetchable: true, why: null },
});
...
// ── THE PROXY'S ERROR, MEASURED RATHER THAN ASSUMED ──────────────────────────
// The residuals in PROXY were fitted once and then trusted. SGOV's live SEC yield makes that
// testable: run the proxy at the SAME DTB3 the issuer's figure is dated to, and the difference is
// the model's error TODAY rather than on the day it was fitted.
//
// Measured 2026-09-10 — DTB3 3.80 discount, proxy says SGOV 3.68, iShares publishes 3.62: six basis
// points high, inside the 10bp re-fit threshold. That number is worth more than SGOV's own yield,
// because USFR's issuer cannot be fetched at all and this is the only evidence anywhere of how far
// off its estimate is likely to be.
export function proxyError(liveSgov, dtb3AtThatDate) {
  if (!liveSgov?.value || dtb3AtThatDate == null) return null;
  const est = secYieldProxy(dtb3AtThatDate);
  if (!est) return null;
  const bp = Math.round((est.SGOV - liveSgov.value) * 100);
  return {
    observed: liveSgov.value, estimated: est.SGOV, bp, asOf: liveSgov.asOf,
    // Stated as a tolerance on the OTHER fund, which is the only actionable thing it says.
    note: `checked against SGOV on ${liveSgov.asOf}: the estimate reads ${Math.abs(bp)}bp ${bp > 0 ? 'high' : 'low'}`
        + `${Math.abs(bp) > PROXY_DIVERGENCE_BP ? ' — past the re-fit threshold' : ''}`,
    diverged: Math.abs(bp) > PROXY_DIVERGENCE_BP,
  };
}
