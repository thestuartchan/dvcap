...
//
// ── BOTH RESIDUALS NOW READ HIGH, AND BY SIMILAR AMOUNTS ─────────────────────
// Until 2026-09-10 only SGOV's true value could be observed. With USFR's read off its own page the
// same day, both can be checked against DTB3 at 3.80 discount (BEY 3.89):
//
//   SGOV  proxy 3.68  ·  published 3.62  →  6bp high
//   USFR  proxy 3.77  ·  published 3.68  →  9bp high
//
// Same direction, similar size, and both inside PROXY_DIVERGENCE_BP. That consistency is what
// makes the tolerance stated on the card honest — the estimate for the fund that cannot be fetched
// is wrong in the same way and by roughly the same amount as the one that can.
//
// THE RESIDUALS ARE NOT RE-FITTED ON ONE OBSERVATION EACH. A fit to a single day is a fit to that
// day; proxyDivergence already computes the model error every run and flags a persistent drift past
// 10bp, which is the evidence a re-fit should rest on.
export const PROXY = {
