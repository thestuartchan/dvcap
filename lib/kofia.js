...
export function koreaFlowImplication(latest, { gate2 = null } = {}) {
  if (!latest) return null;
  const f = latest.foreignNet?.value, i = latest.instNet?.value;
  const ml = latest.marginLoans?.pct, dep = latest.deposits?.pct;
  ...
  if (gate2 === 'suspect' && soleBuyer === true) {
    ...
    const staleNote2 = staleSuffix(latest);
  ...
  return bias + caveat + staleSuffix(latest);
}
