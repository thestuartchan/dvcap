...
export function koreaFlowRead(latest) {
  ...
  const parts = [];
  if (f != null && i != null) {
    if (f > 0 && i > 0)      parts.push(`Foreign +${t(f)}${st('foreignNet')} & institutions +${t(i)}${st('instNet')} — both net buyers (accumulation, not flight)`);
    else if (f < 0 && i < 0) parts.push(`Foreign −${t(f)}${st('foreignNet')} & institutions −${t(i)}${st('instNet')} — both net sellers (broad de-risking)`);
    else                     parts.push(`Foreign ${f >= 0 ? '+' : '−'}${t(f)}${st('foreignNet')}, institutions ${i >= 0 ? '+' : '−'}${t(i)}${st('instNet')} — split flows`);
  } else if (f != null)      parts.push(`Foreign ${f >= 0 ? 'net buyers +' : 'net sellers −'}${t(f)}${st('foreignNet')}`);
