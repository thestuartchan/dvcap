...
export function kofiaStoredLine(key, e, now = new Date()) {
  if (!e || e.value == null) return null;
  const dt = e.asOf ? ` · ${e.asOf.slice(5)}${kofiaStale(e.asOf, now) ? ' ⚠stale' : ''}` : '';
...
export function koreaFlowRead(latest, { now = new Date() } = {}) {
  ...
  const t = (_v, k) => tOf(k);
  const st = k => kofiaStale(latest[k]?.asOf, now) ? ` ⚠stale(${latest[k].asOf.slice(5)})` : '';  // per-input
  const parts = [];
  if (f != null && i != null) {
...
function staleSuffix(latest, now = new Date()) {
  const drivers = ['foreignNet', 'instNet', 'marginLoans', 'deposits']
    .filter(k => latest[k] && kofiaStale(latest[k].asOf, now))
    .map(k => `${KOFIA_NAME_BY_KEY[k]} ${latest[k].asOf.slice(5)}`);
  return drivers.length ? ` ⚠ Stale inputs: ${drivers.join(', ')} — read is dated.` : '';
}

// ── IS THE INSTITUTION THE ONLY BUYER? ───────────────────────────────────────
// The absorption question, asked of all three legs rather than two. Foreign and retail both net
// sellers with institutions the sole positive leg is not a flow reading that supports anything on
// its own — it is one counterparty holding the tape up. Whether that matters depends on what the
// FX gate says about the same session, which is why this is exported and combined rather than
// folded into the bias.
export function instSoleBuyer(latest) {
  if (!latest) return null;
  const legs = { foreign: latest.foreignNet?.value, inst: latest.instNet?.value, retail: latest.retailNet?.value };
  // All three have to be readable. Two-of-three cannot establish "only", and inferring it from a
  // missing leg is how a qualifier starts firing on absent data.
  if (Object.values(legs).some(v => v == null)) return null;
  return legs.inst > 0 && legs.foreign <= 0 && legs.retail <= 0;
}

export function koreaFlowImplication(latest, { gate2 = null, now = new Date() } = {}) {
  if (!latest) return null;
  const f = latest.foreignNet?.value, i = latest.instNet?.value;
  const ml = latest.marginLoans?.pct, dep = latest.deposits?.pct;
  if (f == null && i == null) return null;

  // ── TWO PANELS, ONE FINDING ───────────────────────────────────────────────
  // Observed 2026-09-09. Cross-asset Gate 2 read SUSPECT — "the won strengthened but no unflagged
  // leg moved even half as far. A Korea-specific bid is what official smoothing looks like." On the
  // same render this read said "Dry powder is building — buffer and optionality intact for MORE
  // UPSIDE", off flows of Foreign −₩435bn, Retail −₩2,288bn, Institutional +₩942bn: institutions
  // the sole net buyer, absorbing selling five times heavier from retail than from foreigners.
  //
  // Those are the same finding twice — the bid is official or institutional rather than organic —
  // and the two panels reached it independently with neither aware of the other. This read was
  // generated from the flow table alone.
  //
  // Both conditions are surfaced in the text rather than only their conclusion, so the qualifier
  // can be audited from the panel instead of taken on trust.
  const soleBuyer = instSoleBuyer(latest);
  if (gate2 === 'suspect' && soleBuyer === true) {
    const wn = (v) => `₩${Math.abs(Math.round(v)).toLocaleString()}bn`;
    const r = latest.retailNet?.value;
    const staleNote2 = staleSuffix(latest, now);
    return 'SUPPORTED — NOT ORGANIC. Two independent reads land on the same thing: cross-asset '
      + 'Gate 2 is SUSPECT (the won strengthened with no clean leg moving half as far, which is '
      + 'what official smoothing looks like), and institutions are the sole net buyer here '
      + `(institutional +${wn(i)} against foreign −${wn(f)} and retail −${wn(r)}). `
      + 'A bid held up by one counterparty while both others distribute is support, not '
      + 'accumulation, and it lasts exactly as long as that counterparty keeps buying. No '
      + 'directional read is offered on this flow print.' + staleNote2;
  }
  ...
  return bias + caveat + staleSuffix(latest, now);
}
