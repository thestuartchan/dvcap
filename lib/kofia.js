...
// Stale prefix for the flow read/implication when the manual data hasn't been updated.
function flowStaleNote(latest, now = new Date()) {
  const d = latest.foreignNet?.asOf || latest.instNet?.asOf || latest.marginLoans?.asOf;
  return kofiaStale(d, now) ? `⚠ STALE (as of ${d?.slice(5)}, not updated) — ` : '';
}
...
export function koreaFlowRead(latest) {
  ...
  const t = v => `₩${(Math.abs(v) / 1000).toFixed(1)}T`;
  const parts = [];
  if (f != null && i != null) {
    if (f > 0 && i > 0)      parts.push(`Foreign +${t(f)} & institutions +${t(i)} — both net buyers (accumulation, not flight)`);
    else if (f < 0 && i < 0) parts.push(`Foreign −${t(f)} & institutions −${t(i)} — both net sellers (broad de-risking)`);
    else                     parts.push(`Foreign ${f >= 0 ? '+' : '−'}${t(f)}, institutions ${i >= 0 ? '+' : '−'}${t(i)} — split flows`);
  } else if (f != null)      parts.push(`Foreign ${f >= 0 ? 'net buyers +' : 'net sellers −'}${t(f)}`);
  if (ml != null)  parts.push(`margin loans ${ml >= 0 ? 'up' : 'down'} ${Math.abs(ml)}% (${ml >= 0 ? 'leverage building' : 'deleveraging'})`);
  if (dep != null) parts.push(`deposits ${dep >= 0 ? 'up' : 'down'} ${Math.abs(dep)}% (${dep >= 0 ? 'dry powder building' : 'cash deployed'})`);
  return parts.length ? flowStaleNote(latest) + parts.join('. ') + '.' : null;
}
...
export function koreaFlowImplication(latest) {
  ...
  return flowStaleNote(latest) + bias + caveat;
}
