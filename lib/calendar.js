...
// Every event dated within [from, to] inclusive: the hand-kept calendar plus the weekly recurring
// rules expanded over the same span, deduplicated the same way the week view is. This is the
// position sizer's catalyst window (lib/catalyst.js) — it reads the ONE calendar rather than a
// second store of the same dates. No region filter: the caller keeps what it needs.
export function eventsBetween(from, to) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(from || '') || !/^\d{4}-\d{2}-\d{2}$/.test(to || '') || to < from) return [];
  const normKey = e => e.date + '|' + e.title.replace(/\s*\([^)]*\)\s*/g, ' ').trim().toLowerCase();
  const staticEvents = cal.filter(e => e.date >= from && e.date <= to);
  const seen = new Set(staticEvents.map(normKey));
  // expandRecurring's end is exclusive — one day past `to` keeps the last day in.
  const end = new Date(to + 'T00:00:00Z'); end.setUTCDate(end.getUTCDate() + 1);
  const recurring = expandRecurring(from, end.toISOString().slice(0, 10)).filter(e => !seen.has(normKey(e)));
  return [...staticEvents, ...recurring].sort((a, b) => a.date.localeCompare(b.date) || (a.title < b.title ? -1 : 1));
}
