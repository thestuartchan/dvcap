...
export function weekHighlights(ref = new Date(), region = null, tz = null) {
  // Upcoming events from TODAY (region-local) through the next CAL_HORIZON_DAYS. Region
  // filter keeps that region's events PLUS scope:"global". Static hand-authored events +
  ...
  const staticEvents = cal.filter(e => e.date >= todayStr && e.date < horizonStr).filter(keep);
  const seen = new Set(staticEvents.map(normKey));
  const recurring = expandRecurring(todayStr, horizonStr).filter(keep).filter(e => !seen.has(normKey(e)));
