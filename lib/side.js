...
// The other way round. An oversell is the one event that needs it: selling more than a long holds
// does not clamp in the market, it opens a short for the excess.
export const oppositeSide = (x) => (sideOf(x) === 'short' ? 'long' : 'short');
...
// ── PAST TENSE TOO, BECAUSE A TALLY IS NOT A BUTTON ──────────────────────────
// The buttons on a short row read "+ Shorted" and "− Covered" and the fill form says "RECORD A
// COVER", and directly under them the tally read "100 bought · 0 sold" — on a row whose only fill
// is a sell. `bought` and `sold` are the engine's names for OPENED and CLOSED quantity (see
// lib/positions.js) and are correct as field names; printed raw they are long-only words on a
// short position, which is the one thing this file exists to stop.
export const FILL_VERB = Object.freeze({
  long:  { open: 'Bought',  close: 'Sold',    openShort: 'buy',   closeShort: 'sell',
           opened: 'bought', closed: 'sold' },
  short: { open: 'Shorted', close: 'Covered', openShort: 'short', closeShort: 'cover',
           opened: 'shorted', closed: 'covered' },
});
