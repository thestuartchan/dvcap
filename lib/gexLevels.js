...
export function negativeStack(byStrike, { spot } = {}) {
  ...
  const run = [];
  let i = 0;
  // Skip a first strike that is a hair below spot and positive only if it is inside the pin band?
  // No: the stack starts at the first listed strike below spot. If that is positive there is no
  // stack, and the honest line says so.
  while (i < rows.length && rows[i].net < 0) { run.push(rows[i]); i++; }
  const next = rows.slice(i).find(r => r.net > 0) || null;
  if (!run.length) {
