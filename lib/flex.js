...
import { isDerivativeRow, legsOf, underlyingOf, legKey, optionRow, legLabel } from './instruments.js';
...
// ── ADDING WHAT ONLY IBKR HOLDS ──────────────────────────────────────────────
// A share the console has never seen is added (rowFromPosition); an option now is too, the same
// way — one fill at the broker's average cost, flat by construction and said so in the thesis.
//
// LEGS OPENED TOGETHER ARE ONE SPREAD. A vertical held only at IBKR is two statement lines; added
// line by line it would become two unrelated option rows. So lines on the same underlying, opened
// the same day, in the same absolute size, become one spread row whose legs carry the signs and
// whose combo price is the signed sum of the legs' costs (positive a debit, negative a credit — the
// sign lib/instruments.js definedRisk reads). Anything that does not group cleanly is added one
// line per row. A line optionRow cannot build is returned unbuilt, to be reported.
export function optionAdds(lines = [], { today = new Date().toISOString().slice(0, 10), asOf = null } = {}) {
  const stamp = asOf || today;
  const groups = new Map();
  for (const p of lines) {
    const k = `${p.root}|${p.currency || 'USD'}|${p.openDate || stamp}`;
    groups.set(k, [...(groups.get(k) || []), p]);
  }
  const rows = [], unbuilt = [];
  const thesis = `Added from the IBKR statement of ${stamp}. One fill at the broker's average cost — a statement reports a position, not the fills behind it, so the entry history is flat by construction. Edit it if the detail matters.`;
  const build = (ps) => {
    const size = Math.abs(ps[0].qty);
    const legs = ps.map(p => {
      const c = optionContractOf(p);
      return c && { right: c.type === 'put' ? 'P' : 'C', strike: c.strike, expiry: c.expiry, side: p.qty < 0 ? 'short' : 'long', ratio: 1 };
    });
    if (legs.some(l => !l)) return null;
    const single = ps.length === 1;
    // One line: the row points the way the position does. Several: the legs carry the signs and
    // the row is long the combo at its net price.
    const side = single && ps[0].qty < 0 ? 'short' : 'long';
    const net = single ? ps[0].costBasisPrice
      : +ps.reduce((a, p) => a + (p.qty < 0 ? -1 : 1) * (p.costBasisPrice ?? NaN), 0).toFixed(4);
    if (!Number.isFinite(net)) return null;
    const made = optionRow({ underlying: ps[0].root, legs: single ? legs.map(l => ({ ...l, side: 'long' })) : legs, side,
      qty: size, price: net, date: ps[0].openDate || stamp, currency: ps[0].currency || 'USD', thesis,
      id: `${ps[0].root}-flex-opt-${ps.map(p => String(p.conid || p.symbol).replace(/\s+/g, '')).join('-').slice(0, 40)}` });
    if (made.error) return null;
    return { ...made.row, tags: ['new', 'flex'], fills: made.row.fills.map(f => ({ ...f, id: 'f0', note: single ? 'IBKR position average cost' : 'IBKR legs\' average costs, netted' })) };
  };
  for (const ps of groups.values()) {
    const together = ps.length >= 2 && ps.length <= 4 && ps.every(p => Math.abs(p.qty) === Math.abs(ps[0].qty))
      && ps.some(p => p.qty > 0) && ps.some(p => p.qty < 0);
    const spread = together ? build(ps) : null;
    if (spread) { rows.push(spread); continue; }
    for (const p of ps) { const r = build([p]); if (r) rows.push(r); else unbuilt.push(p); }
  }
  return { rows, unbuilt };
}
...
export function reconcile(rows = [], positions = [], { today = new Date().toISOString().slice(0, 10), asOf = null, derive = null } = {}) {
  ...
  for (const p of positions) {
    ...
    // a contract it was never compared with. Option lines are set aside here and matched by
    // CONTRACT against the console's option and spread legs below — never against a share row.
  ...
  // Held at IBKR and not in the console: added, like a share would be. Only what cannot be built
  // into a row is reported instead.
  const madeOpt = optionAdds(unheld, { today, asOf });
  adds.push(...madeOpt.rows);
  for (const p of madeOpt.unbuilt) {
    const c = optionContractOf(p);
    report.push({ kind: 'option-not-in-console', root: p.root, contract: c ? contractLabel(c) : p.symbol, qty: p.qty,
      note: 'held at IBKR and not in the console, and could not be added automatically — add it by hand' });
  }
...
// A row's name in a sentence: the ticker, and for an option or spread the contract as well.
export const addLabel = (r) => (isDerivativeRow(r) ? `${r.symbol} ${legLabel(legsOf(r))}` : r.symbol);

export function summarise(rec) {
  const bits = [];
  if (rec.adds.length) bits.push(`added ${rec.adds.map(addLabel).join(', ')}`);
...
// `forChannel`: the Discord line names TICKERS only — which contract (strike, expiry) is position
// detail, and like quantities and prices it stays in the console, behind the login.
export function summariseActionable(rec, { forChannel = false } = {}) {
  const bits = [];
  const name = forChannel ? (r) => (isDerivativeRow(r) ? `${r.symbol} ${r.instrument}` : r.symbol) : addLabel;
  if (rec.adds.length) bits.push(`added ${rec.adds.map(name).join(', ')}`);
  if (rec.differs.length) bits.push(`${rec.differs.length} disagree${rec.differs.length === 1 ? 's' : ''} with the statement (${rec.differs.map(d => d.root).join(', ')})`);
  if (rec.ambiguous.length) bits.push(`${rec.ambiguous.length} ambiguous (${rec.ambiguous.map(a => a.root).join(', ')})`);
  const missing = rec.report.filter(r => r.kind === 'missing-at-broker');
  if (missing.length) bits.push(`${missing.length} open here but not at the broker (${missing.map(m => m.root).join(', ')})`);
  const opt = optionFindings(rec);
  if (opt.length) bits.push(`${opt.length} option contract${opt.length === 1 ? '' : 's'} to check (${(forChannel ? [...new Set(opt.map(o => o.root))] : opt.map(o => o.contract)).join(', ')})`);
  return bits.join(' · ');
}
