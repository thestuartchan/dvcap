...
// ── REWINDING A ROW TO THE STATEMENT'S DAY ───────────────────────────────────
...
// cover the far more common case of a row that was merely REDUCED in it.
//
...
//
// So the console's own fills are replayed backwards to the statement's date before any comparison:
// undo the sells that happened after it, redo the buys. If the rewound quantity matches the
// statement, the two agree about the day the statement describes, which is the only day they can
// be asked to agree about.
export function qtyAsOf(row, asOf) { return rewindQty(row?.derived, asOf); }

// The same rewind against a DERIVED object rather than a row, because verify() in flexTrades.js
// derives its own and must ask the identical question — it is the gate that decides whether a batch
// is written at all, and a gate using different arithmetic from the report beside it is worse than
// either being wrong on its own.
export function rewindQty(derived, asOf) {
  const qty = derived?.qty;
  if (qty == null || !asOf) return null;
  const after = (derived?.fills || []).filter(f => f?.date && f.date > asOf);
  if (!after.length) return null;              // nothing moved — the live quantity IS the as-of one
  let q = qty;
  for (const f of after) {
    const n = Number(f.qty);
    if (!Number.isFinite(n)) return null;      // an incomplete fill makes the rewind unsound
    q += f.side === 'sell' ? n : -n;
  }
  return +q.toFixed(6);
}
...
export function reconcile(rows = [], positions = [], { today = new Date().toISOString().slice(0, 10), asOf = null } = {}) {
  ...
  const openRows = rows.filter(r => r?.derived?.status === 'open');
  ...
  for (const p of positions) {
    ...
    const rewound = qtyAsOf(r, asOf);
    if (!qtyOk && rewound != null && Math.abs(rewound - Math.abs(p.qty)) < 1e-6) {
      const moved = (r.derived?.fills || []).filter(f => f?.date && f.date > asOf);
      report.push({ kind: 'changed-since-statement', root: p.root, id: r.id,
        qty: { console: qty, ibkr: Math.abs(p.qty), asOf: rewound },
        note: `the statement of ${asOf} holds ${Math.abs(p.qty)}; ${moved.length} fill${moved.length === 1 ? '' : 's'} since then leave ${qty}, which rewinds to exactly the statement's figure — they agree about ${asOf}` });
      continue;
    }
    differs.push({
      id: r.id, root: p.root,
      qty: qtyOk ? null : { console: qty, ibkr: Math.abs(p.qty),
        // The rewind was tried and did NOT reconcile — which makes this a stronger finding than a
        // bare mismatch, not a weaker one, so it is said out loud rather than dropped.
        ...(rewound == null ? {} : { asOf: rewound }) },
...
//
// Left out deliberately: `unmatched` (the options and cash legs are outside the console's scope
// permanently, so they appear in every run for ever), and the two since-the-statement kinds, which
