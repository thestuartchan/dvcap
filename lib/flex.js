...
export function reconcilingFill(d, { asOf = null } = {}) {
  ...
  const a0 = Number(d.avg?.console), a1 = Number(d.avg?.ibkr ?? d.avg?.console);
  let price = null, priceNote;
  if (side === 'add' && Number.isFinite(a0) && Number.isFinite(a1) && q0 > 0) {
    ...
    priceNote = 'at the price received — a reduction does not move the average, so the statement cannot say';
  }
  const qty = Math.abs(dq);
  return {
    side, qty, price, date: asOf,
    text: `record ${qty} ${side === 'add' ? 'bought' : 'sold'} ${priceNote}${asOf ? `, dated on or before ${asOf}` : ''}`,
  };
}
