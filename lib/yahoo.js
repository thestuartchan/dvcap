...
export async function yahooChart(symbol, { range = "1y", interval = "1d" } = {}) {
  try {
    ...
    // THE BARS, NOT ONLY THE CLOSES. The same response carries a year of highs and lows, which
    // is what an ATR, a 52-week level and "narrowest day in seven" are made of — the watchlist's
    // setup tags come from these without a second fetch. Handed back as {date, high, low, close}.
    const q0 = result?.indicators?.quote?.[0] || {};
    const tsAll = result?.timestamp || [];
    const bars = [];
    for (let i = 0; i < tsAll.length; i++) {
      const h = q0.high?.[i], l = q0.low?.[i], cl = q0.close?.[i];
      if (typeof h !== "number" || typeof l !== "number" || typeof cl !== "number") continue;
      bars.push({ date: new Date(tsAll[i] * 1000).toISOString().slice(0, 10), high: h, low: l, close: cl });
    }
...
export async function yahooDailyOHLC(symbol, range = "1y") {
  return (await yahooDailyOHLCDetailed(symbol, range)).bars;
}

// WHY THERE ARE TWO. The plain form above returns [] for a transport failure, an HTTP error, a
// ticker Yahoo has never heard of, and a symbol with genuinely no bars — four different problems
// with four different remedies, flattened into one empty array. Callers that only need bars keep
// using it; callers that have to TELL A USER why there are none use this and get the reason.
export async function yahooDailyOHLCDetailed(symbol, range = "1y") {
  const fail = (status, extra = {}) => ({ bars: [], ok: false, status, symbol, ...extra });
  try {
    const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?interval=1d&range=${range}`;
    const r = await fetch(url, { headers: { "User-Agent": UA, Accept: "application/json" }, signal: AbortSignal.timeout(8000) });
    // A 404 is Yahoo saying it does not know the ticker; a 429 or 5xx is Yahoo saying "not now".
    // The first is the user's to fix, the second is worth retrying, and conflating them is what
    // made a throttled minute look identical to a typo.
    if (!r.ok) return fail(r.status === 404 ? "no-data" : "fetch-failed", { httpStatus: r.status });
    const body = await r.json();
    if (body?.chart?.error) return fail("no-data", { error: String(body.chart.error?.description || "").slice(0, 160) });
    ...
    if (!out.length) return fail("no-data", { rawBars: stamps.length });
    // WHAT THE FEED RESOLVED. The chart's meta names the instrument — "W&T Offshore, Inc." for
    // WTI — and its type. Carried so every screen that answers for a symbol can say which one it
    // answered for; see lib/tickerHints.js for why that came to matter.
    const meta = result?.meta || {};
    return { bars: out, ok: true, status: "ok", symbol, rawBars: stamps.length,
             name: meta.shortName || meta.longName || null, quoteType: meta.instrumentType || null,
             exchange: meta.exchangeName || null };
  } catch (e) {
    // A timeout is a fetch failure, not a missing symbol. AbortSignal.timeout throws TimeoutError.
    return fail("fetch-failed", { error: String(e?.name || e).slice(0, 60) });
  }
}
