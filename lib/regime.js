...
import { twoDimGate, OAS_LEVELS, OAS_WORDS, OAS_ESCALATE, VKOSPI_LEVELS, VKOSPI_WORDS, KRW_FLIP, levelOf, observationAge, OAS_MARKER, OAS_HISTORICAL } from './gates.js';
import { trend as trendOf } from './series.js';

// The bar both sector cuts speak above. It was a literal 1.5 in two places; a threshold with two
// homes is a threshold that will one day have two values.
export const SPLIT_BAR = 1.5;
...
export function memoryVsFoundry(quotes, names) {
  ...
  let label;
  if (Math.abs(spread) < SPLIT_BAR) label = 'moving together';
...
export function creditState(oasValue, oasSeries = null, oasDate = null) {
  ...
  // P2.2 — level notes are coarse regime context, deliberately flat in tone. No "approaching
  // the stress line" framing: at 2.8x the market is calm and both directions of resolution are
  // equally plausible.
  const NOTE = {
    CALM:         'calm credit market',
    WATCHFUL:     'off the tights — watchful',
    STRESSED:     'stressed',
    RECESSIONARY: 'recessionary territory',
  };
  // Trend over 5d/20d/60d in bps. At historically tight levels this is where the signal is —
  // the level band alone cannot distinguish 2.81 from 2.87.
  const bps = t => t ? Math.round(t.delta * 100) : null;
  const trend = {
    d5:  bps(trendOf(oasSeries, { lookbackDays: 5 })),
    d20: bps(trendOf(oasSeries, { lookbackDays: 20 })),
    d60: bps(trendOf(oasSeries, { lookbackDays: 60 })),
  };
  const obs = observationAge(oasDate);
  ...
  return {
    ...
    d1: g.d1, d5: g.d5,
    trend,                       // P2.2 — 5d/20d/60d in bps
    obs,                         // P2.1 — the FRED observation date, aged. Never computed.
    marker: OAS_MARKER,          // plain reference marker, NOT a gate
    historical: OAS_HISTORICAL,  // static annotations, explicitly not series data
...
export function computeRegime({ quotes, names, macro, korea }) {
  ...
  return {
    ...
    // P2.1 — the observation DATE is passed through from FRED, never derived as today−n.
    credit: creditState(macro?.oas?.value, macro?.oas?.series, macro?.oas?.date),
...
// ── WHAT FLIPS THE REGIME CARD ───────────────────────────────────────────────
// State → evidence → what changes it. The card showed the credit gate's level and direction and
// the two sector cuts' labels, and nothing about where any of them would change. Every threshold
// is a named constant already; this reads them back as one sentence, so the card can say "credit
// leaves CALM at 3.00, or earlier on three widening sessions; the cuts speak at 1.5pp" — and the
// sentence cannot drift from the constants, because it is built from them.
export function regimeFlipsIf(regime = {}, { oas = null } = {}) {
  const bits = [];
  const c = regime?.credit;
  const raw = c?.effective || c?.state || c?.level;
  if (raw) {
    const lvl = String(raw).toUpperCase();
    const i = OAS_LEVELS.findIndex(l => l.label === lvl);
    if (i >= 0) {
      const up = OAS_LEVELS[i].max, down = i > 0 ? OAS_LEVELS[i - 1].max : null;
      const where = [];
      if (up != null) where.push(`${OAS_LEVELS[i + 1]?.label ?? 'higher'} at ${up.toFixed(2)}`);
      if (down != null) where.push(`${OAS_LEVELS[i - 1].label} below ${down.toFixed(2)}`);
      let t = `credit leaves ${lvl}${oas != null && Number.isFinite(+oas) ? ` (now ${(+oas).toFixed(2)})` : ''}${where.length ? ' — ' + where.join(', ') : ''}`;
      // The escalation is the earlier exit: at tight levels the trend carries the signal, not the
      // band, and the gate says so by elevating CALM on a run of widening sessions.
      if (lvl === OAS_ESCALATE.fromLevel && !c?.escalated) {
        const runs = c?.runs || 0;
        t += `; or earlier, to ${OAS_ESCALATE.toLevel} on ${OAS_ESCALATE.minRuns} consecutive ${OAS_WORDS[OAS_ESCALATE.whenDir].toLowerCase()} sessions`
           + (c?.dir === OAS_ESCALATE.whenDir ? ` (${runs} so far)` : '');
      }
      bits.push(t);
    }
  }
  const cut = (name, x) => {
    if (!x || x.spread == null || !Number.isFinite(+x.spread)) return;
    const sp = Math.abs(+x.spread);
    bits.push(sp < SPLIT_BAR
      ? `${name} speaks at ${SPLIT_BAR}pp (now ${sp.toFixed(1)}pp apart)`
      : `${name} goes quiet below ${SPLIT_BAR}pp (now ${sp.toFixed(1)}pp apart)`);
  };
  cut('memory vs foundry', regime?.split);
  cut('AI vs the rest', regime?.aiAxis);
