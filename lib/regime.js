...
export function krxHaltSeverity(indexChangePct, futuresChangePct = null) {
  const out = { circuitBreaker: null, sidecar: null, fired: [], note: null };
  ...
  if (futuresChangePct != null && Math.abs(futuresChangePct) >= 5) {
    ...
    out.note = 'sidecar not evaluated — needs the futures move, not the cash index';
  }
...
export function koreaStress(korea) {
  ...
  const halt = krxHaltSeverity(korea?.kospiChangePct ?? null, korea?.futuresChangePct ?? null);
