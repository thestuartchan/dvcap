...
async function yahooBatch(syms) {
  ...
  for (let i = 0; i < syms.length; i++) {
    ...
    // A bar dated today is the session in progress; setupStats drops it by its date, not by
    // whether the market is open — Yahoo does not always serve the partial bar.
    const setup = c?.bars ? setupStats(c.bars, { today: new Date().toISOString().slice(0, 10) }) : null;
