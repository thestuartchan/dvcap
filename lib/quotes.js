...
function classifyDebasement({ gold, btc, dxy, realYield, oas }) {
  ...
  else if (g5 === 'down' && b5 === 'down') label = 'Deleveraging / dash-for-cash';
...
export async function getKoreaStress() {
  const [krw, vk, ks] = await Promise.all([
    yahooChart('KRW=X'),
    tvQuote('KRX:VKI1!'),
  ...
  return {
    ...
    futuresChangePct: null,   // sidecar needs the FUTURES move; not asserted off the cash index
