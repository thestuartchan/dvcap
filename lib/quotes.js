...
import { fredLatest2, fredSeries, fredYoYLatest } from './fred.js';
...
import { aligned, spreadBps, dayOf } from './derived.js';
import { closeOnlyAtr, spreadAtr } from './atr.js';
import { marketState } from './sessions.js';
import { setupStats } from './watchSetup.js';
...
function shape(sym, o = {}) {
  ...
  return {
    ...
    ts, stale, pctSuspect,
    src: o.src ?? 'unknown',
    // The watchlist's setup stats (lib/watchSetup.js), computed from the bars the chart fetch
    // already returned. Null when the fetch carried too few bars to say anything.
    setup: o.setup ?? null,
  };
}
...
async function yahooBatch(syms) {
  ...
  for (let i = 0; i < syms.length; i++) {
    ...
    // Before the open the last bar is yesterday and complete; during the session it is today and
    // partial. setupStats is told which so "yesterday" is the right bar either way.
    const setup = c?.bars ? setupStats(c.bars, { open: marketState(syms[i]) === 'open' }) : null;
    out[syms[i]] = c ? shape(syms[i], { ...c, setup, src: 'yahoo' })
...
export function classifyDebasement({ gold, btc, dxy, realYield, oas, breakeven, fwdBreakeven }) {
  ...
  // ── WHERE ON THE CURVE DID INFLATION EXPECTATIONS MOVE? ────────────────────
  // The discriminator took one undifferentiated "breakevens" input and had no view of the TERM
  // STRUCTURE, which is the only thing that separates the two stories it is meant to tell apart.
  //
  // Observed 2026-09-09: gold +1.27%, 10Y BE 2.37% (+2bps), 5y5y forward 2.34% (+1bp). The old rule
  // read "gold up + breakevens up" and confirmed debasement. But near-term expectations sitting
  // ABOVE the far forward is a supply-shock signature, not a monetary one — and Brent 100.45,
  // oil +2.70%, OVX +8.07% were on the same panel. A monetary repricing shows up in the FAR
  // forward, which moved one basis point.
  //
  // So: the 5y5y forward carries the monetary claim, and the 10Y-minus-5y5y spread says whether
  // what near-term movement there is came from the front of the curve.
  const FWD_FLAT_BPS = 2;   // ±2bps on the 5y5y — a far-forward move smaller than this is noise
  const fwdDir = fwdBreakeven?.deltaBps == null ? null
    : (Math.abs(fwdBreakeven.deltaBps) <= FWD_FLAT_BPS ? 'flat' : fwdBreakeven.deltaBps > 0 ? 'up' : 'down');
  // Positive = near-term expectations above the far forward = the front of the curve is doing it.
  const beSpreadBps = (breakeven?.value != null && fwdBreakeven?.value != null)
    ? +((breakeven.value - fwdBreakeven.value) * 100).toFixed(1) : null;
  ...
  // A 5d SPLIT BETWEEN GOLD AND BTC IS EVIDENCE, NOT ITS ABSENCE. Debasement is a monetary claim
  // and should bid both hard assets; gold rising while BTC falls over the same week is affirmative
  // evidence AGAINST the label, and returning "no regime signal" threw that away. Observed
  // 2026-09-09: gold 5d up, BTC 5d down. `n/a` is reserved for genuinely insufficient data.
  else if (g5 === 'up' && b5 === 'down')   label = 'DIVERGENT — gold-specific bid, debasement not supported';
  else if (g5 === 'down' && b5 === 'up')   label = 'DIVERGENT — BTC-specific bid, not a gold debasement move';
  else if (g5 == null || b5 == null)       label = 'n/a — no regime signal';
  else                                     label = 'MIXED — one leg flat, no regime signal';

  // P0.2 — the discriminator, now on the TERM STRUCTURE rather than one blended breakeven.
  //
  //   5y5y UP                          → DEBASEMENT CONFIRMING. The far forward is where a
  //                                      monetary repricing appears, and it moved.
  //   5y5y FLAT, 10Y ABOVE 5y5y        → OIL PASS-THROUGH. Near-term expectations above the far
  //                                      forward is the supply-shock signature: the front of the
  //                                      curve is pricing an energy move, the long run is not.
  //   5y5y FLAT, 10Y AT/BELOW 5y5y     → AMBIGUOUS. Do not label it either way.
  //   5y5y DOWN                        → REAL-YIELD TRADE, not debasement.
  //
  // Falls back to the old single-series rule only when the forward is unavailable, and says so —
  // a discriminator quietly running on half its inputs is the failure this replaces.
  // Two separable questions, and the answer to the first does not retire the second. "Gold and BTC
  // disagree, so this is not a monetary move" and "the front of the curve is pricing oil, which is
  // what lifted gold" are both true on 2026-09-09 and the reader wants both. So the term structure
  // is read whenever GOLD is up, and only REWRITES the label on the branch that was claiming
  // debasement; elsewhere it is added as the explanation of what did move the metal.
  const debasementish = label === 'Debasement bid (liquidity)' || label === 'Fiat distrust (warning)';
  let discriminator = null;
  if (g5 === 'up') {
    if (fwdDir === 'up') {
      discriminator = `DEBASEMENT CONFIRMING — 5y5y forward ${fwdBreakeven.deltaBps >= 0 ? '+' : ''}${fwdBreakeven.deltaBps}bps: the long-run inflation view is repricing, which is where a monetary story shows up`;
    } else if (fwdDir === 'down') {
      if (debasementish) label = 'Real-yield trade, not debasement (gold up, 5y5y forward down)';
      discriminator = `REAL-YIELD TRADE — 5y5y forward ${fwdBreakeven.deltaBps}bps: long-run inflation expectations are FALLING while gold rises`;
    } else if (fwdDir === 'flat' && beSpreadBps != null && beSpreadBps > 0) {
      if (debasementish) label = 'OIL PASS-THROUGH — not debasement (10Y BE above 5y5y, forward flat)';
      discriminator = `OIL PASS-THROUGH — the 5y5y forward moved ${fwdBreakeven.deltaBps}bps (flat within ${FWD_FLAT_BPS}) while the 10Y breakeven sits ${beSpreadBps}bps ABOVE it. Near-term expectations above the far forward is a supply-shock signature; a monetary repricing would show in the forward, and it did not`;
    } else if (fwdDir === 'flat') {
      if (debasementish) label = 'AMBIGUOUS — do not label (gold up, 5y5y forward flat)';
      discriminator = `AMBIGUOUS — the 5y5y forward is flat (${fwdBreakeven.deltaBps}bps) and the 10Y breakeven is ${beSpreadBps == null ? 'unavailable' : `${Math.abs(beSpreadBps)}bps ${beSpreadBps < 0 ? 'below' : 'level with'} it`}, so neither the monetary nor the supply story is supported`;
    } else if (beDir) {
      // No forward available — the old single-series rule, explicitly degraded.
      if (debasementish && beDir === 'flat')      label = 'AMBIGUOUS — gold up, breakevens flat (bid unexplained)';
      else if (debasementish && beDir === 'down') label = 'Real-yield trade, not debasement (gold up, breakevens down)';
      discriminator = '⚠ 5y5y forward unavailable — discriminator running on the 10Y breakeven alone, which cannot separate oil pass-through from a monetary repricing';
    }
  }
  ...
  // FROM THE INPUTS, NOT FROM THE LABEL TEXT. These were regexes over the label, and /Debasement/i
  // matches "Real-yield trade, NOT debasement" and "OIL PASS-THROUGH — NOT debasement" — so every
  // label that exists to deny the debasement reading was being treated as asserting it, and the
  // coherence check fired backwards on exactly the cases it was built to catch. The direction the
  // label implies is just what the 5d co-movement was; read it from there.
  const impliedUp   = g5 === 'up' && b5 === 'up';
  const impliedDown = g5 === 'down' && b5 === 'down';
  let mismatch = null;
  ...
  return {
    label, mismatch, windowSplit, discriminator,
    // The term structure, so the label can be audited from the panel rather than trusted.
    breakevens: { tenYrBps: breakeven?.deltaBps ?? null, fwd5y5yBps: fwdBreakeven?.deltaBps ?? null,
                  fwdDir, spreadBps: beSpreadBps, fwdFlatBps: FWD_FLAT_BPS },
...
export async function getMacro() {
  const [wti, brent, dgs2, dgs10, dgs30, oas, dxyC, goldC, btcC, dfii10, t10yie, fwd5y5y, termPrem, moveC, ovxC, oasHist, dgs30Hist, dfii10Hist, coreCpiRaw, corePceRaw, dgs5, dgs10Hist] = await Promise.all([
    ...
    // T10YIE IS PUBLISHED AHEAD OF ITS OWN INPUTS. FRED defines the 10Y breakeven as DGS10 − DFII10,
    // yet on 2026-08-27 it carried the 27th while both DGS10 and DFII10 carried the 26th — so the
    // breakeven on screen may be constructed against a nominal that is not visible here yet.
    // Observed, not explained; the likeliest reading is a different release cadence on the derived
    // series, but nothing in the API says so and it is not worth asserting. It is why the Fisher
    // check below aligns on dates rather than assuming these three move together.
    ...
    // 40 obs ≈ two months of sessions, so a 20-session sparkline still has a full window after
    // FRED's placeholder rows are filtered out.
    fredSeries('DFII10', 40),        // the 10Y real-yield sparkline: the TURN, not the level
    // The two core inflation series, so the brief can carry the gap between them rather than a
    // level. They disagree structurally — PCE normally runs below core CPI on a much lighter
    // shelter weight — so the SIGN is the information, and it is inverted right now.
    fredYoYLatest('CPILFESL'), fredYoYLatest('PCEPILFE'),
    fredLatest2('DGS5'),             // Section E — 5s30s slope (long-end steepening)   // real prior prints — backs the OAS 2-D gate's direction
    // THE 10Y HISTORY EXISTS FOR ONE REASON: the 10s30s ATR. The scenario board gates every
    // magnitude on half that instrument's own daily range, and the range of a SPREAD cannot be
    // inferred from either leg — 5bp is nothing on a 30Y yield and a great deal on a 10s30s box.
    // Without it the condition is permanently neutral, which is honest and useless.
    fredSeries('DGS10', 1260),
  ]);
  ...
  const us30y = { ...yf(dgs30, 'US 30Y', 'DGS30'), series: dgs30Hist,
    // The instrument's own daily range, in its own units (percent of yield). What the scenario
    // board divides by before deciding whether a move said anything.
    atr: closeOnlyAtr(dgs30Hist),
  ...
  const fives30sA     = spreadBps({ name: 'DGS30', value: dgs30.value, date: dgs30.date },
                                  { name: 'DGS5',  value: dgs5.value,  date: dgs5.date });
  const fives30sPrevA = spreadBps({ name: 'DGS30', value: dgs30.prev, date: dgs30.prevDate },
                                  { name: 'DGS5',  value: dgs5.prev,  date: dgs5.prevDate });
  const fives30s = fives30sA.value, fives30sPrev = fives30sPrevA.value;
  ...
  // EVERY DERIVED VALUE GOES THROUGH `aligned`. A spread built from a stale leg and a live one is a
  // real number of the right magnitude with a plausible date attached, and nothing about it looks
  // wrong — see lib/derived.js. The card gets the alignment result, not just the number, so it can
  // say that it stopped rather than going quiet.
  const twos10sA     = spreadBps({ name: 'DGS10', value: dgs10.value, date: dgs10.date },
                                 { name: 'DGS2',  value: dgs2.value,  date: dgs2.date });
  const twos10sPrevA = spreadBps({ name: 'DGS10', value: dgs10.prev, date: dgs10.prevDate },
                                 { name: 'DGS2',  value: dgs2.prev,  date: dgs2.prevDate });
  const twos10s = twos10sA.value, twos10sPrev = twos10sPrevA.value;

  // ── Regime inputs (debasement/stagflation read) ──
  const gold = of(goldC, 'Gold', 'GC=F');
  const btc = of(btcC, 'BTC', 'BTC-USD');
  const realYield = { ...yf(dfii10, '10Y Real', 'DFII10'), series: dfii10Hist };
  ...
  const regimeSignal = classifyDebasement({ gold, btc, dxy, realYield, oas: oasF, breakeven, fwdBreakeven });
  ...
  // Two live ticks are comparable; a live one against yesterday's settle is not, and that is
  // exactly how a negative Brent−WTI gets manufactured out of two individually-correct prices.
  const brentWtiA = aligned([
    { name: 'Brent', value: brentF.value, date: dayOf(brentF.ts) },
    { name: 'WTI',   value: wtiF.value,   date: dayOf(wtiF.ts) },
  ], (b, w) => +(b - w).toFixed(2));
  const brentWtiSpread = brentWtiA.value;
  ...
  // ── FISHER IDENTITY: 10Y nominal ≈ 10Y real + 10Y breakeven ──────────────────
  // The guard that catches a stale or mis-mapped DFII10 — and the one place on the board that
  // genuinely derives a value from three series, so it was the one most exposed to the vintage
  // problem. It was comparing a 26 Aug nominal and real against a 27 Aug breakeven and passing at
  // 1bp, entirely because the tolerance was ten times the error it was making.
  //
  // TOLERANCE: 3bp, not 10. On matched vintages the identity holds near-exactly; the only
  // legitimate slack is rounding, and three series published to two decimals gives about 1bp of it.
  // Ten was wide enough to pass a genuinely mis-mapped series — DFII5 substituted for DFII10 would
  // plausibly land inside it — which is a guard that cannot catch the thing it exists for.
  //
  // AND IT SAYS WHEN IT SKIPS. A check that silently stops running is worse than one that never
  // existed: the card looks the same and nothing is watching it. On a vintage mismatch the result
  // carries checked:false and the reason, for the frontend to render.
  const fisher = aligned([
    { name: 'DGS10',  value: us10y.value,     date: us10y.date },
    { name: 'DFII10', value: realYield.value, date: realYield.date },
    { name: 'T10YIE', value: breakeven.value, date: breakeven.date },
  ], (n, r, b) => Math.round((n - (r + b)) * 100));
  const FISHER_TOL_BPS = 3;
  if (fisher.ok && Math.abs(fisher.value) > FISHER_TOL_BPS) {
    sanity.push(`Fisher identity off: 10Y ${us10y.value}% vs real+BE ${(realYield.value + breakeven.value).toFixed(2)}% (${fisher.value}bps, tolerance ${FISHER_TOL_BPS}) — check DFII10/T10YIE`);
  }
  ...
  return {
    ...
    // In BASIS POINTS, matching the units the 10s30s condition is expressed in.
    tenThirtyAtrBps: spreadAtr(dgs30Hist, dgs10Hist, 14, 100),
    // The alignment beside the number, so a card can show WHY it is blank instead of just being
    // blank. `date` is the one both legs share — never one leg's date standing in for the pair.
    twos10sAlign: { checked: twos10sA.checked, date: twos10sA.date, reason: twos10sA.reason },
    fives30sAlign: { checked: fives30sA.checked, date: fives30sA.date, reason: fives30sA.reason },
    brentWtiAlign: { checked: brentWtiA.checked, date: brentWtiA.date, reason: brentWtiA.reason },
    fisher: { checked: fisher.checked, gapBps: fisher.value, date: fisher.date, reason: fisher.reason, toleranceBps: FISHER_TOL_BPS },
    brentWtiSpread, sanity,
    gold, btc, realYield, breakeven, fwdBreakeven, termPremium, move, ovx, regimeSignal,
    coreCpi: coreCpiRaw, corePce: corePceRaw,
...
export const CROSS_ASSETS = {
  ...
  // USD/CNH is here for the same reason USD/KRW is: half the Asia book is China and Hong Kong —
  // HSTECH, HSI, SMIC, BYD, Alibaba, Tencent, Biren — and the board had a currency gauge for Korea
  // and none for China. Offshore (CNH) rather than onshore (CNY): CNY is a managed fix and its
  // quote goes stale outside Shanghai hours, so it reports the PBoC's intention where CNH reports
  // what is being paid.
  fx:        { label: 'FX legs (DXY is a blend — read the legs)', rows: [
    { sym: 'EURUSD=X', name: 'EUR/USD' }, { sym: 'JPY=X', name: 'USD/JPY' },
    { sym: 'KRW=X', name: 'USD/KRW' }, { sym: 'CNH=X', name: 'USD/CNH' },
    { sym: 'DX-Y.NYB', name: 'DXY (summary)' }] },
};

// ── EVERY FX LEG IN THE DOLLAR'S FRAME ───────────────────────────────────────
// EUR/USD is quoted as dollars per euro, so it rises when the dollar WEAKENS. USD/JPY and USD/KRW
// are quoted the other way up and fall when the dollar weakens. All three can therefore report the
// same event with opposite signs, and on 2026-09-03 they did: EUR/USD +0.09%, USD/JPY −2.03%,
// USD/KRW −1.08%, every one of them meaning a softer dollar.
//
// The panel already knew this and only said so in prose. fxDivergence's note read "USD −0.09% vs
// EUR" while the card four rows below read "+0.09%" — the same fact, opposite signs, on one screen,
// under a heading that says READ THE LEGS and so invites exactly the left-to-right comparison the
// mixed convention breaks.
//
// The quoted number is NOT touched. It is what every other source shows and inverting it would
// make this panel the odd one out. This adds the second reading beside it, so the row can be
// scanned without the reader holding a conversion in their head.
// CNH=X is quoted offshore yuan per dollar, the same way up as the yen and the won — so a RISING
// print is a weaker yuan and a stronger dollar. Registering it here is what stops `usdLeg`
// returning null for it and silently dropping the largest Asian currency out of the dollar read.
const USD_IS_BASE = { 'JPY=X': 'JPY', 'KRW=X': 'KRW', 'CNH=X': 'CNH' };   // quoted as foreign per USD
const USD_IS_QUOTE = { 'EURUSD=X': 'EUR' };               // quoted as USD per foreign — inverted

export function usdLeg(row) {
  const pct = row?.changePct;
  if (pct == null || !Number.isFinite(+pct)) return null;
  const inverted = USD_IS_QUOTE[row.sym];
  const direct = USD_IS_BASE[row.sym];
  if (!inverted && !direct) return null;                  // DXY is the dollar already; not a leg
  const usdPct = +(inverted ? -pct : pct).toFixed(2);
  return {
    vs: inverted || direct,
    pct: usdPct,
    inverted: !!inverted,
    // Below a rounding hair the sign is noise, so it is called flat rather than given a direction.
    dir: Math.abs(usdPct) < 0.005 ? 'flat' : usdPct > 0 ? 'stronger' : 'weaker',
    label: `USD ${usdPct >= 0 ? '+' : ''}${usdPct.toFixed(2)}% vs ${inverted || direct}`,
  };
}
...
function crossRow(sym, name, c) {
  if (!c || c.price == null) return { sym, name, price: null, dir: null, usdLeg: null, note: 'no print' };
  ...
  // Computed here rather than in the panel, so the sign convention lives with the rest of the FX
  // knowledge and the client renders what it is given. Null for everything that is not an FX leg.
  row.usdLeg = usdLeg(row);
  return row;
}
