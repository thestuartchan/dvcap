...
export async function getMacro() {
  const [wti, brent, dgs2, dgs10, dgs30, oas, dxyC] = await Promise.all([
    ...
    fredLatest('DGS2'), fredLatest('DGS10'), fredLatest('DGS30'), fredLatest('BAMLH0A0HYM2'),
  ...
  return {
    ...
    us30y: { ...dgs30, name: 'US 30Y' },
    oas:   { ...oas,   name: 'HY OAS', note: 'FRED daily — last hard print' },
    dxy:   dxyC ? { value: dxyC.price, changePct: dxyC.changePct, ts: dxyC.ts, name: 'DXY' }
                : { value: null, name: 'DXY' },
    // 2s10s in bps (10Y − 2Y). Negative = inverted.
    twos10s: (dgs10.value != null && dgs2.value != null)
      ? Math.round((dgs10.value - dgs2.value) * 100) : null,
  };
}
