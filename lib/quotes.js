...
export async function getMacro() {
  const [wti, brent, dgs2, dgs10, dgs30, oas, dxyC] = await Promise.all([
  ...
  // ── Sanity / relationship checks: flag, don't hide (values still render) ──
  const sanity = [];
  const band = (v, lo, hi, label) => { if (v != null && (v < lo || v > hi)) sanity.push(`${label} out of band: ${v}`); };
  band(wtiF.value, 20, 150, 'WTI'); band(brentF.value, 20, 150, 'Brent');
  band(dxy.value, 80, 120, 'DXY'); band(oasF.value, 1, 12, 'HY OAS');
  band(us2y.value, 0, 10, 'US 2Y'); band(us10y.value, 0, 10, 'US 10Y'); band(us30y.value, 0, 10, 'US 30Y');
  band(twos10s, -200, 300, '2s10s');
  const brentWtiSpread = (brentF.value != null && wtiF.value != null) ? +(brentF.value - wtiF.value).toFixed(2) : null;
  if (brentWtiSpread != null && brentWtiSpread < 0) sanity.push(`⚠ Brent < WTI (${brentWtiSpread}) — check tickers`);
