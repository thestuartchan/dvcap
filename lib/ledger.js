...
export function handKeptLedger({ manual = {}, kofia = null, consts = {}, files = {}, now = new Date() } = {}) {
  ...
  const fp = manual?.fedPath?.latest || null;
  {
    ...
    const feed = consts.fedPathFeed;
    if (feed?.ok) {
      // Fed from the CBOT strip. The hand entry is now an override for its own date, not a chore.
      rows.push(row({ key: 'fedPath', label: 'Fed path — ZQ futures', group: 'Manual entry', asOf: feed.asOf ?? null, days: feed.asOf ? calDays(feed.asOf, now) : null,
        cadence: 'fed daily (Yahoo, last close)', state: 'retired', rule: 'fed; a typed settle outranks the feed for its own date', effect: 'Fed path card, ladder',
        where: 'Macro · Market-implied Fed path panel (entry optional)', note: fp?.date ? `last hand entry ${fp.date}` : 'no hand entry needed' }));
    } else {
      rows.push(row({ key: 'fedPath', label: 'Fed path — ZQ futures settle', group: 'Manual entry', asOf: fp?.date ?? null, days: fp?.date ? calDays(fp.date, now) : null, bizDays: bd,
        cadence: 'daily (3 business days) — feed unavailable', state: !fp ? 'missing' : bd > 3 ? 'stale' : 'fresh',
  ...
  if (calendar) {
    ...
    rows.push(row({ key: 'calendar', label: 'Macro and earnings calendar (auctions are fed from TreasuryDirect)', group: 'Data files', asOf: lastDate, days: lastDate ? -calDays(lastDate, now) : null, cadence: 'ongoing',
