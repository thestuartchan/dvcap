...
export const REGIME_STATES = {
  HAWKISH_RATES_REPRICING: { label: 'Hawkish rates repricing', color: P.amber700 },
  DEFLATIONARY_RECESSION:  { label: 'Deleveraging / dash-for-cash', color: P.blue700 },
  STAGFLATION:             { label: 'Stagflation', color: P.amber700 },
  REFLATIONARY_GROWTH:     { label: 'Reflationary growth', color: C.green },
  INFLATIONARY_BOOM:       { label: 'Inflationary boom', color: P.violet600 },
  ...
  DISINFLATIONARY_EASING:  { label: 'Disinflationary easing', color: P.teal700 },
  INSUFFICIENT_DATA:       { label: 'Insufficient data', color: C.muted },
};
...
// P2 (extends F2) — what the gold-pair reading means specifically for GOLD MINERS. Miners are
// leveraged gold equities, so the pair decides whether they're a genuine stagflation asset or a
// leveraged rates bet wearing a gold costume:
//   INFLATION_HEDGE  (gold↑ + breakevens↑)  → miners are a real stagflation asset.
//   REAL_YIELD_DRIVEN(gold↑ + breakevens↓)  → miners are a leveraged bet on Fed easing that
//                                             reverses hard on a hawkish repricing — NOT a hedge.
// Returns null for readings that don't support a miners call (ambiguous/neutral/gold falling).
export function minersPairImplication(reading) {
  switch (reading) {
    case 'INFLATION_HEDGE':
      return { live: 'inflation-hedge', ok: true,
        text: 'Gold is being bought as an inflation hedge (breakevens up) — miners are a genuine stagflation asset in this state.' };
    case 'REAL_YIELD_DRIVEN':
      return { live: 'real-yield trade', ok: false,
        text: 'Gold is bid on FALLING REAL YIELDS, not inflation (breakevens down) — miners are a leveraged bet on Fed easing and reverse hard on a hawkish repricing, not a stagflation hedge.' };
    case 'DISINFLATION':
      return { live: 'disinflation', ok: false,
        text: 'Gold and breakevens both point to receding inflation — miners are not an inflation play here.' };
    default:
      return null; // AMBIGUOUS / NEUTRAL / gold flat-or-down — no clean miners call
  }
}
