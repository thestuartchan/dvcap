...
export function mapRow(name, { spot, putWall, callWall, flipLevel, trapdoor = null } = {}) {
  ...
  // The trapdoor first, so a support or wall rounding onto the same column wins the cell: the
  // hedging that leans against price is the mark a reader is placing against.
  const td = trapdoor?.near?.strike ?? trapdoor?.deep?.strike ?? null;
  if (td > 0) put(pctOf(td), 'T');
...
//
// ── GROUPED BY INSTRUMENT, NOT BY SECTION ────────────────────────────────────
// This used to run five loops over the symbols — summaries, then every symbol's real levels, then
// every symbol's day shape, then every symbol's expiries — so reading what QQQ's book was saying
// meant finding four QQQ headings scattered among four SPY ones. Nobody trades both at once. One
// block per instrument, in one place, with the cross-instrument comparisons (the map, and what is
// anchored against what) kept where they belong: outside the blocks, because neither row can make
...
export function renderGexSection(rows = [], { rung = 'none', from = null, asOf = null, tense = 'preview', spotSource = null, ivAgeMin = null } = {}) {
  ...
  out.push('```');
  const split = live.some(r => r.levels);
  out.push(split
    ? '`^` price · `|` pivot · `P` put support · `T` trapdoor · `C` call wall · `B` support and call wall at one strike'
    : '`^` price · `|` pivot · `P` put wall · `C` call wall · `B` both at one strike');

  // The band's caveat is said ONCE, here, rather than repeated under every instrument. A BAND IS
  // NOT A BOUNDARY: roughly one day in three finishes outside it, and a number printed without
  // that reads as a limit.
  if (live.some(r => expectedRange(r.spot, r.iv))) {
  ...
  live.forEach((r, i) => {
    ...
    out.push(SPACER);
    out.push(`__**${r.name}** ${f2(r.spot)}__`);
    if (r.levels) {
      // ── THREE OBJECTS ON THE PUT SIDE, NEVER ONE WORD ──
      // "put wall 700" was a trapdoor on 17 and 18 Sep, a sum on the 21st and the pin on the 22nd.
      const lv = r.levels;
      const sp = lv.support?.strike ? `put support ${f2(lv.support.strike)} (${pctFrom(lv.support.strike, r.spot)})` : 'put support: none inside 5%';
      const tdn = lv.trapdoor?.near?.strike ?? lv.trapdoor?.deep?.strike ?? null;
      const td = tdn ? `trapdoor ${f2(tdn)} (${pctFrom(tdn, r.spot)})` : 'no trapdoor inside 10%';
      const kind = lv.callWall?.kind ? `, ${lv.callWall.kind}` : '';
      out.push(`• ${sp} · ${td}`
        + ` · pivot ${f2(r.flipLevel)} (${pctFrom(r.flipLevel, r.spot)})`
        + ` · call wall ${f2(r.callWall)} (${pctFrom(r.callWall, r.spot)}${kind})`);
    } else {
      out.push(`• put wall ${f2(r.putWall)} (${pctFrom(r.putWall, r.spot)})`
        + ` · pivot ${f2(r.flipLevel)} (${pctFrom(r.flipLevel, r.spot)})`
    ...
    const e = expectedRange(r.spot, r.iv);
    if (e) out.push(`• 📏 **Priced for** ±${e.pts} (±${e.pct}%)`);

    const lv = realLevels({ spot: r.spot, putWall: r.putWall, callWall: r.callWall,
                            byStrike: r.byStrike, agreement: r.agreement, pin: r.pin, levels: r.levels });
    if (lv.length) { out.push(SPACER, '**levels that are real**', ...lv); }

    const dk = dayKind(r);
    if (dk.length) { out.push(SPACER, '**what kind of day**', ...dk); }

    // ── P8 — WHAT STOPS EXISTING TONIGHT ─────────────────────────────────────
    // Every level above has an expiry date and none of them said when. A put wall carrying 6.5× the
    // calls is a floor; the same wall at 62% today's expiry is a floor until 4pm, and the two used
    // to render identically. Computed in lib/gexDecay.js by running walls() and flipLevel() over the
    // chain with the front expiry removed — exact, not an estimate.
    const dl = r.decay?.lines || [];
    if (dl.length) { out.push(SPACER, '**what expires**', ...dl.map(l => `• ${l}`)); }
  });
  out.push(SPACER);

  // ── WHAT IS HOLDING WHAT ───────────────────────────────────────────────────
  // Said as a comparison rather than two independent facts, and kept OUTSIDE the per-instrument
  // blocks because that is what it is: an index with nothing expiring near spot is free to trend,
  // and that matters most when it is also the one nearest a wall — a sentence neither row can
  // produce on its own.
...
export function realLevels({ spot, putWall, callWall, byStrike = [], agreement = null, pin = null, levels = null } = {}) {
  if (!(spot > 0)) return [];
  const out = [];
  // With the put side split into its three objects, the call side is described here and the put
  // side by downsideLevels — which never calls a negative node a wall.
  if (levels) {
    if (callWall > 0) {
      const d = wallDominance(byStrike, callWall);
      const kind = levels.callWall?.kind === 'magnet'
        ? 'call wall, but a magnet: positive gamma below spot pulls price back up to it rather than capping it'
        : `call wall, a ceiling${levels.callWall?.inPin ? ' inside the pin band' : ''}`;
      let line = `• **${f2(callWall)}** — ${kind}, ${callWall > spot ? '+' : ''}${((callWall / spot - 1) * 100).toFixed(1)}%.`;
      if (d && !d.twoSided && d.heavy === 'call') line += ` ${d.ratio}x call-heavy.`;
      const a = agreement?.call;
      if (a?.total > 1 && a.agree === 0) line += ` No single expiry peaks exactly here, so read it as **${(callWall - spot * ZONE_PCT / 100).toFixed(0)}–${(callWall + spot * ZONE_PCT / 100).toFixed(0)}** rather than a line.`;
      else if (a?.agree === 1 && a.matched?.length) line += ` Owned by the ${a.matched[0]} expiry — it matters into that date more than today.`;
      else if (a?.agree > 1) line += ` Holds across ${a.agree} of ${a.total} expiries — a standing level rather than one day's book.`;
      out.push(line);
    }
    out.push(...downsideLevels(levels, spot));
    if (pin?.pinned) {
  ...
  const describe = (strike, kinds) => {
    ...
    // Which side the LABEL carries. A shared strike carries both, so it has no side to contradict.
    const labelSide = kinds.startsWith('heaviest call') ? 'call'
                    : kinds.startsWith('heaviest put') ? 'put' : null;
    if (d?.twoSided) bits.push('Roughly two-sided, so it cuts both ways');
    // THE LABEL AND THE HEAVY SIDE CAN DISAGREE, and the short form made that read as a
    // contradiction: "heaviest call positioning ... 1.5x put-heavy". Both halves were true — the
    // strike wins the call count across the board while the puts sitting on it are the bigger
    // number — but the reader was left to reconcile them. Say why they are both true.
    else if (d && labelSide && d.heavy !== labelSide) {
      bits.push(d.heavy === 'put'
        ? `But the puts there outweigh the calls ${d.ratio}x, so it leans as a floor rather than a ceiling`
        : `But the calls there outweigh the puts ${d.ratio}x, so it leans as a ceiling rather than a floor`);
    }
...
// ── THE PUT SIDE, AS THREE OBJECTS ───────────────────────────────────────────
// Observational: what hedging does at each strike, never what to do about it.
export function downsideLevels(lv, spot) {
  const out = [];
  if (!lv || !(spot > 0)) return out;
  const usdM = (v) => v == null ? '—' : `${v < 0 ? '−' : '+'}$${Math.abs(v) >= 1e9 ? `${(Math.abs(v) / 1e9).toFixed(2)}B` : `${Math.round(Math.abs(v) / 1e6)}M`}`;
  const mon = (e) => { const m = /^\d{4}-(\d{2})-(\d{2})/.exec(String(e || '')); return m ? `${['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][+m[1] - 1]}-${m[2]}` : (e || '—'); };
  const sp = lv.support;
  if (sp?.strike) {
    out.push(`• **${f2(sp.strike)}** — put support, −${sp.pctBelow}%. Net gamma is positive here (${usdM(sp.netGexUsd)}, mostly ${mon(sp.expiry)}), so hedging leans against a decline into it. `
      + (sp.peaks > 1 ? `Holds across ${sp.peaks} of ${sp.of} expiries — a standing level rather than one day's book.`
                      : `Owned by the ${mon(sp.expiry)} expiry — it matters into that date more than today.`));
  } else {
    out.push(`• **No put support inside 5%.** Nothing below spot carries positive gamma far enough from price, big enough, and owned by an expiry — there is no cushion on this book, only what expires around spot.`);
  }
  const td = lv.trapdoor;
  const one = (t) => `**${f2(t.strike)}** (${usdM(t.netGexUsd)}, mostly ${mon(t.expiry)}, −${t.pctBelow}%)`;
  if (td?.near || td?.deep) {
    const parts = [td.near ? one(td.near) : null, td.deep ? `deeper ${one(td.deep)}` : null].filter(Boolean);
    out.push(`• Trapdoor ${parts.join(', and ')} — net gamma is negative there, so hedging leans WITH a decline through it. Moves extend below it rather than slow.`);
  }
  return out;
}
