...
import { eventPositioning, EVENTS } from './events.js';
...
export async function assembleRegion(region) {
  ...
  // D2 — event positioning into known catalysts (NVDA Aug 26, …). Global, not region-scoped.
  const eventTrends = await Promise.all(EVENTS.map(e => yahooTrend(e.sym)));
  const events = eventPositioning({ events: EVENTS, trends: eventTrends, nowMs: Date.now() });
  ...
  return { R, quotes, idxRaw, macro, korea, regime, cross, sox: soxRes?.[0] || null, hyg, leaning, csop7709, volTerm, handoff, scenarios, posture, smhSoxx, fxPnl, correlation, events, nqLow, read, marketRegime, ladder, fx, won, intervention };
}
