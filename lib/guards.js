...
export function stopAtrGuard({ width } = {}, cfg = GUARD_CFG) {
  if (!width || !width.known) {
    // A MISSING STOP IS NOT A BROKEN FEED. It is the one cause in this set that is the trader's to
    // fix rather than the system's, and it is a real risk finding rather than an absence of data —
    // so it reads amber where every other unknown reads grey. Lumping it in with "ATR still
    // loading" is what made a genuinely unstopped position look like a temporary blank.
    const st = width?.status ?? null;
    const state = st === 'no-stop' ? 'amber' : 'unknown';
    return g('stopAtr', 'Stop distance', state, { reason: st }, width?.note || 'no stop or no ATR');
  }
  const a = width.atrs;
...
export function addToLoserGuard({ add } = {}) {
  ...
  return g('addToLoser', 'Add to a loser', 'red',
    { addNumber: add.addNumber, priorBuys: add.priorBuys, drawdownPct: add.drawdownPct,
      worseThanAverage: add.worseThanAverage ?? null, adverseSide: add.adverseSide ?? null },
