...
export async function settledGex(symbol, { spot = null, now = new Date(), rate = null, divYield = null,
  ...
  let occ = null, cboe = null, want = expiries, vintage = null;
  let cached = null;
  ...
  if (!fresh) {
    ...
    want = (expiries && expiries.length) ? expiries : defaultExpiries(occ.expiries, now);
  ...
  if (!fresh && !ivBlackout) {
    try {
      await kvSetJson(OCC_CHAIN_KEY(symbol), { at: now.toISOString(), expiries: want, merged, prov });
