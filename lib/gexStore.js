...
export async function settledGex(symbol, { spot = null, now = new Date(), rate = null, divYield = null,
  ...
  let cached = null;
  try { cached = await kvGetJson(OCC_CHAIN_KEY(symbol)); } catch { /* a cache miss costs time, not correctness */ }
  ...
  if (!fresh && !ivBlackout) {
    try {
      await kvSetJson(OCC_CHAIN_KEY(symbol), { at: now.toISOString(), expiries: want, merged, prov, theirs });
