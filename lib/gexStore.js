...
export async function settledGex(symbol, { spot = null, now = new Date(), rate = null, divYield = null,
  ...
  const merged = fresh ? cached.merged : mergeOccIv(occ, cboe.contracts);
  ...
  return {
    ...
    chain: { cached: !!fresh, builtAt: fresh ? cached.at : now.toISOString(),
             ageMin: fresh ? Math.round((now - Date.parse(cached.at)) / 60000) : 0 },
  };
}
