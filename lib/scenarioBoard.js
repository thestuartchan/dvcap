...
export const SCENARIO_DEFS = Object.freeze([
  ...
  { id: 'B', group: 'rates', name: 'Term-premium break', tone: 'amber', side: 'supportive', weight: 6,
    ...
    not: [{ id: 'C', shared: 'Both are yields rising', discriminator: 'curve shape — B is the long end leading and dollar-negative; C is the front end leading and dollar-positive' }],
  ...
  { id: 'C', group: 'rates', name: 'Hawkish repricing', tone: 'amber', side: 'adverse', weight: 6,
    ...
    not: [{ id: 'D', shared: 'Both can come with equities falling', discriminator: 'credit — C without a crack is orderly' },
          { id: 'B', shared: 'Both are yields rising', discriminator: 'curve shape — C flattens and lifts the dollar; B steepens and does not' }],
  ...
  { id: 'GS', group: 'rates', name: 'Growth scare', tone: 'amber', side: 'adverse', weight: 7,
    ...
    not: [{ id: 'SF', shared: 'Both have equities falling', discriminator: 'yields and oil — falling in a growth scare, rising in stagflation' },
          { id: 'D', shared: 'Both have equities falling', discriminator: 'D is credit cracking; a growth scare can run with credit orderly' }],
  ...
  { id: 'SF', group: 'rates', name: 'Stagflation squeeze', tone: 'red', side: 'adverse', weight: 7,
    ...
    not: [{ id: 'C', shared: 'Both push yields higher', discriminator: 'gold — C sells it on rising real yields, stagflation bids it' }],
  ...
  { id: 'D', group: 'stress', name: 'Credit crack', tone: 'red', side: 'adverse', weight: 9,
    ...
    not: [{ id: 'SD', shared: 'Both are equities falling with volatility up', discriminator: 'credit — mechanical de-risking leaves high yield orderly; D does not' }],
...
export function evaluateBoard(barsBySym = {}, { defs = SCENARIO_DEFS } = {}) {
  ...
  const scenarios = defs.map(d => {
    ...
    return {
      ...
      notLines: (d.not || []).map(x => `${x.id ? `Not ${x.id}. ` : ''}${x.shared} — discriminator: ${x.discriminator}.`),
      breaksIf,
      source: 'bars',
    };
  });
