...
export function derivePosition(fills = []) {
  ...
  let qty = 0, avgCost = 0, realized = 0, bought = 0, sold = 0, costOut = 0, proceeds = 0;
  const warnings = [];
  ...
  for (const f of clean) {
    if (f.side === 'buy') {
      ...
      if (sellQty > 0) {
        realized += sellQty * (f.price - avgCost);
        costOut += sellQty * avgCost;
        proceeds += sellQty * f.price;
        qty -= sellQty;
        sold += sellQty;
      }
      if (qty === 0) avgCost = 0;
  ...
  const status = clean.length === 0 ? 'setup' : qty > 0 ? 'open' : 'closed';
  return {
    ...
    bought, sold, proceeds: +proceeds.toFixed(2),
    status, warnings,
...
export function positionPnl(derived, price) {
  ...
  const unrealized = hasLive && derived.avgCost != null
    ? +((p - derived.avgCost) * derived.qty).toFixed(2) : null;
  const unrealizedPct = hasLive && derived.avgCost > 0
    ? +(((p - derived.avgCost) / derived.avgCost) * 100).toFixed(2) : null;
  const marketValue = hasLive ? +(p * derived.qty).toFixed(2) : null;
