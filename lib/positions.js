...
import { sideOf, dirSign, openSideFor, closeSideFor, oppositeSide, DEFAULT_SIDE } from './side.js';
...
// ── THE OTHER COST BASIS ─────────────────────────────────────────────────────
// This engine is average-cost and deliberately so: a sell never moves `avgCost`, which is what
// makes R multiples and scale-out percentages comparable across a position's whole life.
//
// A broker reporting a PARTIALLY EXITED position does something different. It matches the sale to
// specific lots — FIFO unless the account says otherwise — and reports the basis of the lots that
// remain. The two are both right and cannot be equal, which is why reconciliation carries a
// `costBasisAck` for the case.
//
// Worked on ARM, 2026-09-09, and it reconciles exactly:
//   buy 8 @ 321.855 · buy 1 @ 409.17 · sell 8 @ 295.6375 · buy 9 @ 237.30
//   average cost  246.725667   (the sale does not touch it)
//   FIFO basis    254.487      (the sale consumes the 8 @ 321.855, leaving 1 @ 409.17 + 9 @ 237.30)
//   IBKR reports  254.605003   — $1.18 above FIFO, the commission on the 09-03 buy
//
// The divergence is not an error and there is nothing to fix. What was missing was any way to SEE
// that from the console: the average cost was on screen, the lot basis was not, and the broker's
// figure lived only inside a reconciliation nobody opens.
//
// Reported, never used for P&L. Substituting it would corrupt every scale-out percentage, which is
// measured against the average cost at the moment of each sell.
export function fifoOpenLots(fills = [], { side = DEFAULT_SIDE } = {}) {
  const openSide = openSideFor(side);
  const clean = (Array.isArray(fills) ? fills : [])
    .map(f => ({ side: f?.side, qty: num(f?.qty), price: num(f?.price), date: f?.date || '' }))
    .filter(f => (f.side === 'buy' || f.side === 'sell')
      && Number.isFinite(f.qty) && f.qty > 0 && Number.isFinite(f.price) && f.price >= 0)
    .sort((a, b) => String(a.date).localeCompare(String(b.date)));

  const lots = [];
  let overSold = 0;
  for (const f of clean) {
    if (f.side === openSide) { lots.push({ qty: f.qty, price: f.price, date: f.date }); continue; }
    // A close consumes the OLDEST open lots first. Partially consumed lots keep their price and
    // carry the remainder, which is the whole point — a half-eaten lot is still at its own cost.
    let left = f.qty;
    ...
    if (left > 1e-9) overSold += left;
  }
  const qty = lots.reduce((a, l) => a + l.qty, 0);
  const notional = lots.reduce((a, l) => a + l.qty * l.price, 0);
  return {
    lots: lots.map(l => ({ qty: +l.qty.toFixed(6), price: l.price, date: l.date || null })),
    qty: +qty.toFixed(6),
    basis: qty > 1e-9 ? +(notional / qty).toFixed(6) : null,
    overSold: overSold > 1e-9 ? +overSold.toFixed(6) : 0,
  };
}
...
export function derivePosition(fills = [], { multiplier = 1, side = DEFAULT_SIDE } = {}) {
  ...
  return {
    ...
    // The lot basis the broker will report, alongside the average cost this engine runs on. Only
    // meaningful once something has been sold out of a still-open position — before that the two
    // are identical by construction and printing both would be noise.
    ...(() => {
      if (!(qty > 0 && sold > 0)) return { fifoBasis: null, fifoLots: null };
      const f = fifoOpenLots(clean, { side });
      return { fifoBasis: f.basis, fifoLots: f.lots };
    })(),
  };
}

// ── SELLING MORE THAN YOU HOLD ───────────────────────────────────────────────
// derivePosition CLAMPS a close that exceeds the position and warns, which is the right answer for
// a data error — a fill list that closes 400 out of 300 is usually a typo, and inventing a short
// from a typo would be worse than saying so.
//
// But it is not always a typo. On 2026-09-11 a 300-share AAPU exit went out as 400 by mistake and
// was bought back the same minute, and what the market did is not what the clamp describes: the
// extra 100 OPENED A SHORT, and the buy-back covered it. Entered as it happened on one row, the
// clamp swallowed the oversell and then read the cover as a new opening buy — leaving a phantom
// 100-share long nobody held, at a cost basis nobody paid.
//
// So the excess is offered as what it is: this row closes at the size it actually held, and the
// remainder opens a row in the OTHER direction. Two trades, because that is two trades.
//
// Returns null when there is nothing to split — the ordinary case — so a caller can treat a split
// as the exception it is rather than branching on arithmetic.
export function oversellSplit(derived, fill) {
  const held = num(derived?.qty);
  const q = num(fill?.qty);
  // OPENING MORE IS NOT OVERSELLING. Only a CLOSING fill can exceed the position; a buy on a long
  // of any size is simply a bigger position.
  if (!fill || fill.side !== closeSideFor(derived?.side)) return null;
  // NOTHING OPEN IS A DIFFERENT QUESTION and deliberately not answered here. A sell against a flat
  // row is either a short being opened on the wrong row or a position that was never recorded, and
  // guessing between them is how an inverted position gets created.
  if (!(held > 0) || !Number.isFinite(q) || !(q > held + 1e-9)) return null;
  return {
    closeQty: +held.toFixed(6),
    excessQty: +(q - held).toFixed(6),
    // The direction the excess opens in — long oversold is short, short overcovered is long.
    side: oppositeSide(derived?.side),
    fillSide: openSideFor(oppositeSide(derived?.side)),
  };
}
...
// `priceOf(position, level)` — the level is passed because an option row watches its COMBO MARK by
// default and the underlying only where a level says so (lib/instruments.js LEVEL_ON); a share
// row's priceOf ignores the second argument and nothing changes for it.
export function levelHits(positions = [], priceOf = () => null) {
  const out = [];
  for (const pos of positions) {
    for (const lv of (pos.levels || [])) {
      const price = num(priceOf(pos, lv));
