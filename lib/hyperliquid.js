...
export function spotHoldings(balances = [], prices = new Map(),
  ...
  const rows = balances.map(b => {
    // By contract first: pool prices are keyed by address (lib/wallet.js), so two tokens sharing a
    // symbol each get their own. The venue's marks are keyed by symbol and are the fallback.
    const p = (b.address && prices.get(String(b.address).toLowerCase())) || prices.get(b.coin) || {};
    ...
    return {
      ...
      // Tri-state: true swapped for, false checked and not, null never checked. See lib/wallet.js.
      acquired: b.acquired == null ? null : !!b.acquired,
