...
export function spotHoldings(balances = [], prices = new Map(),
  ...
  // THREE TIERS, THEN VALUE. Sorting on value alone put the junk on top: the burn address's
  // fictional $6.2tn airdrop led the list while its real holdings sat below seventy rows of
  // untradeable tokens. What you own comes first, what cannot be valued comes last.
  const tier = (r) => (!r.priced ? 2 : r.thin ? 1 : 0);
  ...
  const solid = rows.filter(r => r.priced && !r.thin);
...
export async function fetchHlSpot({ address = process.env[HL_ADDRESS_ENV], timeoutMs = 8000 } = {}) {
  ...
  try {
    ...
    // THE METADATA IS 270KB and only the quote asset needs no price, so it is fetched only when
    // there is something to price. A wallet holding nothing, or only USDC, pays nothing for this.
    const needsPrices = balances.some(b => b.coin !== HL_SPOT_QUOTE);
    let prices = new Map([[HL_SPOT_QUOTE, { pair: null, price: 1, changePercent: null }]]);
    if (needsPrices) {
      const m = await ask({ type: 'spotMetaAndAssetCtxs' });
      if (m.ok) prices = spotPrices(await m.json());
    }
    return { ok: true, configured: true, error: null, ...spotHoldings(balances, prices) };
  } catch (e) {
