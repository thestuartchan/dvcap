...
// ── FIXED DECIMALS DESTROY A SUB-CENT INSTRUMENT, HERE TOO ───────────────────────────────────
// The same defect lib/price.js exists to prevent, one layer down and worse. SHIB at 0.00000892
// against a stop at 0.0000080 risks 9.2e-7 a unit; `toFixed(6)` made that 0.000001 and the
// following `toFixed(4)` made it ZERO — so the risk budget was divided by nothing, `exact` came
// back Infinity, and the size came out 0. A sizer that answers "none" because the arithmetic
// overflowed looks exactly like a sizer answering "the budget is too small".
// Significant figures have no scale assumption; twelve is float-noise normalisation, not rounding.
const sig = (v, digits = 12) => {
  const n = num(v);
  if (!Number.isFinite(n) || n === 0) return Number.isFinite(n) ? 0 : NaN;
...
//
// A COIN IS NOT A SHARE EITHER, in the opposite direction. Every branch below floors to a whole
// unit, which is right for something you cannot own a third of and wrong for something you can:
// a 1% risk budget on a $200k book against BTC at $79,707 with a $75,000 stop is 0.42 BTC, and
// this returned 0 with "the suggested size rounds to zero" — a sizer that cannot size the position
// at all, failing in the one direction that looks like an answer. Divisible instruments round to a
// STEP instead of an integer.
//
...
export const DIVISIBLE_STEP = 1e-6;   // ~$0.08 of bitcoin — far finer than any sizing decision

export function roundQty(q, multiplier = 1, { divisible = false, step = DIVISIBLE_STEP } = {}) {
  if (!Number.isFinite(q) || q <= 0) return 0;
  // Divisible: floor to the step rather than to a whole unit, and never to zero while the budget
  // buys anything at all. The rounding rationale above is about equity being approximate, which
  // does not stop applying — it just cannot be expressed as an integer here.
  if (divisible) {
    const st = Number.isFinite(+step) && +step > 0 ? +step : DIVISIBLE_STEP;
    const sig = Math.max(0, Math.min(12, -Math.floor(Math.log10(q)) + 2));   // ~3 significant figures
    const grid = Math.max(st, Math.pow(10, -sig));
    return +(Math.floor(q / grid) * grid).toFixed(12);
  }
...
// `vintage` is lib/regimeVintage.js's regimeVintage(): a decayed or expired consensus takes the
// same kind of haircut as a contested regime, and for the same reason — the regime is unresolved.
// The uncertainty haircuts do NOT stack: contested AND expired is one unresolved regime, not two,
// so the deepest applicable haircut is taken and every reason is named.
export function regimeMultiplier({ regimeId, creditDanger = false, contested = false, pinnedDiverged = false, vintage = null, sizing = REGIME_SIZING } = {}) {
  ...
  const unc = [];
  if (contested) unc.push({ why: 'regime contested', h: UNCERTAINTY_HAIRCUT });
  else if (pinnedDiverged) unc.push({ why: 'pinned ≠ live regime', h: UNCERTAINTY_HAIRCUT });
  if (vintage && vintage.grade && vintage.grade !== 'fresh' && vintage.haircut < 1) {
    unc.push({ why: `consensus ${vintage.grade}${vintage.pct != null ? ` (${vintage.pct}% alive)` : ''}`, h: vintage.haircut });
  }
  if (unc.length) {
    const h = Math.min(...unc.map(u => u.h));
    mult *= h;
    reasons.push(`${unc.map(u => u.why).join(' + ')} → ×${h} uncertainty haircut`);
  }
  return { mult: +mult.toFixed(3), base: base.mult, reasons, vintage: vintage ? { grade: vintage.grade, alive: vintage.alive ?? null } : null };
}
...
export function sizeSuggestion({
  ...
  // Spot crypto is divisible; shares and contracts are not. Declared by the caller rather than
  // guessed from the price, because a $79,707 share and a $79,707 coin size very differently.
  divisible = false, step = DIVISIBLE_STEP,
} = {}) {
  ...
  const fullExact = divisible ? exact : Math.floor(exact);
  const fullQty = roundQty(fullExact, m, { divisible, step });
  const roomQty = Math.max(0, +(fullQty - held).toFixed(12));
  const trancheQty = divisible ? +(roomQty / nTranche).toFixed(12) : Math.floor(roomQty / nTranche);
  ...
  // The minimum-tradeable-unit warning is about GRANULARITY setting the position instead of the
  // risk model. A divisible instrument has no such minimum, so it must never fire there.
  if (!divisible && m > 1 && exact < 1) {
    ...
    warnings.push(divisible
      ? 'the risk budget does not buy a meaningful fraction at this price'
      : 'the suggested size rounds to zero — the risk budget is small relative to this price');
  }
