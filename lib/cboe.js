...
// ── NAMED CONTRACTS, WITH THEIR PUBLISHED GREEKS ─────────────────────────────
// A different question from the gamma walls, and it needs a different pass. parseCboe filters to a
// strike band and drops zero open interest, both correct for measuring where the market's gamma
// sits — and both wrong for looking up a contract someone HOLDS. A position does not stop existing
// because nobody else is in it, and a deep-in-the-money leg outside the band is exactly the one
// carrying the delta.
//
// Delta, gamma, theta and vega come back as CBOE publishes them. They are exchange numbers, not
// ours: no assumed rate, no assumed dividend, no Black-Scholes of our own — which is the property
// that makes a book exposure total a summing exercise rather than a modelling one.
export function pickCboeGreeks(json, wanted = []) {
  const spot = num(json?.data?.current_price);
  const raw = Array.isArray(json?.data?.options) ? json.data.options : [];
  ...
  for (const o of raw) {
    const p = parseOsi(o?.option);
    if (!p) continue;
    const key = `${p.root}|${p.expiry}|${p.type === 'call' ? 'C' : 'P'}|${p.strike}`;
    if (want.size && !want.has(key)) continue;
    matched++;
    out[key] = {
      // Greeks are per SHARE per unit — the caller multiplies by quantity and contract size.
      delta: num(o?.delta), gamma: num(o?.gamma), theta: num(o?.theta), vega: num(o?.vega),
      iv: num(o?.iv), oi: num(o?.open_interest),
      bid: num(o?.bid), ask: num(o?.ask), mark: num(o?.theo),
      strike: p.strike, expiry: p.expiry, type: p.type,
    };
  }
  return { spot, asOf: cboeAsOf(json?.timestamp), greeks: out, matched, requested: want.size || null };
}

// ── THE CHAIN AS A LIST, FOR AN ENTRY FORM ───────────────────────────────────
// Which expiries this root trades and which strikes each carries — no greeks, no open interest,
// nothing about any position. The brief asked for a pre-fill "from IBKR contract lookup where
// possible"; IBKR is read through Flex here, an end-of-day statement with no chain in it, so the
// list comes from the exchange's own delayed feed instead, which is the same chain.
export function pickCboeIndex(json, { today = new Date().toISOString().slice(0, 10) } = {}) {
  const spot = num(json?.data?.current_price);
  const raw = Array.isArray(json?.data?.options) ? json.data.options : [];
  if (!raw.length) return null;
  ...
  for (const o of raw) {
    ...
    if (!byExpiry.has(p.expiry)) byExpiry.set(p.expiry, new Set());
    byExpiry.get(p.expiry).add(p.strike);
  }
  const expiries = [...byExpiry.entries()].sort(([a], [b]) => a.localeCompare(b))
    .map(([expiry, ks]) => ({ expiry, strikes: [...ks].sort((a, b) => a - b) }));
  return { spot, asOf: cboeAsOf(json?.timestamp), expiries, contracts: raw.length };
}

export async function fetchCboeIndex(symbol, { timeoutMs = CBOE_TIMEOUT_MS } = {}) {
  try {
    const r = await fetch(CBOE_URL(symbol), { headers: { Accept: 'application/json' }, signal: AbortSignal.timeout(timeoutMs) });
    if (!r.ok) return { ok: false, symbol, reason: `CBOE answered ${r.status}` };
    const idx = pickCboeIndex(await r.json());
    if (!idx) return { ok: false, symbol, reason: 'CBOE payload carried no options' };
    return { ok: true, symbol, ...idx };
  } catch (e) {
    return { ok: false, symbol, reason: `CBOE fetch failed — ${String(e?.message || e)}` };
  }
}

export async function fetchCboeGreeks(symbol, wanted = [], { timeoutMs = CBOE_TIMEOUT_MS } = {}) {
  try {
    ...
    if (!r.ok) return { ok: false, symbol, reason: `CBOE answered ${r.status}` };
    const picked = pickCboeGreeks(await r.json(), wanted);
    if (!picked) return { ok: false, symbol, reason: 'CBOE payload carried no options' };
    return { ok: true, symbol, ...picked };
  } catch (e) {
    return { ok: false, symbol, reason: `CBOE fetch failed — ${String(e?.message || e)}` };
  }
}
...
export function cboeSummary(contracts = [], spot) {
  ...
  const cw = heaviest('call'), pw = heaviest('put');
  // THE SIGN AT THEIR WALL. CBOE's put wall is the heaviest put-side strike, which is the
  // definition this panel retired for its own put side: it can be a positive node (a support)
  // or a negative one (a trapdoor), and the cross-check has to know which before it compares.
  const netAt = (k) => rows.find(r => r.strike === k)?.netGexUsd ?? null;
  return {
    callWall: cw, putWall: pw, callWallNet: netAt(cw), putWallNet: netAt(pw),
...
// ── WHERE THEIR STRIKE SITS ON OUR MAP ──
// When we show no qualifying support, "✗ disagrees on put wall" compares two differently-defined
// numbers. The honest line says what their strike IS on our side: inside the pin box, our
// trapdoor, or a candidate that failed a named test.
function whereOnOurs(levels, k) {
  const pin = levels?.pin;
  if (pin?.pinned && k >= pin.lo && k <= pin.hi) return `their ${k} is inside our pin box ${pin.lo}–${pin.hi}`;
  const td = levels?.trapdoor;
  if (td?.near?.strike === k || td?.deep?.strike === k) return `their ${k} is our trapdoor (negative net gamma there)`;
  const rej = (levels?.support?.rejected || []).find(r => r.strike === k);
  if (rej) return `their ${k} fails our support test — ${rej.why}`;
  return `their ${k} is not a qualifying level on our map`;
}

export function compareGex(ours, theirs, { spot = null, tol = TOL, levels = null } = {}) {
  ...
  // ── LIKE WITH LIKE ──
  // Their put wall is the heaviest put-side strike, sign untested. Ours is one of three objects.
  // A negative node on their side is compared to our TRAPDOOR; a positive one to our PUT
  // SUPPORT; and when we show no qualifying support the line is context, not a ✗. The check's
  // name says which comparison was made. Without `levels` the legacy put-wall comparison runs.
  if (levels) {
    const theirK = num(theirs.putWall), theirNet = num(theirs.putWallNet);
    if (theirK == null) {
      checks.push(check('put wall ↔ put support', num(levels.support?.strike), null, { state: 'unknown', detail: 'CBOE did not solve a put wall' }));
    } else if (theirNet != null && theirNet < 0) {
      const cands = [levels.trapdoor?.near?.strike, levels.trapdoor?.deep?.strike].filter(v => v != null);
      const oursK = cands.length ? cands.reduce((a, b) => Math.abs(b - theirK) < Math.abs(a - theirK) ? b : a) : null;
      const c = wall('put wall ↔ trapdoor', oursK, theirK);
      c.detail = `their put wall is a negative node, so it is compared to our trapdoor — ${c.detail}`;
      checks.push(c);
    } else {
      const oursK = num(levels.support?.strike);
      if (oursK == null) {
        checks.push(check('put wall ↔ put support', null, theirK, { state: 'context', score: false,
          detail: `CBOE put wall ${theirK} — we show no qualifying support; ${whereOnOurs(levels, theirK)}` }));
      } else {
        const c = wall('put wall ↔ put support', oursK, theirK);
        c.detail = `their put wall is a positive node, so it is compared to our put support — ${c.detail}`;
        checks.push(c);
      }
    }
  } else {
  ...
  const oiCall = within('call OI', num(ours.callOi), num(theirs.callOi), tol.oiPct, 'pct');
  const oiPut = within('put OI', num(ours.putOi), num(theirs.putOi), tol.oiPct, 'pct');
  // The basis is named when the caller narrowed it. A residual gap on the same contracts is a
  // settlement-vintage gap — OCC's overnight file against CBOE's earlier snapshot — and reads so.
  if (theirs.oiBasis) for (const c of [oiCall, oiPut]) if (c.state !== 'unknown') c.detail += ` on ${theirs.oiBasis}${c.state === 'differ' ? ' — the same contracts, so this is settlement vintage, not universe' : ''}`;
  checks.push(oiCall, oiPut);
