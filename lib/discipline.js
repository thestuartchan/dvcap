...
import { openSideFor } from './side.js';
...
// ADDING IS NOT BUYING. On a short, committing more capital to a losing idea means SELLING more —
// so a test written as `side !== 'buy'` did not merely miss the case, it inverted it: every add to
// a losing short went unrecorded, while every buy-to-cover (which REDUCES the risk) would have been
// flagged as the pattern. The interceptor's whole purpose is the direction it was blind to.
export function addToLoser({ derived, pct, side = 'buy', fillPrice = null, positionSide = null } = {}) {
  const qty = num(derived?.qty);
  const p = num(pct);
  const opens = openSideFor(positionSide ?? derived?.side);
  if (side !== opens || !(qty > 0) || p == null || p >= 0) return null;
  const buys = (derived?.fills || []).filter(f => f.side === opens).length;
  ...
  return {
    ...
    // RENAMED FROM `belowAverage`, because that name was itself long-only. A long adds adversely
    // BELOW its average cost; a short adds adversely ABOVE its average sale price — in both cases
    // the fill is on the side the position has already moved against you. The old name would have
    // printed "below your 100.00 average" under a fill at 106, which is not a wording problem so
    // much as a claim that is simply untrue. It is not the CONDITION either way (the position being
    // down is), but it is the classic shape and worth naming.
    worseThanAverage: (fp != null && avg != null) ? (opens === 'buy' ? fp < avg : fp > avg) : null,
    // The direction the adverse side lies in, so a caller can word it without re-deriving the side.
    adverseSide: opens === 'buy' ? 'below' : 'above',
