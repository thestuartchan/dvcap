...
export function composePosture({ scenarios = [], leaning, volTerm, credit, tape = null, regime = null,
  ...
  if (nothing) flipsIf = 'any input arrives — a scenario, a gauge with a print, or a tape reading';
    ...
    flipsIf = `the score drops below 2.5 — now ${score.toFixed(1)}${reasons.length ? ` from ${reasons.join(', ')}` : ', from confirmed adverse scenarios'}`;
  }
    ...
    flipsIf = `to RISK-ON at score ≤ 0, to RISK-OFF at ≥ 2.5 — now ${score.toFixed(1)}${reasons.length ? ` (${reasons.join(', ')})` : ''}`;
  }
