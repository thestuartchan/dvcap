...
import { isHlPerp } from './hyperliquid.js';
...
// WHICH BOOK a row belongs to. Deliberately NOT isSpotCrypto: that answers a narrower question —
// "is this divisible into fractions of a unit" — and a Hyperliquid perp is not, because 133 of the
// venue's markets trade in whole units. Asked the narrow question, HL:BTC was filed under TradFi:
// the most crypto thing in the book, in the wrong half of it. Rendering a mixed card is what
// showed it, which is the argument for rendering one.
export const isCryptoAsset = (symbol) => isSpotCrypto(symbol) || isHlPerp(symbol);

export function splitByClass(items = [], symbolOf = (x) => x?.symbol) {
  const crypto = [], tradfi = [];
  for (const it of items) (isCryptoAsset(symbolOf(it)) ? crypto : tradfi).push(it);
...
// ── HOW MANY DECIMALS A PRICE OF THIS THING DESERVES ─────────────────────────────────────────
// Two is right for a share: it is quoted in cents and nothing finer, and an average cost carrying
// five is division residue rather than precision. A coin is quoted finer — XRP prints 2.4471, and
// the eight hundredths of a percent between that and 2.45 is a real quote.
//
// Only ABOVE a dollar does this matter. Below one, lib/price.js is already on significant figures
// for everything, because two decimals would turn MJY's 0.0064 into 0.01.
export const CRYPTO_PRICE_DP = 4;
export const EQUITY_PRICE_DP = 2;
export const priceMaxDp = (symbol) => isCryptoAsset(symbol) ? CRYPTO_PRICE_DP : EQUITY_PRICE_DP;
