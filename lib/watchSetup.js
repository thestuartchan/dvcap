...
import { watchName } from '../data/watchMeta.js';
...
// `bars` are daily {date, high, low, close}, oldest first, as lib/yahoo.js hands them over. The
// last bar is the session Yahoo calls "today": before the open that is yesterday, complete; during
// the session it is today, partial. `open` says which, so "yesterday" is picked correctly.
export function setupStats(bars = [], { open = false } = {}) {
...
export function setupTags(row, { earnings = null, today = null, tomorrow = null } = {}) {
  ...
  if (s) {
    ...
    const coil = [];
    if (s.atrPctile != null && s.atrPctile <= COIL_PCTILE_MAX) coil.push(`14-day range at the ${ordinal(s.atrPctile)} percentile of its year`);
...
export function setups(rows = [], { earnings = {}, today = null, tomorrow = null, max = SETUP_MAX } = {}) {
  ...
  for (const r of rows) {
    ...
    out.push({ sym: r.sym, name: watchName(r.sym), price: num(r.ext?.price) ?? num(r.price), tags, gap, coil });
  }
  return out
...
export function renderSetups(list = [], { extended = null } = {}) {
  const lines = list.map(r => {
    ...
    return `• ${ident} · ${fmtPx(r.price)} — ${r.tags.map(t => t.text).join(' · ')}`;
  });
