...
export async function snapshotSymbol(symbol, { now = new Date(), bandPct = STRIKE_BAND_PCT, gapMs = FETCH_GAP_MS } = {}) {
  ...
  return {
    ...
    // Surfaced, not swallowed: a snapshot where a sixth of the book has no usable vol is a
    // materially thinner measurement than one where all of it does, and the panel should say so.
    ivRejected, ivUsable: contracts.filter(c => c.iv != null).length,
    ok: contracts.length > 0 && failed.length === 0,
  };
}
