...
export async function snapshotSymbol(symbol, { now = new Date(), bandPct = STRIKE_BAND_PCT, gapMs = FETCH_GAP_MS } = {}) {
  ...
  if (!front || front.spot == null) {
    ...
    return { symbol, spot: null, asOf: new Date().toISOString(), date: new Date().toISOString().slice(0, 10),
      ...
      ivBroken: false, frontStarved: false, ivOnly: false,
  ...
  return {
    ...
    // `reason` is a cascade and reports only the first failure, which is enough to tell a human
    // what went wrong and not enough for a caller to decide what to do about it. The vol surface
    // is the one failure a caller CAN route around: open interest is settled overnight and does
    // not move intraday, so a chain whose only defect is its quotes can be answered from the last
    // stored chain repriced at the live spot. A starved front expiry or missing open interest
    // cannot — there is no positioning to reprice. So the flags come out individually, and
    // `ivOnly` says the vol surface is the sole thing standing between here and a usable answer.
    ivBroken, frontStarved,
    ivOnly: ivBroken && !frontStarved && failed.length === 0
      && coverage >= MIN_OI_COVERAGE && frontCoverage >= 0.5,
