...
export const PUBLIC_FIELDS = Object.freeze(['symbol', 'label', 'side', 'price', 'avg', 'pct', 'r', 'rReconstructed', 'exit', 'closedOn', 'realisedPct', 'stop', 'targets', 'trims', 'days', 'since', 'status', 'flags', 'tags']);
...
import { isCryptoAsset, splitByClass, priceMaxDp } from './crypto.js';

// ── TRADFI AND CRYPTO ARE TWO BOOKS ──────────────────────────────────────────
// They do not share a session, a settlement, a weekend or a volatility regime, and reading a
// coin's overnight move next to a share's close invites a comparison that means nothing. Split
// wherever a list of trades appears.
//
// SPLIT ONLY WHEN BOTH ARE PRESENT — the same rule the SHORT marker follows. A heading over an
// all-equity book is a label that never varies, which is why the direction column was removed from
// this card in the first place; adding two of them back for an empty half would repeat the mistake.
export const ASSET_CLASS = Object.freeze({ crypto: 'Crypto', tradfi: 'TradFi' });
export const classOfView = (v) => (isCryptoAsset(v?.symbol) ? 'crypto' : 'tradfi');
export { splitByClass };
...
export function publicView(row, { today = new Date().toISOString().slice(0, 10) } = {}) {
  ...
  const stopLevel = levels.find(l => l.kind === 'stop') || null;
  const stop = stopLevel?.at ?? null;
  ...
  return {
    ...
    // ── WHERE THE RISK UNIT CAME FROM ─────────────────────────────────────────
    // R is only as good as the stop under it, and a stop entered after the trade closed is a
    // different kind of evidence from one entered before it opened: the first is what you
    // committed to, the second is what you remember committing to, written knowing the outcome.
    // The archive can be edited so a missing stop can be put back — which is right, because
    // without one a closed trade prints no R at all — and this is what stops the R that follows
    // from reading as though it had always been there.
    rReconstructed: stopLevel?.backfilled ? true : null,
...
// Sub-dollar prices need significant figures, not four decimals: 0.006452 and 0.00629 both
// rendered "0.0065"/"0.0063" and lost the difference. lib/price.js owns the rule.
//
// ABOVE a dollar the cap depends on the instrument, so the formatter is bound to the SYMBOL rather
// than called bare: a share is quoted in cents and an average cost carrying five decimals is
// division residue, while a coin is genuinely quoted finer and XRP's 2.4471 is a real price.
const pxFor = (symbol) => (v) => fmtPrice(v, { maxDp: priceMaxDp(symbol) });
...
export function tradeLine(v, detail = 'full', { showSide = false } = {}) {
  ...
  const p = pxFor(v.symbol);
  const bits = [`${dot(dir)}${showSide ? ` \`${isShort(v.side) ? 'S' : 'L'}\`` : ''} **${v.symbol}** ${p(v.price)}`];
  if (v.avg != null && detail !== 'minimal') bits.push(`avg ${p(v.avg)}`);
  ...
  if (v.stop && detail !== 'minimal') {
    ...
    bits.push(`SL ${p(v.stop.at)}${dist(v.stop.dist)}${floor}`);
  }
  if (v.targets.length && detail !== 'minimal') {
    ...
    bits.push(`TP ${t.map(x => `${p(x.at)}${x.r != null ? ` (+${x.r}R)` : dist(x.dist)}`).join(', ')}`);
  }
...
export function buildCard(rows = [], { updatedAt = null, title = 'Portfolio' } = {}) {
  ...
  // Each half gets its own budget so a long equity book cannot crowd the coins off the card, and
  // the heading is only drawn when there is something on both sides of it.
  const o = splitByClass(open);
  const block = (list, budget) => fitLines(sortForCard(list), budget).body;
  const description = !open.length ? '_Nothing open._'
    : !o.mixed ? block(open, DESC_BUDGET)
    : `__**${ASSET_CLASS.tradfi}**__\n${block(o.tradfi, Math.floor(DESC_BUDGET / 2) - 40)}`
      + `\n\n__**${ASSET_CLASS.crypto}**__\n${block(o.crypto, Math.floor(DESC_BUDGET / 2) - 40)}`;

  const fields = [];
  const watchLine = (v) => `${DOT.watching} **${v.symbol}** ${v.label || 'levels set'}`;
  if (setups.length) {
    const w = splitByClass(setups);
    if (!w.mixed) {
      fields.push({ name: `Watching · ${setups.length}`, value: setups.map(watchLine).join('\n').slice(0, 1024) });
    } else {
      // Two fields rather than one with a divider: Discord lays fields out side by side, so the
      // split is structural instead of a heading the reader has to notice inside a block of text.
      for (const [key, list] of [['tradfi', w.tradfi], ['crypto', w.crypto]]) {
        fields.push({ name: `Watching · ${ASSET_CLASS[key]} · ${list.length}`,
                      value: list.map(watchLine).join('\n').slice(0, 1024), inline: true });
      }
    }
  }
...
export function closedLine(v) {
  ...
  if (v.exit != null) bits.push(pxFor(v.symbol)(v.exit));
  ...
  // A DAGGER, NOT A SENTENCE. An R rebuilt from a stop entered after the exit is still worth
  // printing — it is the best available answer — but it must be distinguishable from one the trade
  // was actually taken against, and a clause per line would drown the column it sits in. The
  // footnote under the card says what the mark means.
  if (v.r != null) bits.push(`**${v.r > 0 ? '+' : ''}${v.r}R**${v.rReconstructed ? '†' : ''}`);
  if (v.days != null) bits.push(`held ${v.days}d`);
  return bits.join(' · ');
}
...
export function buildClosedCard(rows = [], { updatedAt = null, limit = 25, today = new Date().toISOString().slice(0, 10), windowDays = CLOSED_WINDOW_DAYS } = {}) {
  ...
  const rebuilt = recent.filter(v => v.r != null && v.rReconstructed).length;
  const c = splitByClass(recent);
  const lines = (list) => list.map(closedLine).join('\n');
  const body = !recent.length ? '_Nothing closed in the last ' + windowDays + ' days._'
    : !c.mixed ? lines(recent).slice(0, DESC_BUDGET)
    : (`__**${ASSET_CLASS.tradfi}**__\n${lines(c.tradfi)}`
       + `\n\n__**${ASSET_CLASS.crypto}**__\n${lines(c.crypto)}`).slice(0, DESC_BUDGET);
  ...
  return {
    embeds: [{
      ...
      // ── THE FOOTER CARRIES BOTH ABSENCES ──────────────────────────────────
      // A missing R and a rebuilt R are two different gaps in the same column, and a card that
      // names only one of them implies the rest are solid. † is explained wherever it appears,
      // even when every trade on the card has an R.
      ...((() => {
        const notes = [
          recent.length && withR < recent.length
            ? `${recent.length - withR} closed without a recorded stop, so there is no R to show for them` : null,
          rebuilt ? `† R from a stop recorded after the trade closed` : null,
        ].filter(Boolean);
        return notes.length ? { footer: { text: notes.join(' · ') } } : {};
      })()),
      ...(updatedAt ? { timestamp: updatedAt } : {}),
    }],
  };
...
export function buildAlert(ev, { mentionId = null } = {}) {
  ...
  if (ev.kind === 'opened') { const p = pxFor(v.symbol);
    return { content: `${who}🔔 New trade — ${name}${v.avg == null ? '' : ` · avg ${p(v.avg)}`}${v.stop ? ` · SL ${p(v.stop.at)}` : ''}`, allowed_mentions: { users: mentionId ? [mentionId] : [] } }; }
  if (ev.kind === 'closed') {
