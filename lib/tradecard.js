...
export const PUBLIC_FIELDS = Object.freeze(['symbol', 'label', 'price', 'avg', 'pct', 'r', 'exit', 'closedOn', 'realisedPct', 'stop', 'targets', 'trims', 'days', 'since', 'status', 'flags', 'tags']);
...
//
// LONGS ONLY, which is what the console models (see the header note on direction markers). For a
// short the same geometry means the opposite and this would be actively wrong, so it returns null
// unless the stop is genuinely above the average.
export function lockedPct(stopAt, entry) {
  const s = num(stopAt), e = num(entry);
  if (s == null || e == null || e <= 0) return null;
  if (s <= e) return null;                   // an ordinary stop — R already describes it
...
export function publicView(row, { today = new Date().toISOString().slice(0, 10) } = {}) {
  ...
  return {
    ...
    stop: levels.filter(l => l.kind === 'stop')
      .map(l => ({ ...lvl(l, price, entry, stop), locked: lockedPct(l.at, entry) }))[0] || null,
...
export const dirOf = (pct, mark, avg) => {
  if (pct != null && pct !== 0) return pct;
  if (mark != null && avg != null && mark !== avg) return mark - avg;
...
export function tradeLine(v, detail = 'full') {
  const dir = dirOf(v.pct, v.price, v.avg);
  const bits = [`${dot(dir)} **${v.symbol}** ${px(v.price)}`];
