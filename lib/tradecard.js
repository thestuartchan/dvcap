...
import { isSpotCrypto } from './crypto.js';
...
export const classOfView = (v) => (isSpotCrypto(v?.symbol) ? 'crypto' : 'tradfi');
export function splitByClass(views = []) {
  const tradfi = views.filter(v => classOfView(v) === 'tradfi');
  const crypto = views.filter(v => classOfView(v) === 'crypto');
