...
export function buildCard(rows = [], { stats = null, updatedAt = null, title = 'Open trades' } = {}) {
  ...
  // The footer says what the closed record looks like, and nothing else. "live · updates in place"
  // was describing the delivery mechanism to someone who has no way to care about it, and Discord
  // already renders the embed timestamp underneath — so the footer was announcing that it was
  // fresh, immediately above the time proving it.
  const footParts = [];
  ...
  return {
    embeds: [{
      ...
      color: EMBED_COLOUR,
      fields,
      ...(footParts.length ? { footer: { text: footParts.join(' · ') } } : {}),
