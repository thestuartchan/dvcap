...
export function actionItems({ rows = [], hits = [], swingLines = [], atrOf = () => null, priceOf = () => null, today = new Date().toISOString().slice(0, 10) } = {}) {
  ...
  for (const h of hits) {
    ...
    items.push({ id: r.id, rank: 2, kind: 'alert', tone: 'warn', text: `${nameOf(r)} — ${h.level.kind} level ${h.level.at}${h.level.to ? '–' + h.level.to : ''} hit, ${what} ${h.price}${h.level.note ? ` · ${h.level.note}` : ''}` });
  }
  for (const r of live) {
