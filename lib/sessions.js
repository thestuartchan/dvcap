...
export function closedExchanges(symbols = [], now = new Date()) {
  ...
  const shut = keys.filter(k => {
    ...
    if (!ex) return false;
    // Futures run the globex week and are not what a regional equity pre-read is about; they
    // never make a region look open on a day its cash markets are shut.
    if (ex.globex) return false;
    const d = localDateStr(ex.tz, now);
    return isWeekendIn(ex.tz, now) || (HOLIDAYS[k]?.closed || []).includes(d);
  });
  const cash = keys.filter(k => EXCHANGES[k] && !EXCHANGES[k].globex);
  return {
    exchanges: cash,
    closed: shut,
