...
// The comment four lines above has always said 'delayed' is "what stops the blanket ⚠️ stale badge
// from firing on live-but-delayed feeds". freshness() returned 'stale' instead, and the state did
// not exist.
//
// It matters because the keyless feed is DELAYED on every Asian and European venue — twenty
// minutes on Seoul, fifteen on Hong Kong — while the freshness budget was a flat thirty minutes
// written for a real-time feed. A twenty-minute-delayed quote therefore sits between 20 and 35
// minutes old for the whole session and crosses the line at random.
//
// MEASURED on 2026-09-10: at 00:56Z the Korean quotes were 20 minutes old and correctly read
// `live`; nine minutes later the live brief carried "⚠️ Equity prints are stale — the market is
// open but these are prior closes". The prints were not prior closes. They were exactly as fresh
// as that feed ever gets, and the same flag suppressed the foundry/memory split and the AI axis —
// so a false alarm silently removed two real readings from the brief.
//
// The budget is now the venue's own delay PLUS the intraday cadence, so "late" means late for that
// feed rather than late for a feed we do not have.
const FEED_DELAY_MIN = {
  SEHK: 15, KRX: 20, TWSE: 20, TSE: 20,      // Yahoo's published delays for these venues
  EURONEXT: 15, XETRA: 15, LSE: 15,
  US: 0,                                      // US equity quotes arrive effectively real time
  CME: 10, CRYPTO: 0,
};
export const feedDelayFor = (sym) => FEED_DELAY_MIN[exchangeFor(sym)] ?? 0;
...
export function freshness(sym, q, now = Date.now()) {
  ...
  // No timestamp is not the same as an old one, and it is not evidence of staleness either — it is
  // the absence of evidence. Reported as its own state so a caller can decide, rather than being
  // silently folded into 'stale' as it was.
  if (ageMin == null) return { state: 'undated', ageMin: null };
  const delay = feedDelayFor(sym);
  if (ageMin <= delay + CADENCE_MIN.intraday) {
    // Inside the budget. 'delayed' where the venue's feed is known to lag, so the brief can say
    // the print is behind without claiming it is wrong.
    return delay > 0 && ageMin > CADENCE_MIN.intraday / 2
      ? { state: 'delayed', ageMin, delay }
      : { state: 'live', ageMin };
  }
  return { state: 'stale', ageMin };
}
...
export function freshnessText(fr) {
  ...
  switch (fr.state) {
    ...
    case 'delayed':     return `delayed feed · ${humanizeAge(fr.ageMin)}`;
    // An undated print is not claimed to be fresh and not accused of being old.
    case 'undated':     return 'no timestamp';
...
// ── HOW OLD IS THE LAST CLOSE? ───────────────────────────────────────────────
// A pre-read is composed before its own market opens, so it always quotes a PRIOR close. That is
// the normal state and says nothing. What varies, and does say something, is how far back that
// close is: on a Tuesday it is yesterday, on a Monday it is three days ago, and after a holiday
// Monday it is four. Stale-by-a-weekend is a different read from stale-by-a-day, and the caveat
// that used to fire here fired identically in all three cases.
//
// Walks back day by day from `now` in the exchange's own timezone, skipping weekends and the
// full-day closures in data/holidays.json. Returns { date, daysBack } or null if the exchange is
// unknown. Bounded at 10 days so an exhausted holiday file cannot loop.
export function previousSessionDate(sym, now = new Date()) {
  const ex = exchangeFor(sym);
  if (!ex) return null;
  const tz = EXCHANGES[ex]?.tz;
  if (!tz) return null;
  const closed = new Set(HOLIDAYS?.[ex]?.closed || []);
  const startMs = Date.parse(`${localDateStr(tz, now)}T12:00:00Z`);
...
// ── HOW LONG UNTIL THIS MARKET DOES SOMETHING ────────────────────────────────
// The brief could say a market was "closed" but never how long that would last, so a reader at
// 06:45 in Hong Kong and one at 08:55 got the same word for a two-hour wait and a five-minute one.
// Returns minutes to the next open (or to the close, when it is already trading), measured in the
// exchange's own clock so no UTC arithmetic happens here either.
//
// IT LOOKS PAST TODAY, and it has to. The Asia brief fires at 07:00 HKT, which is 19:00 the
// previous evening in New York — the US session has finished, so a today-only countdown had
// nothing to measure and the line read "🇺🇸 US closed". That is true, useless, and the single
// question an Asian or European reader most wants a US answer to. It now walks forward to the next
// day the exchange actually trades, skipping weekends and its own holiday calendar, and returns
// the wait to that open.
//
// The walk is bounded at ten days. An exchange with no open inside ten days means the holiday file
// has run out (data/holidays.json ends 2026-12-31) or the calendar is wrong, and a wrong countdown
// stated confidently is worse than none — so the bound returns null rather than a number.
const LOOKAHEAD_DAYS = 10;

export function sessionCountdown(sym, now = new Date()) {
  const exKey = exchangeFor(sym);
  const ex = EXCHANGES[exKey];
  if (!ex) return { phase: 'closed', toOpen: null, toClose: null, tz: null, nextDay: false };
  if (ex.always) return { phase: 'live', toOpen: null, toClose: null, tz: ex.tz, nextDay: false };
  const phase = sessionPhase(sym, now);
  const t = localMinutesOfDay(ex.tz, now);
  const open = ex.sessions[0][0];
  const close = ex.sessions[ex.sessions.length - 1][1];

  if (phase === 'live')  return { phase, tz: ex.tz, toOpen: null, toClose: close - t, nextDay: false };
  if (phase === 'lunch') return { phase, tz: ex.tz, toOpen: ex.sessions[1][0] - t, toClose: null, nextDay: false };
  if (phase === 'pre')   return { phase, tz: ex.tz, toOpen: open - t, toClose: null, nextDay: false };

  // Shut for the day, for a weekend, or for a holiday — all three walk forward the same way.
  // Measured in the exchange's OWN local days, because "tomorrow" in Seoul and "tomorrow" in UTC
  // are different dates for most of the hours this brief runs in.
  const DAY = 24 * 60;
  // Minutes from now until local midnight tonight, then whole days on top of that.
  let mins = DAY - t;
  for (let d = 1; d <= LOOKAHEAD_DAYS; d++) {
    const probe = new Date(now.getTime() + (mins + open) * 60000);
    const wd = new Intl.DateTimeFormat('en-US', { timeZone: ex.tz, weekday: 'short' }).format(probe);
    const trades = wd !== 'Sat' && wd !== 'Sun'
      && !(HOLIDAYS[exKey]?.closed || []).includes(localDateStr(ex.tz, probe));
    if (trades) return { phase, tz: ex.tz, toOpen: mins + open, toClose: null, nextDay: true };
    mins += DAY;
  }
  return { phase, tz: ex.tz, toOpen: null, toClose: null, nextDay: false };
}
