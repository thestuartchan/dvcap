...
// ── NO INSTRUMENT BESIDE THE NAME ────────────────────────────────────────────
// The row used to end with the leveraged or inverse product for the direction of the move —
// "[7709.HK up to 2x · HKEX]" after a rising SK Hynix. On a public channel that reads as "here is
// how to go long it", a trade suggestion from a list whose only claim is that a name moved and
// whether its peers agreed. Removed 2026-09-21, with the map that fed it.
{
  const { WATCH_UNIVERSE, IN_WATCH_UNIVERSE } = await import('../data/watchUniverse.js');
  const { existsSync } = await import('node:fs');
  eq('the adjacent-instrument map is gone', existsSync(new URL('../data/adjacent.js', import.meta.url)), false);
  const src = (await import('node:fs')).readFileSync(new URL('../lib/watchlist.js', import.meta.url), 'utf8');
  ok('and the watchlist no longer renders one', !/renderAdjacent|adjacentFor/.test(src));
  ok('no leverage factor reaches a row', !/\d+x\b|up to 2x/.test(src.replace(/\/\/[^\n]*/g, '').replace(/\/\*[\s\S]*?\*\//g, '')));

  // ── THE UNIVERSE ───────────────────────────────────────────────────────────
  // By CORE ticker: nothing Hong Kong-listed reaches the US brief, and nothing US-listed is
  // filed under Asia.
  ...
  // LEVERAGED PRODUCTS ARE NOT ENTRIES. A leveraged product is a way to express a view on
  // something else; the underlying is what moved, and the underlying is what is listed.
  for (const t of ['AMDL', 'IRE', 'RGTZ', 'TSLL', 'UNHG', 'METU', 'METD', 'RGTX', '7709.HK', '7747.HK', '7347.HK']) {
    eq(`${t} is not a watchlist entry`, IN_WATCH_UNIVERSE.has(t), false);
  }
  ok('while their underlyings are', ['AMD', 'IREN', 'RGTI', 'TSLA', 'UNH', 'META', '000660.KS', '005930.KS'].every(c => IN_WATCH_UNIVERSE.has(c)));
